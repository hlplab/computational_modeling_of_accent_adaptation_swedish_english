...
```{r}
d.english.d <- d.stims %>% 
  filter(NativeLanguage == "English" & Accent == "non-native" & Sound == "/d/") 


m.eng.d <- lm(vowel ~ 1 + (number_syllables + has_stress), data = d.english.d, offset = offset_vowel)

tidy(m.eng.d)

[[2,2]]

length(d.english.d$has_stress)

d.native.english %<>%  mutate(offset_segment = tidy(m.eng.native)[[2,2]])

tidy(m.eng.native)[[2,2]]

d.native.swedish <- d.stims %>% 
  filter(NativeLanguage == "Swedish" & Accent == "native" & Block.Type != "exposure") 


m.swe.native <- lm(vowel ~ 1 + (number_segments + has_long_vowel), data = d.native.swedish)
d.native.swedish %<>%  mutate(offset_segment = tidy(m.swe.native)[[2,2]])
tidy(m.eng.native)
...
Model.Accent.matchesGroup.labs <- c("Model Mismatches Group", "Model Matches Group")
names(Model.Accent.matchesGroup.labs) <- c("no", "yes")
...
ggscatter(data = d.test.IO %>%
  ...
  facet_grid(NativeLanguage + Group ~ Model.Accent.matchesGroup, 
             ...
             labeller = labeller(Model.Accent.matchesGroup = Model.Accent.matchesGroup.labs))

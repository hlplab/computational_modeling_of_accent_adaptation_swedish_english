...
correct_for_segmental_context = function(cue.name, 
  ...
  # If unnesting doesn't work consider using this
  # data.predict[, cue.name] - predict(l, newdata = data.predict) %>%
  #   as_tibble() %>%
  #   rename_at(cue.name, .funs = function(x) paste0("r", x))
}
```
...
warning("Check whether this way of unnesting does what we want!")

d.stims %>%
#  unnest(cols = c(data, rvowel, rclosure, rburst), .names_repair = "unique") %>%

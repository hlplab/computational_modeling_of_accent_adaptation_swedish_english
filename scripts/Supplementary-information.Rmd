...
We first evaluated the five folded models on the entire test set, following the same procedure outlined in Section \@ref(sec:IO-test). DESCRIBE RESULTS HERE BEFORE WE MOVE ON.
...
# Each training iteration uses 4/5 of each talker's training data; data from the non-native talkers are never down-sampled. exclude() drops 1/5 of the native talkers' training data but keeps 5/5 of non-native talkers' data.

exclude <- function(.data, excluded_set) {
  .data %>% mutate(Used_for = case_when(Talker == "M5" ~ "training",
                                        Talker == "F1" ~ "training",
                                        Training.Set == excluded_set ~ "exclude",
                                        TRUE ~ "training"))
}
...
models <- map(
  seq(1,5), 
  ~ make_IO(
    exclude(d.IO.training.CV, .x), 
    selected_cues) %>%
    mutate(Training.Iteration = paste0("iteration_", .x)))
```

```{r cross-validation-test-IO, include= FALSE}
# divide English test data into 4 test bins; Swedish test data into 2 bins. Ensure minimal pair members are in the same bin.

d.nonnative.test <- d.stims %>%
      filter(Block.Type == "test", Accent == "non-native") 

d.nonnative.test.bins <- d.nonnative.test %>% distinct(Item.MinimalPair) %>%
  group_by(NativeLanguage) %>% 
...
# test each of 5 models on each test bin 
run_tests <- function(model, test_data, test_bin_count) { 
  runs = map(seq(1, test_bin_count), ~ IO_categorization(filter(test_data, Test.Bin == .x), model) %>%  
              mutate(RunCount = paste0(Training.Iteration, "_bin", .x)) %>% droplevels()) %>% bind_rows()
}
...
runs_full_set <- map(seq(1, length(models)), ~ IO_categorization(select(d.nonnative.test,-c(Test.Bin)), models[[.x]]) %>% join_to_human_ratings(d.test)) %>% bind_rows()
...
# NOTE: I think this isn't working because of the joins. Each test run of the training iteration requires the human ratings for the exact same tokens. For e.g. Iteration1 and testbin1 has different tokens to Iteration1 and testbin2. I think the human ratings test dataframe needs a column identifying the tokens that have been assigned to the test bins so that each test run can be joined correctly.

---
title: "Supplementary Information for PAPER TITLE_testing with raw measures: Step 1 & 2 (n_syllable only; English and Swedish both use new correction method step 2; apply NN data to N data) ; talkers with high release; correct by speaker"
...
1) Final clean-up:
1a) Check whether warnings are addressed, and then (after) set to silent.
1b) Run final compilation with echo = F.

2) Check that all remaining Maryan,Xin,Florian comments have been addressed. In particular, do we want to add a section on the alternative Swedish recordings?
...
5) Remove spurious regression line plot. THIS IS DUE TO WEIRD DEFAULT IN PLOT_MODEL, WHICH FOR A SINGLE PREDICTOR ALSO PLOTS THE REGRESSION LINE.
...
Power analyses presented in Section \@ref(sec:bootstrap) strongly suggest that a) the English and Swedish experiment indeed differed in the statistical power they provided, and b) that the difference in statistical power is *highly unlikely* to explain the difference in results between the two experiments.
...
It is therefore possible that the decision to use stimuli from the assisted recording condition caused the null effect of /d/-exposure. This would not, however, make the analyses we present here any less relevant for future work. *Any* experiment on speech perception is based on a finite set of speech stimuli. These stimuli can be more or less representative of 'typical' speech from that talker or speech community. In no case, however, is any single experiment going to reflect the range of speech styles and talker types that are present in the speech community. While this raises interesting questions about the generalizability of research on speech perception (for discussion, see REF-Xie et al., 2020), these questions are orthogonal to the more modest goal we pursue here: to predict, for a given set of exposure and test stimuli, what consequences exposure is likely to have on listeners' perception and categorization responses during test. This is the question the ideal observer analyses in Sections \@ref(sec:IO-methods) and \@ref(sec:IO-methods) address.
...
<!-- Xin: should we perhaps code vowel lenght, coda lateral and rhotic for the English data or are there good reasons to believe that these variables don't matter for the English data? Or perhaps we don't have enough of those contexts? (there seemed to be quite a few, but they might not be balanced across data parts in a way that let's us estimate their effect?)  ANSWER: these variables are coded and loaded into d.stims. We can model them anytime-->
...
d.stims = 
  ...
  full_join(
    read.csv("../data/English/English_production_Li et al 2020.csv") %>%
      filter(!(Speaker %in% c("E12", "E3", "E7", "E9"))) %>%
      select(-Word) %>%
      rename(
        Talker = Speaker,
        has_stress = phon_ctxt_stress,
...
*In the current version, no correction steps were included. Instead, we correct only for speaking rate by taking the proportional measures of cues (cue duration/word duration). In this way, we examine how the predictions align with the previous results (as performed by Xin's early analyses).*

```{r create proportional measures for production data, include=FALSE, eval = FALSE}
d.stims.original <- d.stims

d.stims %<>%
  gather(Measure, duration, vowel:burst) %>%
  mutate(duration.prop = duration/syllable_duration) %>%
  select(-duration) %>%
  spread(Measure, duration.prop)


```

The realization of phonetic cues is known to be strongly influenced by phonological context (@ladefoged1957information; @lindblom1967role), and human listeners are known to effectively discount these effects---a process often referred to as *normalization* (e.g., @mann1980influence; @mann1981influence; @mcmurray2011information). Additionally, some of the cues to syllable-final /d/ voicing are simultaneously used to signal other contrasts. This is most obviously the case for vowel duration in Swedish. Swedish has a phonological contrast between long and short vowels (e.g., *vit*---'white, comm. gend.' vs. *vitt*---'white, neuter'). Vowel duration is an important cue to this contrast. 
...
correct_for_segmental_context = function(cue.name, 
  ...
  l = 
    if(language == "English") 
      # lmer(
      #   y ~ 1 + Accent /  (onset_has_nasal + coda_has_nasal + onset_has_rhotic + coda_has_lateral) + (1 + onset_has_nasal + coda_has_nasal + onset_has_rhotic + coda_has_lateral| Talker), 
      #   data = data.fit)
      lm(
        y ~ 1 + (coda_has_nlr + onset_has_nlr), 
        data = data.fit) else
      lm(
...
# xxx <- d.stims %>% filter(Talker == "M5")
# 
# xxx %<>%
#   group_by(NativeLanguage, Talker) %>%
#   nest() %>%
#   mutate(
#     rvowel = map2(
#       data, NativeLanguage,
...
Supra-segmental factors like---lexical stress and speech rate---are also known to affect the phonetic realization of sounds. For example, segments tend to have longer durations in stressed, compared to unstressed, syllables (@lindblom1963spectrographic; @van1987effects). Processes like compensatory lengthening/shortening (@peterson1960duration; @lindblom1973some; @fowler1981relationship) can also cause segments to be longer in syllables with fewer segments (REF), and words with fewer syllables (Warner et al., 2004).<!--Florian: are you referring to anticipatory shortening? MARYANN: I am referring to compensartory shortening/lengthening (the same segment might be shorter if the syllable has more segements.) --> Additionally, local changes in speech rate are likely to affect durational measures like the ones used here (e.g. @crystal1982segmental; @gahl2012reduce). Given that non-native speakers tend to speak more slowly, it is necessary to control for cue duration variation caused by differences in speaking rate. We thus removed effects of speech rate, lexical stress, and syllable/word complexity on vowel, closure, and burst duration.
...
my.resids.length = function(var.name, data.fit, data.pred) {
  ...
  l = lm(y ~ 1 + number_syllables + has_stress + rsyllable_duration, data = data.fit)
  print(summary(l))

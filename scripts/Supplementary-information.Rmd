...
For both the English and the Swedish data, we compare the predictions of a native model and a non-native model. The native models are meant to predict the behavior of listeners without exposure to the foreign-accented /d/, i.e., listeners in the control condition. The non-native models are meant to predict the behavior of listeners with exposure to the foreign-accented /d/, i.e. listeners in the the /d/-exposure condition. 

To this end, we train and test a total of four ideal observers: the native and non-native English models, and the native and non-native Swedish models. Both the native and non-native ideal observer are *tested* on the same foreign-accented minimal pair stimuli from the test phase of the experiments. The two models are, however, *trained* on different inputs. In particular, the two models received *different /d/ tokens during training*---paralleling the difference between the /d/- and control exposure in the experiments. To derive predictions from the ideal observers it is also necessary to give them a /t/ category. Since /t/ was never (or hardly ever) observed in either exposure condition, both the native and non-native model were training on *native* /t/. In other words, our non-native ideal observer makes the simplifying assumption that the /d/-exposure condition did not convey any information about the /t/ category, so that listeners in the /d/-exposure condition change their beliefs about the /d/ category, but do not change their beliefs about the /t/ category. This simplifying assumption is almost certainly wrong since, for example, talkers' means for different categories tend to correlate (Chodroff et al., 2017, 2018). <!-- Xin: if it's also the case in the Li et al data, we could say so here --> However, it is unlikely to bias our results: here we ask whether the predictions of native and non-native ideal observers differ for the English and the Swedish data. Critically, we apply the same simplifying assumption to both the English and the Swedish ideal observers. Any difference we find between the predictions for English and Swedish can therefore not be due to this simplifying assumption.
...
In the following sections, we walk through the steps that are summarized in Figure \@ref(fig:ideal-observer-and-data). As already visible in that figure, we draw on multiple databases. The speech materials in these databases differed along dimensions that were not of theoretical interest to us, but that could affect the performance of the ideal observers. For example, if we train the ideal observers on the vowel, closure, and burst duration of multi-syllabic words and then test them on the vowel, closure, and burst duration of our mono-syllabic test items, this might make the comparison of the native and non-native models' performance on the test items uninformative. We thus prepared the training and test data by correcting for effects of phonological context, as we describe next Following that, we describe the training and test procedure for the ideal observers.
...
#### English
For the native English IO, the /d/ and /t/ distributions were estimated from a database of 10 male native talkers of American English (Li, Jaeger, & Xie, 2020). To the best of our knowledge, this is the largest database of syllable-final stop voicing in native-accented American English for which vowel, closure, and burst duration are annotated. The 10 talkers had the same sex and spanned a similar age range (18-35) as the Mandarin-accented talker in Xie et al. (2017). The recording procedure was the same as in Xie et al. (2017). We used the data of the 10 male native speakers in Li et al. (2020) to estimate the mean and covariance for a *prototypical* native talker's /d/ and /t/. Specifically, we took the prototypical category mean to be the mean of 10 talkers' category means, and the prototypical category covariance to be the mean o fthe 10 talkers' covariance matrices. This provides an---admittedly very coarse-grained---point estimate of a native listeners' beliefs about a prototypical native talker's /d/ and /t/ categories.

For the non-native English IO, the /t/ distribution was the same as the /t/ distribution for the native English IO. This reflects our simplifying assumption that the /d/-exposure condition did not contain any information about the non-native /t/. The /d/ distribution for the non-native English IO was estimated from the 30 critical words with syllable-final /d/ used during the foreign-accented exposure in Xie et al. (2017). This foreign-accented speech is part of the non-native portion of the Li et al. (2020) database (hence the dotted line between the native English IO's /d/ category and the Li et al., 2020 database in Figure \@ref(fig:ideal-observer-and-data)). <!-- Xin: is this prev. sentence actually true? was that speech part of Li et al? if not, Maryann should change the figure above. and we should remove this sentence. --> Note that we intentionally did *not* use data from further non-native talkers in Li et al.'s database: we intend the non-native IO to approximate the beliefs held by a native listener who has perfectly adapted to the foreign-accented /d/ distribution during exposure. 

Of note, the words that were used to estimate native English /d/ and /t/ distributions were not the same as those used for estimating non-native /d/.<!-- Xin: wasn't the non-native /d/ estimated from multi-syllabic exposure words? --> However, both groups of words were monosyllabic with word-final /d-t/. Additionally, we report below how we corrected for effects of phonological context.

#### Swedish
The procedure for the Swedish data was identical with one exception. We are aware of no Swedish database comparable to Li et al. (2020). We thus had access to only the recordings of one gender and age-matched native-accented speaker, judged to have standard Swedish pronunciation. This was the speaker recorded to provide the native pronunciations for the foreign-accented talker (see \@ref(sec:recording)). Instead of estimating a prototypical native talker of Swedish, we thus use the speech from the only native talker we had recordings of to estimate the /d/ and /t/ categories for the native Swedish IO. Thus, unlike for the English IOs, the words that were recorded to estimate the native and non-native Swedish categories were the same. <!-- Xin, Maryann: I'm confused. there were no /t/-exposure words. Didn't we use /t/-test words for both english and swedish? but then the descriptions given here are wrong. Also, unless those TEST words were part of the Li et al database, the figure is also wrong -->
...
<!-- Xin how was "long vowel" coded for the english data? or was it excluded? REPLY: we didn't encode long vs short vowel for English as it was not a phonological contrast.-----Were there differences between english and swedish? how was this model then USED? also MARYANN POINTED OUT THAT WE MIGHT NOT NEED THE R-CONTROL ANYMORE, UNLESS WE NEED IT FOR THE ENGLISH DATA? REPLY: Okay, removed the R-control below-->

 + **Step 1:** We used the foreign-accented /d/- and /t/-final words from the test phase to estimate the effects of phonological contexts. The specific contexts considerd differ for the Swedish and English datasets, based on previous findings on context effects in the two languages as well as the type of context variation in each dataset. For the English data, we considered the effect of preceding nasals and following nasals. This was achieved by fitting a separate linear regression to each of the three cues `lm(cue ~ 1 + has_long_vowel + preceded_by_nasal + followed_by_nasal)`. For the Swedish data, we considered two phonological contexts (phonologically long vs. short vowel; following nasal, e.g., *XXX*---'XXX' vs. *XXX*---'XXX'). This was achieved by fitting a separate linear regression to each of the three cues: `lm(cue ~ 1 + has_long_vowel + preceded_by_rhotic + preceded_by_nasal)`. Across the two languages, there were a total of six linear regressions. 
 
<!-- Xin, Maryann: what about lexical stress? I guess words like lemonade do have secondary stress on the d-syllable. was d or t ever at the end of an unstressed syllable? Reply here: in the English dataset, most exposure words had a lexical stress on the syllable of /d/, but there were a few instances where the words ended in an unstressed syllable, e.g. "haphazard". We did not model lexical stress. All /t/ words were stressed as they were all monosyllabic.--> 


 + **Step 2:** The /d/- and /t/-final words from the test phase words were always monosyllabic, so that the /d/-final and /t/-final syllable always received primary lexical stress. This contrasted with both the foreign-accented exposure words and the native-accented words (Swedish data only) used to estimate the 'typical' language experience of native listeners, which were multisyllabic. Inspection of the cue distributions confirmed that, in particular, vowel durations were strongly affected by word length: vowel durations were substantially shorter in longer words). We thus used the foreign-accented exposure and test words with syllable-final /d/ to estimate the effect of word length on the duration of each cue (all native-accented words were multi-syllabic). This again resulted in six linear regressions: `lm(cue ~ 1 + number_of_syllables)`
...
<!-- ATTENTION: Step 3 was previously applied only to the Swedish data. -->
...
<!-- Maryann: The number of stimuli from the native speaker (23 exposure /d/ items, 25 test /t/ items, after excluding the 4 -r words) was fewer than that from the non-native speaker. Was it because some bursts were not released? Can you double-check? -->


```{r load data for correcting for phonological context}


# load stimuli
d.stims.swedish <- read.csv("../data/Swedish/test_stimuli_analysis/d.stims.swedish.csv") %>%
  filter(mimic == "irrelevant" | mimic == "yes") %>%
  filter(! Item.Word %in% c("voart", "voard", "hoart", "hoard")) %>%
  select(-mimic) %>%
  rename(Talker = language) %>%
  droplevels()

# xtabs(~Talker + Block.Type + Sound, d.stims.swedish)

d.stims.english <- read.csv("../data/English/d.stims.english.csv")
  

d.stims.database <- read.csv("../data/English/d.stims.database.csv")
 
```

```{r functions-residualize-data}

# Correct for phonological context for English data

my.resids.phon.english = function(var.name, data.fit, data.pred) {
  # data.fit should be test /d/ and /t/ words only
  # data.pre should be all words: exposure /d/ + test /d/ + test /t/
  if (is.null(data.pred)) data.pred = data.fit
  
  data.fit$y = data.fit[,var.name]

  # phon_ctxt_stress and phon_post_fricative are not excluded because it does not vary within test
  l = lm(y ~ 1 + (phon_pre_nasal + phon_post_nasal), data = data.fit)
  print(summary(l))
  return(data.pred[,var.name] - predict(l, newdata = data.pred))
}

# Correct for phonological context for Swedish data

my.resids.phon.swedish = function(var.name, data.fit, data.pred) {
  # data.fit should be test /d/ and /t/ words only
  # data.pred should be all words: exposure /d/ + test /d/ + test /t/
  if (is.null(data.pred)) data.pred = data.fit
  
  data.fit$y = data.fit[,var.name]

  # phon_ctxt_l is excluded because it does not vary within test
  l = lm(y ~ 1 + (phon_ctxt_vl + phon_ctxt_n), data = data.fit)
  print(summary(l))
...
my.resids.length = function(var.name, data.fit, data.pred) {
  # data.fit should be exposure /d/ and test /d/ words only
  # data.pre should be all words: exposure /d/ + test /d/ + test /t/
  if (is.null(data.pred)) data.pred = data.fit
  
  data.fit$y = data.fit[,var.name]
  
  l = lm(y ~ 1 + syllable, data = data.fit)
  print(summary(l))
...
```{r residualize-English-data-Xieetal}

# step 1: correct for phonological contexts effect
d.stims.english.step1 <- 
  d.stims.english %>%
  mutate(rvowel = my.resids.phon.english("vowel", data.fit = d.stims.english %>%
                          filter(Block.Type == "test"), data.pred = .),
         rclosure = my.resids.phon.english("closure", data.fit = d.stims.english %>%
                                   filter(Block.Type == "test"), data.pred = .),
         rburst = my.resids.phon.english("burst", data.fit = d.stims.english %>%
                                   filter(Block.Type == "test"), data.pred = .)
  )

# step 2: correct for syllable length effect, on the corrected measures from step 1

d.stims.english.step2 <- d.stims.english.step1 %>%
  mutate(rvowel = my.resids.length("rvowel", data.fit = d.stims.english.step1  %>%
                                   filter(Sound == "/d/"), data.pred = .),
         rclosure = my.resids.length("rclosure", data.fit = d.stims.english.step1  %>%
                                     filter(Sound == "/d/"), data.pred = .),
         rburst = my.resids.length("rburst", data.fit = d.stims.english.step1  %>%
                                     filter(Sound == "/d/"), data.pred = .)
  )

d.stims.english.corrected <- d.stims.english.step2

```
```{r residualize-Swedish-data}

d.stims.swedish.corrected = data.frame()


# STEP 1: correct for phonological contexts effect
levels.Talker = c("native", "non-native")

for (i in 1:length(levels.Talker)){
  
data.step1 = d.stims.swedish %>%
    filter(Talker == levels.Talker[i]) %>%
    filter(Block.Type == "test")
  
print(
  paste0("Current talker is ", levels.Talker[i])
)

d.stims.swedish.step1 <- d.stims.swedish %>%
  filter(Talker == levels.Talker[i]) %>%
  mutate(rvowel = my.resids.phon.swedish("vowel", data.fit = data.step1, data.pred = .),
         rclosure = my.resids.phon.swedish("closure", data.fit = data.step1, data.pred = .),
         rburst = my.resids.phon.swedish("burst", data.fit = data.step1, data.pred = .)
  )

# STEP 2: correct for syllable length effect, on the corrected measures from step 1
d.stims.swedish.step2 <- d.stims.swedish.step1 %>%
  filter(Talker == levels.Talker[i]) %>%
  mutate(rvowel = my.resids.length("rvowel", data.fit = d.stims.swedish.step1 %>%
                                     filter(Sound == "/d/"), data.pred = .),
         rclosure = my.resids.length("rclosure", data.fit = d.stims.swedish.step1 %>%
                                       filter(Sound == "/d/"), data.pred = .),
         rburst = my.resids.length("rburst", data.fit = d.stims.swedish.step1 %>%
                                     filter(Sound == "/d/"), data.pred = .)
  ) %>%
  mutate(CorrectionType = levels.Talker[i])

d.stims.swedish.corrected <- bind_rows(d.stims.swedish.corrected, d.stims.swedish.step2)
}

# STEP 3: align the /t/ tokens from the native speaker and the non-native speaker

t_mean_diff <- d.stims.swedish.corrected %>%
  mutate(CorrectionType = ifelse(Talker == "non-native", "nonnative", CorrectionType)) %>%
  filter(Sound == "/t/") %>%
  group_by(CorrectionType) %>%
  summarise_at(c("rvowel", "rclosure", "rburst"), mean, na.rm = TRUE) %>%
  ungroup() %>%
  gather(cue, value, rvowel:rburst) %>%
  spread(CorrectionType, value) %>%
  mutate(diff = native-nonnative) %>%
  select(cue, diff) %>%
  mutate(type = "native_over_nonnative")
  
d.stims.swedish.corrected.aligned <- d.stims.swedish.corrected %>%
  mutate(rvowel = case_when(
    CorrectionType == "non-native" ~ rvowel + t_mean_diff[t_mean_diff$cue %in% "rvowel", ]$diff,
    CorrectionType == "native" ~ rvowel
  ),
  rclosure = case_when(
    CorrectionType == "non-native" ~ rclosure + t_mean_diff[t_mean_diff$cue %in% "rclosure",]$diff,
    CorrectionType == "native" ~ rclosure
  ),
  rburst = case_when(
    CorrectionType == "non-native" ~ rburst + t_mean_diff[t_mean_diff$cue %in% "rburst",]$diff,
    CorrectionType == "native" ~ rburst
  )
)

# saveRDS(d.stims.swedish.corrected.aligned, "../data/Swedish/d.stims.swedish.corrected.rds")

```



```{r prepare-native-English-data-stims, cache=TRUE, dependson=c('classification-helpers')}

# prepare functions to run for all speakers
my.resids.phon = function(var.name, data.fit, data.pred) {
  # data.fit should be test /d/ and /t/ words only
  # data.pre should be all words: exposure /d/ + test /d/ + test /t/
  if (is.null(data.pred)) data.pred = data.fit
...
# correct for phonological contexts effect -- do this by speaker so that the estimates are comparable to those of the single non-native talker and those of the Swedish data

d.stims.database.step1 <- d.stims.database %>%
  nest(-c(Speaker)) %>%
  group_by(Speaker) %>%
  mutate(rvowel = purrr::map(data, ~my.resids.phon("vowel", data.fit = ., data.pred = .)),   # create rvowel as a list element
         rclosure = purrr::map(data, ~my.resids.phon("closure", data.fit = ., data.pred = .)),
         rburst = purrr::map(data, ~my.resids.phon("burst", data.fit = ., data.pred = .))
  ) %>%
  unnest() %>%
  mutate(current_speaker = Speaker,
    rvowel = vowel1,
         rclosure = closure1,
         rburst = burst1,
         vowel1 = NULL,
...
d.stims.database.step2 <- d.stims.database.step1 %>%  # Caveat: because the stimuli from the English speaker are all monosyllabic, here the effect of syllable length was estimated using the Mandarin talker's data and applied to the English talker
  nest(-c(Speaker)) %>%
  group_by(Speaker) %>%
  mutate(rvowel = purrr::map(data, ~my.resids.length("rvowel", data.fit = d.stims.english.step1 %>% 
                                   filter(Sound == "/d/"), data.pred = .)),
         rclosure = purrr::map(data, ~my.resids.length("rclosure", data.fit = d.stims.english.step1 %>%
                                     filter(Sound == "/d/"), data.pred = .)),
         rburst = purrr::map(data, ~my.resids.length("rburst", data.fit = d.stims.english.step1 %>%
                                     filter(Sound == "/d/"), data.pred = .))
  ) %>%
  unnest() %>%
  mutate(rvowel = rvowel1,
         rclosure = rclosure1,
         rburst = rburst1,
         rvowel1 = NULL,
         rclosure1 = NULL,
...
# combine the data from the native English database and the non-native talker in Xie et al

d.stims.english.corrected <- plyr::rbind.fill(d.stims.database.step2 %>% 
                            mutate(Accent = "English"), 
                          d.stims.english.step2 %>% mutate(Accent = "Mandarin",
                                                           Speaker = "M5")) %>%
  mutate(Talker = Speaker, Speaker = NULL) %>%
  mutate(Block.Type = ifelse(Accent == "English", "exposure", as.character(Block.Type))) %>%
  droplevels(.)

#saveRDS(d.stims.english.corrected, "../data/English/d.stims.english.corrected.rds")
```

# Testing ideal observers

## Correcting test tokens for phonetic context

Additionally, all *test* tokens must be annotated for the same cues. XXXXXX
...
d.stims.swedish <- d.stims.swedish.corrected.aligned

d.stims.english <- d.stims.english.corrected
```
...
# TS model: Exposure /d/ comes from the non-native talker's exposure /d/, exposure /t/ comes from the native talker's test /t/;      
      ...
      temp1.TS <- plyr::rbind.fill(temp1.TS.d,temp1.TS.t)
...
# train ideal observers for English data

```{r train-ideal-observers-English-stims}

English_talkers <- levels(as.factor(d.english$Speaker))
...
df.TS1 <- res.TS %>%
  ...
  dplyr::select(combo, part,Sound, label, data, model, C1, C2, C3, C1_C1, C2_C2, C3_C3) # select mean and sd from TS model (mean_nn, sd_nn)

df.native1 <- res.native %>%
  mutate(label = paste(part,combo,Sound,sep = "_")) %>%
  mutate(cor12 = C1_C2/sqrt(C1_C1*C2_C2),     # mutate correlation from native model (cor_n)
         cor13 = C1_C3/sqrt(C1_C1*C3_C3),
         cor23 = C2_C3/sqrt(C2_C2*C3_C3)) %>%
  dplyr::select(label, cor12, cor13, cor23)  ##here take the correlation from the native model


df_new1 <- left_join(df.TS1, df.native1, by = "label")

res.TS1 <- df_new1 %>%
 group_by(combo, part,sound) %>%
  mutate(model_old = model,
         model = list(list(mu = structure(c(C1, C2, C3), names = selected_cues),
                           Sigma = matrix(c(C1_C1,C1_C2,C1_C3, C2_C1,C2_C2, C2_C3, C3_C1, C3_C2, C3_C3), ncol=3, nrow=3, dimnames = list(selected_cues,selected_cues)))))

...
correct_for_segmental_context = function(cue.name, 
  ...
  message("NB: correct_for_segmental_context() assumes that the input data contains only one native language at a time.")
...
d.stims %<>%
  ...
  nest() %>%
   mutate(
    offset_segmenta_context_vowel = map2(
      data, NativeLanguage,
      ~ correct_for_segmental_context("vowel", 
                             data.fit = .x,
                             language = .y)),
    offset_segmenta_context_closure = map2(
      data, NativeLanguage, 
      ~ correct_for_segmental_context("closure", 
                             data.fit = .x,
                             language = .y)),
    offset_segmenta_context_burst = map2(
      data, NativeLanguage, 
...
d.stims %<>%
  unnest(c(data, offset_segmenta_context_vowel, offset_segmenta_context_closure, offset_segmenta_context_burst)) 
...
```{r step 2 functions to fit for word length effects}
correct_for_word_length = function(cue.name,
                               data.fit,
                               language) {
  data.fit %<>%
    filter(Accent == "non-native" & Sound == "/d/")
  
  data.fit$y = as_vector(data.fit[, cue.name])
  l = lm(y ~ 1 + (number_syllables + has_stress), data = data.fit, 
         offset = case_when(
      cue.name == "vowel"  ~ data.fit$offset_vowel,
      cue.name == "closure"  ~ data.fit$offset_closure,
      cue.name == "burst"  ~ data.fit$offset_burst,
      T ~ NA_real_))
  
  print(plot_model(l,
                   show.p = T, show.values = T,
                   title = paste("Effects of supra-segmental context on", gsub("^r", "", cue.name), "duration for the non-native", language, "/d/-final words"),
                   axis.title = c(paste("Change in", cue.name, "duration (in msec)"), "")))
...
```{r fitting for word length effects}
d.stims %<>%
  group_by(NativeLanguage) %>%
  nest() %>%
   mutate(
    offset_vowel2 = map2(
      data, NativeLanguage,
      ~ correct_for_word_length("vowel", 
                             data.fit = .x,
                             language = .y)),
    
    offset_closure2 = map2(
      data, NativeLanguage,
      ~ correct_for_word_length("closure", 
                             data.fit = .x,
                             language = .y)),
    offset_burst2 = map2(
      data, NativeLanguage,
      ~ correct_for_word_length("burst", 
                             data.fit = .x,
                             language = .y)))

d.stims %<>% unnest(c(data, NativeLanguage, offset_vowel2, offset_closure2, offset_burst2))
```

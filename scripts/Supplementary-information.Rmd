  ...
  data %>%
    mutate(
      ...
      log_likelihood = log(
        ...
        (1 - Rating.Response.d.normalized.d) * (1 - prediction.d))) %>%
...
d.test.IO.byItem %>%
  ...
  nest() %>%
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), .f = function(x, y, z) get_log_likelihood(x, y, z)))) %>%

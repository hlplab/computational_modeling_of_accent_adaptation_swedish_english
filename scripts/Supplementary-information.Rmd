...
x = d.test.IO %>%
    select(
      NativeLanguage, Item.MinimalPair, Sound, rvowel, rclosure, rburst) %>%
    distinct() %>%
    filter(NativeLanguage == "English", Item.MinimalPair == "E1", Sound == "/d/") %>%
    make_vector_column(
      cols = c("rvowel", "rclosure", "rburst"),
      vector_col = "x")  %>%
    pull(x) %>%
...
belief = d.IBBU %>% filter(NativeLanguage == "English", prior_kappa == 1, prior_nu == 8, observation.n == 30)

get_categorization_from_NIW_belief(x, belief, decision_rule = "proportional")
...
# # Join IBBU models and full test data 
# # (to get posterior probability of test ratings re-interpreted as posterior probabilities)
# d.test.IBBU <-
#   d.IBBU %>%
#   # select final (updated) IOs
#   filter(observation.n == 30) %>%
...
#   mutate(
#     Posterior = pmap(
#       list(get_categorization_from_NIW_belief())
```

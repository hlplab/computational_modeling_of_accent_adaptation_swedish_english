...
x <- d.test.IO.wTraining_folds.wTest_folds.byItem %>% 
   crossing(
    bias.d = .5,
  ...
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), .f = function(x, y, z) get_per_token_log_likelihood(x, y, z))),
         Training_fold = factor(Training_fold),
         Test_fold = factor(Test_fold)) 

x %>% 
  ggplot(mapping = aes(x = lapse_rate, y = per_token_loglik, color = Group)) +
  # stat_summary(fun.data = mean_cl_boot, geom = "pointrange", 
  #              alpha = .6, size = .8,
  #              position = position_dodge(.3)) +
  # stat_summary(mapping = aes(linetype = Model.Group.matchesParticipant.Group),
  #              fun = mean, geom = "line", 
  #              alpha = .6, size = .8,
  #              position = position_dodge(.3)) +

...
In an additional analysis, we randomly split the test data into folds of 15 minimal pairs each, resulting in 2 folds for the Swedish test data (which had 30 test tokens) and 4 folds for the English test data (which had 60 test tokens). By comparing across test bins within each fold, we can estimate how robust our results depend on the specific test bins. Each fold was created by sampling minimal pairs---rather than individual words---to hold the phonological context constant between the /d/ and /t/ test items within each fold. To compare like with like, we then compared the model predictions for each test fold against the human ratings on the same test items. DESCRIBE THESE RESULTS HERE.
...
```{r IO-CV-plots-posterior-predictions-english, fig.height=4.5, out.width='75%'}

...
d.test.IO.t <- d.test.IO %>% filter(Model.Group.matchesParticipant.Group == "yes") %>%
  group_by(Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>% 
  summarise(mean.model.rating.d = mean(Model.Rating.d.zscored)) %>% 
  group_by(Language, Sound) %>% 
  rstatix::pairwise_t_test(mean.model.rating.d ~ Model.Group, paired = TRUE) 
...
p.results.percept.byItem + stat_pvalue_manual(d.test.IO.t %>% add_x_position(x = "Sound", dodge = 0.93), y.position = 1.25, label = "p.adj.signif", tip.length = 0.01)
...
# To provide annotations for the training-test plot grid, get the mean model rating of d, for each word within each training-test fold:
my.filter <- d.test.IO.wTraining_folds.wTest_folds %>%
  filter(Model.Group.matchesParticipant.Group == "yes") %>%
  group_by(Language, Model.Group, Sound, Training_fold, Test_fold, Item.WordID) %>%
  summarise(mean.model.rating.d = mean(Model.Rating.d.zscored))
 
# set up t-test function to compare the predicted ratings by each model.group (i.e., diff between predicted ratings for /d/ by control and predicted ratings for /d/ by d-exposure, etc…)
 signif_test <- function(.data) {
    model.t <- t.test(mean.model.rating.d ~ Model.Group, paired = TRUE, .data)
    pvalue <- 3
    model.t[[pvalue]]
}
 
  
# Get the p-values of the t-test:
 my.pvalues <- my.filter %>% group_by(Language, Sound, Training_fold, Test_fold) %>%
  nest() %>%
  mutate(t.test.p = map_dbl(data, ~ signif_test(.x))) %>% 
   unnest(data) %>% 
   group_by(Language, Model.Group, Sound, Training_fold, Test_fold, t.test.p) %>% 
   nest() %>% select(-data) %>% 
   group_by(Training_fold, Test_fold) %>% 
   pivot_wider(names_from = Model.Group, values_from = Sound) %>% 
   mutate(signif = ifelse(t.test.p <= .05, "*", "n.s")) 
...
my.pvalues %<>%  rename(group1 = `/d/-exposure`,
          group2 = `control`) %>% 
  group_by(Language, Training_fold, Test_fold) %>% summarise(n())
...
plot_grid(
  ...
  p.IO_prediction %+% 
    (d.test.IO.wTraining_folds.wTest_folds %>%
       ...
       summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1))) +
    facet_grid(Training_fold ~ Test_fold, scales="free_y") +
    stat_pvalue_manual(my.pvalues %>% filter(Language == "Flemish-accented Swedish"),
                       y.position = 1.125, 
                       label = "signif") +
...
## Do ideal observers that model an exposure condition predict item-lvel human /d/-goodness ratings *better* than ideal observers intended for a different exposure condition? {#sec:matching-vs-mismatching-IO}

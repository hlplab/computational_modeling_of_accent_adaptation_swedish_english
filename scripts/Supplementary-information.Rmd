...
The ideal observers do, however, predict the qualitative patterns in the human goodness ratings:
 ...
 + For the English data, the ideal observers predict significantly higher ratings from the /d/-exposure model over the control model for both /d/ and /t/. 
...
Our initial analysis did not employ cross-validation or similar methods. This decision was made since the data the ideal observers are tested on do not overlap with the data the ideal observers are tested on. During the review process, both reviewers raised questions about the amount of data that would be required for the type of computational analysis conducted here. Here we address this questions by subsetting the training and test data to assess the variability of the results across different folds of the training and test data. This also allows us to assess the extent to which the results in the previous section are due to overfitting to the training data.

Our ideal observers are training on two types of data: data that estimate native listeners' expectations about /d/ and /t/ based on previous lifelong experienced with their native language, and data that estimate the effects of the exposure phase (for the /d/ category in the /d/-exposure condition). For the former data, we randomly created five non-overlapping folds of 20% of the data. The folds were created by sampling (without replacement) the native data described in Section \@ref(sec:IO-training) *within* each unique combination of talker and category. Each model was then trained on four of the five folds (80% of the data, i.e., 24 tokens per category and talker), resulting in five models, each omitting another 20% of the native training data. Since the exposure data---by hypothesis---form the basis of adaptation, we do *not* resample or subset these data but rather hold them constant across all folds of the training data. By comparing across these five models, we can thus assess the extent to which the specific native data used to train the ideal observers affects our results (recall that these data determine the /t/ category in both the control and the /d/-exposure model, as well as the /d/ category in the control model). At the same time, this analysis sheds light on whether 24 tokens per talker and category are even remotely sufficiently to achieve robust estimates of category means and covariances. This is a question, in particular, for the Swedish models, for which we had phonetically annotated data for only *one* native talker (as opposed to 10 talkers for the English data). <!--it's 6 talkers isn't it?-->

```{r cross-validation-train-IO, include = FALSE}
k <- 5 # set number of training iterations
...
# check that above random flagging worked as desired
# d.IO.training.CV %>% group_by(Used_for.Model.NativeLanguage, Used_for.Model.Group,
#                               Talker, Used_for.Model.Sound, Training.Set) %>% summarise(n())

# Each training iteration uses 4/5 of each talker's training data; data from the non-native talkers are never down-sampled. 
# exclude() drops 1/5 of the native talkers' training data but keeps 5/5 of non-native talkers' data.
...
# train models, excluding 1/5 each iteration.
d.IO.5folds <- map(
  seq(1,5), 
  ~ make_IO(
...
We first evaluated the five folded models on the entire test set, following the same procedure outlined in Section \@ref(sec:IO-test). Figure \@ref(fig:IO-ratings-results-byItem-5folds) plots the predictions of the ideal observers. 

(ref:IO-ratings-results-byItem-5folds) **5-fold cross-validated ideal observer /d/-goodness ratings** during test (z-scored) predicted by the control and /d/-exposure ideal observers. Error bars show 95\% bootstrapped confidence intervals over by-item predictions (since there aren't multiple subjects). Each row shows one training fold excluding a different 20% of the native training data. For additional details, see Figure \@ref(fig:IO-ratings-results-byItem).

DESCRIBE RESULTS HERE BEFORE WE MOVE ON.

```{r cross-validation-ratings-results-byItem, fig.cap="(ref:ratings-results-byItem-5folds)", fig.height=22.5, out.width='75%'}
# Following same steps as above for non-CV testing
d.IO.test.5folds <- categorize_stims_by_IO(d.stims, d.IO.5folds)
d.test.IO.5folds <- join_IO_categorizations_with_human_test(d.IO.test.5folds, d.test)
save(d.test.IO.5folds, file = "../data/data_perception_test phase plus IO predictions_both languages-5 training folds.Rdata")
...
ggsave(filename = "../figures/Result-ratings-byItem-5folds.png",
       width = 7, height = 20)
```


```{r}
# Run test on all data or folds
fold_test_and_categorize <- function(model, .data_test, test_fold_count) { 
  runs = map(
    seq(1, test_fold_count), 
    ~ IO_categorization(
      filter(
        .data_test, 
        Test.Fold == .x),
      model) %>%
      mutate(RunCount = paste0("Training-test fold: "Training.Iteration, "-", .x)) %>% 
      droplevels()) %>% 
    bind_rows()
}

# join each training iteration with human test ratings
join_to_human_ratings <- function(model.test.run, d.test.humans) {
   d.test.humans %>% droplevels() %>% 
    left_join(model.test.run) %>% 
    mutate(
    Model.Group.matchesParticipant.Group = factor(case_when(
      NativeLanguage != Model.NativeLanguage ~ NA_character_,
      Group == Model.Group ~ "yes",
      Group != Model.Group ~ "no",
      T ~ NA_character_), levels = c("no", "yes"))) %>% IO_ratings()
}

# test models on all nonnative test tokens.
d. <- 
  map(
    seq(1, length(models)), 
    ~ IO_categorization(select(d.nonnative.test,-c(Test.Bin)), models[[.x]]) %>% 
      join_to_human_ratings(d.test)) %>% 
  bind_rows() %>%
  join_to_human_ratings(d.test)
```

```{r cross-validation-test-IO-allTest-plot, fig.height=6, out.width='100%'}
# plot model posteriors of intended category by language
p.english <-  
  d.test.IO.CV %>% filter(Model.NativeLanguage == "English") %>% 
  ggplot(aes(x=Sound, y=Model.Posterior.Intended)) + 
  geom_hline(yintercept = 1/2, color = "gray") +
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
               position = position_dodge(.93)) +
  stat_summary(aes(group = Model.Group), fun.data = mean_cl_boot, geom = "linerange", 
               size = .8, color = "black",
  ...
  scale_fill_manual(values = colors.Group) +
  scale_y_continuous("Accuracy") +
  xlab("Intended sound category") +
  facet_wrap(.~ RunCount, nrow = 5)
p.english
```

In an additional analysis, we randomly split the test data into folds of 30 tokens each, resulting in 2 folds for the Swedish test data (which had 60 test tokens) and 4 folds for the English test data (which had 120 test tokens). By comparing across test bins within each fold, we can estimate how robust our results depend on the specific test bins. Each fold was created by sampling minimal pairs---rather than individual words---to hold the phonological context constant between the /d/ and /t/ test items within each fold. To compare like with like, we then compared the model predictions for each test fold against the human ratings on the same test items. DESCRIBE THESE RESULTS HERE.



```{r cross-validation-test-IO-subTests, include= FALSE}
# divide English test data into 4 test folds; Swedish test data into 2 folds (each with 15 minimal pairs per fold)
d.nonnative.test <- d.stims %>%
      filter(Block.Type == "test", Accent == "non-native") 

d.nonnative.test.bins <- 
  d.nonnative.test %>% 
  distinct(Item.MinimalPair) %>%
  group_by(NativeLanguage) %>% 
  mutate(Test.Bin = case_when(
    NativeLanguage == "English" ~ ntile(runif(length(Item.MinimalPair)), 4),
    NativeLanguage == "Swedish" ~ ntile(runif(length(Item.MinimalPair)), 2))) 

# join the assigned min.pairs with the main cross-validation-test dataframe
d.nonnative.test <- left_join(d.nonnative.test, d.nonnative.test.bins)

# run_and_join <- function(model, test_data, test_bin_count) {
#   run_tests(model, test_data, test_bin_count) %>% 
#   join_to_human_ratings(d.test)
# }

#test each of 5 models on different test bins
test_bin_count = 4
results.CV = map(seq(1,length(models)), ~ run_tests(models[[.x]], d.nonnative.test, test_bin_count))

# take the model-test results list and put them into a dataframe
d.test.CV <- results.CV %>% bind_rows()
# join to human data
d.test.IO.CV <- join_to_human_ratings(d.test.CV, d.test)

# test_bin_count = 4
# results.CV.English = map(seq(1,length(models)), ~ run_tests(models[[.x]], d.nonnative.test %>% filter(NativeLanguage == "English"), test_bin_count))
# # do test for Swedish
# test_bin_count = 2
# results.CV.Swedish = map(seq(1,length(models)), ~ run_tests(models[[.x]], d.nonnative.test %>% filter(NativeLanguage == "Swedish"), test_bin_count))
# CV.data.English <- results.CV.English %>% bind_rows() %>% group_by(Training.Iteration, Test.Bin)
# CV.data.Swedish <- results.CV.Swedish %>% bind_rows() %>% group_by(Training.Iteration, Test.Bin)
# d.test.data.CV <- bind_rows(CV.data.English, CV.data.Swedish)
# d.test.IO.CV <- d.test.CV %>% left_join(d.test) %>% 
#   mutate(Model.Group.matchesParticipant.Group = factor(case_when(
#       NativeLanguage != Model.NativeLanguage ~ NA_character_,
#       Group == Model.Group ~ "yes",
#       Group != Model.Group ~ "no",
#       T ~ NA_character_
...
```{r IO-CV-plots-posterior-predictions-english, fig.height=6, out.width='100%'}
# plot model posteriors of intended category by language
p.english <-  d.test.IO.CV %>% filter(Model.NativeLanguage == "English") %>% 
  ggplot(aes(x=Sound, y=Model.Posterior.Intended)) + 
  geom_hline(yintercept = 1/2, color = "gray") +
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
...
```{r IO-CV-plots-posterior-predictions-swedish, include= FALSE}
p.swedish <- d.test.IO.CV %>% filter(Model.NativeLanguage == "Swedish") %>% 
  ggplot(aes(x=Sound, y=Model.Posterior.Intended)) + 
  geom_hline(yintercept = 1/2, color = "gray") +
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
  ...
  scale_fill_manual(values = colors.Group) +
   scale_y_continuous("Accuracy") +
  xlab("Intended sound category") +
  facet_wrap(.~ RunCount, nrow = 3, ncol = 4)
p.swedish
```


```{r IO-CV-plots-by-test-bin-english, include= FALSE}
# facet plots of each training fold by test bin 

 d.test.IO.CV %>% filter(Model.NativeLanguage == "English" & Model.Group.matchesParticipant.Group == "yes") %>%
   group_by(RunCount, Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
   summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)) 
    
  #   ggplot(aes(x = Sound, y = Model.Posterior.d)) +
  #   stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
  #              alpha = .6, position = position_dodge(.93)) +
  # stat_summary(aes(group = Model.Group), fun.data = mean_cl_boot, geom = "linerange", 
  #              size = .8, color = "black", position = position_dodge(.93)) +
  # scale_color_manual(values = colors.Group) +
  # scale_fill_manual(values = colors.Group) +
  # ylab("Ideal observer predicted\n/d/-goodness rating (z-score aligned)") +
  # scale_fill_manual("Ideal observer",
  ...
  #   values = colors.Accent) +
  # ylim(-1.25, 1.25) +
  # facet_grid(Training.Iteration ~ Test.Bin)
```


```{r IO-CV-plots-all-test-tokens-english, include= FALSE}
# facet plot of the training runs tested on all test tokens (English) with their respective predicted d-goodness ratings for each item

runs_full_set %>% filter(Model.NativeLanguage == "English") %>% 
  ggplot(aes(x=Sound, y=Model.Posterior.Intended)) + 
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
               position = position_dodge(.93)) +
  ...
  scale_color_manual(values = colors.Group) +
  scale_fill_manual(values = colors.Group) +
  facet_wrap(.~ Training.Iteration)


 p.results.perception.bySubject %+%
(runs_full_set %>% filter(Model.NativeLanguage == "English" & Model.Group.matchesParticipant.Group == "yes") %>%
     group_by(Training.Iteration, Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1))) +
  ylab("Ideal observer predicted\n/d/-goodness rating (z-score aligned)") +
  scale_fill_manual("Ideal observer",
    breaks = c("/d/-exposure", "control"),
...
```{r IO-CV-plots-all-test-tokens-swedish, include= FALSE}
# facet plot of the training runs tested on all test tokens Swedish with their respective predicted d-goodness ratings for each item
 p.results.perception.bySubject %+%
(runs_full_set %>% filter(Model.NativeLanguage == "Swedish" & Model.Group.matchesParticipant.Group == "yes") %>%
     group_by(Training.Iteration, Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1))) +
  ylab("Ideal observer predicted\n/d/-goodness rating (z-score aligned)") +
  scale_fill_manual("Ideal observer",
...
d.test.IO.byItem <- 
  ...
  mutate(
    Model.Posterior.d = Model.Posterior.d, # do we need this line?

...
d.test.IO.t <- d.test.IO %>% filter(Model.Group.matchesParticipant.Group == "yes") %>%
  ...
  rstatix::pairwise_t_test(Rating.mean ~ Model.Group, paired = TRUE) 
...
p.IO_prediction + 
  stat_pvalue_manual(
    d.test.IO.t %>% add_x_position(x = "Sound", dodge = 0.9), 
    y.position = 1.2,
    label = "p.adj.signif",
    remove.bracket = FALSE,
    bracket.size = .3,
...
stat.test <- d.test.IO.wTraining_folds %>%
     ...
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)) %>% 
  group_by(Language, NativeLanguage, Sound, Training_fold) %>%
     pairwise_t_test(Rating.mean ~ Model.Group, paired = T
)

# stat.test <- compare_means(
#   Rating.mean ~ Model.Group, 
#   data = d.test.IO.wTraining_folds %>%
#      filter(Model.Group.matchesParticipant.Group == "yes") %>%
#      group_by(Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group, Training_fold) %>%
#      summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)), 
#   group.by = c("Language", "NativeLanguage", "Sound", "Training_fold"),
#   method = "wilcox.test",
#   comparison = c(1,2),
#   paired = T
# )
...
#           group2 = `control`) %>% 
#   group_by(Language, Training_fold, Test_fold) 
...
stat.test <- d.test.IO.wTraining_folds.wTest_folds %>%
     ...
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)) %>% 
  group_by(Language, NativeLanguage, Sound, Training_fold, Test_fold) %>%
     pairwise_t_test(Rating.mean ~ Model.Group, paired = T
)
...
plot_grid(
  p.IO_prediction %+% 
    ...
    stat_pvalue_manual(stat.test %>% 
                         ...
                         add_x_position(x = "Sound", dodge = .9),
                       y.position = 1.125, 
                       label = "p.adj.signif",
                       bracket.size = .5,
                       tip.length = .01) +

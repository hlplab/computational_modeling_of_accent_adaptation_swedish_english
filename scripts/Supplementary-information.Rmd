...
We thus obtained context-corrected phonetic cues by estimating (via linear regression) and removing (via residualization) the effects of phonological context from vowel, closure, and burst duration. By both training and testing the ideal observer on context-corrected cues, we aim to compare likes with likes. The literature contains many competing approaches to normalization [for a recent review, see @weatherholtz2016speech; @flynn2011comparing; @clopper2009computational; and @adank2004comparison]. Here we follow the *C-CuRE* model [@mcmurray2011information], which captures the intuition behind many previous proposals---that listeners interpret cues relative the cue value that is *expected* in the present phonological context (*C-CuRE* stands for "computing cues relative to expectation"). C-CuRE-corrected cues have been found to provide a good fit against human categorization of English fricatives [@mcmurray2011information; @mcmurray2016comes] though other studies have found that C-CuRE does not fully capture listeners' ability to correct for talker-specific characteristics, [@kleinschmidt2019structure; @xie2021encoding]. Beyond studies that have directly evaluated the empirical coverage of C-CuRE, it is common to center (and sometimes standardize) cues when dealing with speech from multiple talkers [@lobanov1971classification; @nearey1978phonetic; @johnson200815]. This is essentially what C-Cure does. 
...
<!-- to be added once we are sure that C-CuRE normalisation is best.
In determining the linear regression model for estimating the effects of segmental context we used the ```validation()``` function available in the rms package for R [@Harrell2020rms]. Each cue was regressed onto the full combination of contextual factors that could explain its variability. The predictors considered for English were 
...
The validation employed a group cross-validation method with the backward step-down approach for each training sample. The final models for the respective cues for English were as follows:
```vowel ~ 1 + number_segments + Talker + coda_has_lateral + coda_has_nasal + onset_has_rhotic```
```closure ~ 1 + number_segments + Talker + coda_has_lateral + onset_has_lateral + onset_has_rhotic``` 
```burst ~ 1 + number_segments + Talker + coda_has_lateral```

For Swedish phonological vowel length and the presence of a nasal were expected to have significant effects on vowel and closure durations. The model for the respective cues were ```cue ~ 1 + number_segments + has_long_vowel + coda_has_nasal```
//-->
...
d.test.IO$Model.Posterior.d = d.test.IO$Model.Posterior.d - mean(d.test.IO$Model.Posterior.d)

lmer.IOtoRating.ModelComparison =
  lmer(
    Rating.Response.d.zscored ~ 1 + NativeLanguage / (Model.Posterior.d * Model.Accent.matchesGroup) + 
      (1 | Item.MinimalPair) + (1 | Participant), 
                     data = d.test.IO)
  ...
  if (show.difference) {
    .data.test %<>%
      select(
        ...
        -c(Model.Posterior.d, Model.Posterior.t)) %>%
      pivot_wider(

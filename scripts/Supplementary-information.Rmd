---
title: "Supplementary Information for 'Using rational models to interpret the results of experiments on accent adaption.'"
...
plot_IO_test = function(
  ...
  .data.test %<>% 
    filter(Model.Group.matchesParticipant.Group == "yes") %>%
     distinct(NativeLanguage,Item.Word,Item.WordID, Item.MinimalPair, Model.Group,Model.Posterior.t, Model.Posterior.Intended, Model.Group.matchesParticipant.Group, rvowel, rclosure, rburst, Sound)
  ...
  if (!is.null(.data.test)) {
      ...
      p = plot_ly(
        ...
        mode="markers"
                  ...
                  marker = list(color = ~Model.Posterior.Intended.Difference,
                                size = I(200),
                                autocolorscale = F,
                                colorscale = list(c(0,'rgb(153,153,153)'), 
                                                   list(0.5,'rgb(255, 255, 255)'),
                                                   list(1,'rgb(255,10,10)')), # list three colors: gray,white and red
                                cmin = -1,
                                cmid = 0,
                                cmax = 1,
    ...
    # ... or posterior by accent
      ...
      data = .data.test %>% # plot the ellipses
        filter(NativeLanguage %in% language) %>%
        mutate(
          vowel = !! rlang::sym(cues[1]),
          closure = !! rlang::sym(cues[2]),
      ...
      frame= ~Model.Group,
      type="scatter3d", 
      text = ~paste("Posterior predicted /t/:", signif(Model.Posterior.t, 4),
      ...
      showlegend = T) %>%
        add_markers(x= ~vowel,
                    ...
                    color = ~Sound,
                    colors = colors.Category,
                    size = I(200), #plot the individual tokens with a large size (show up as the outline color)
                    showlegend = F) %>%
        add_markers(x= ~vowel, # plot the individual tokens
                    y= ~closure,
                    z= ~burst,
                    marker = list(color= ~Model.Posterior.t, # the bottom of the scale is 0 (1d and 0t, the middle is 0.5d and 0.5t, the top is most 0d and 1t)
                                  size = I(10),
                                  autocolorscale = F,
                                  colorscale = list(c(0,'rgb(143, 206, 255)'),
                                                    list(0.5,'rgb(255, 255, 255)'),
                                                    list(1,'rgb(255, 165, 0)')
                                                   ), # list three colors: lightblue, white and orange
                                  cmin = 0,
                                  cmid = 0.5,
                                  cmax = 1,
                                  colorbar = list(len = 0.5,
                                                  title = 'Model-predicted posterior\nof /t/ category')
                                  )
                    )
  } else p = plot_ly()
...
```{r plot-3d-test-IO-English-d-exposure, fig.cap="(ref:plot-3d-test-IO-English)", fig.height=4.5, out.width="75%"}
...
d.temp <- d.test.IO %>%
  filter(Model.Group.matchesParticipant.Group == "yes") %>%
     distinct(NativeLanguage,Item.Word,Item.WordID, Item.MinimalPair, Model.Group, Model.Posterior.t, Model.Posterior.Intended, Model.Group.matchesParticipant.Group, rvowel, rclosure, rburst, Sound) %>% 
  mutate(
    rvowel.d = ifelse(Sound == "/d/", rvowel, NA),
    rburst.d = ifelse(Sound == "/d/", rburst, NA),
    rclosure.d = ifelse(Sound == "/d/", rclosure, NA),
    rvowel.t = ifelse(Sound == "/t/", rvowel, NA),
    rburst.t = ifelse(Sound == "/t/", rburst, NA),
    rclosure.t = ifelse(Sound == "/t/", rclosure, NA),
  )

# use get_axes() to set axes
d.IO.unnested <- d.IO %>% unnest(ellipse)
rvowel <- as.numeric(d.IO.unnested $rvowel)
rclosure <- as.numeric(d.IO.unnested $rclosure)
rburst <- as.numeric(c(min(d.IO.unnested$rburst), range(d.temp$rburst))) #include the minimum of the ellipse to fully capture it; use range from test data because its maximum value exceeds ellipse's (without, it misses points)
d.temp.cues <- cbind(rvowel, rclosure, rburst)
cues <- names(as_tibble(d.temp.cues))
axes <- get_axes(d.temp.cues, cues)

# fix colorscale to lightblue, white orange; colorbar, and hover information
colorscale.IO = list(c(0,'rgb(143, 206, 255)'),
                      list(0.5,'rgb(255, 255, 255)'),
                      list(1,'rgb(255, 165, 0)'))  # list three colors: lightblue, white and orange
colorbar.posterior.t = list(len = 0.5, title = 'Model-predicted posterior\nof /t/ category')
text.posterior.t = ~paste("Posterior predicted /t/:", signif(Model.Posterior.t, 4),
                          "<br>Intended sound:", Sound,
                          "<br>vowel:", signif(rvowel, 4),
                          "<br>closure:", signif(rclosure, 4),
                          "<br>burst:", signif(rburst, 4))

p = plot_ly(
        data = d.temp %>%
          filter(NativeLanguage == "English" & Model.Group == "/d/-exposure") %>% 
          mutate(
            vowel = rvowel,
            closure = rclosure,
            burst = rburst),
      ...
      #frame= ~Model.Group,
      type="scatter3d", 
      text = text.posterior.t,
      hoverinfo = "text",
      mode="markers") %>% 
      add_markers(x = ~vowel,
        y = ~closure,
        z = ~burst,
        marker = list(color= ~Model.Posterior.t,
                                  size = I(250),
                      opacity = 0.7,
                                  autocolorscale = F,
                                  colorscale = colorscale.IO, 
                                  cmin = 0,
                                  cmid = 0.5,
                                  cmax = 1,
                                  colorbar = colorbar.posterior.t)) %>%
    layout(legend = list(itemsizing = "constant"),
              scene = list(xaxis = axes[[1]],
                           yaxis = axes[[2]],
                           zaxis = axes[[3]],
                           aspectmode='cube'))

  # add_trace(x= ~rvowel.d,
  #                   y= ~rclosure.d,
  #                   z= ~rburst.d,
  #           marker = list(symbol = "circle-open",
  #                         line = list(color = 'rgb(143, 206, 255)'))) %>%
  # add_trace(x= ~rvowel.t,
  #                   y= ~rclosure.t,
  #                   z= ~rburst.t,
  #           marker = list(symbol = "square-open",
  #                   line = list(color = 'rgb(255, 165, 0)'))) %>% 

plot_IO(d.IO, language = "English", .plot = p, cues = cues, alpha = 0.2, addSlider = T)
```


```{r plot-3d-test-IO-English-control, fig.cap="(ref:plot-3d-test-IO-English)", fig.height=4.5, out.width="75%"}

# plot Control Exposure English IO points separately because plotly confuses the point fills if plotted together with the d-exposure condition

p2 = plot_ly(
        data = d.temp %>%
          filter(NativeLanguage == "English" & Model.Group == "control") %>% 
          mutate(
            vowel = rvowel,
            closure = rclosure,
            burst = rburst),
      ...
      add_markers(x = ~vowel,
        ...
        marker = list(color= ~Model.Posterior.t,
                      ...
                      opacity = 0.7,
                                  autocolorscale = F,
                                  colorscale = colorscale.IO, # list three colors: lightblue, white and orange
                                  cmin = 0,
                                  cmid = 0.5,
                                  cmax = 1,
                                  colorbar = colorbar.posterior.t)) %>%
...
plot_IO(d.IO, language = "English", .plot = p2, cues = cues, alpha = 0.2, addSlider = T)

#currently the ellipses layer includes both exposure conditions, for current plot, slide it to the appropriate exposure condition (control)
```



```{r plot-3d-test-IO-English-difference, fig.cap="(ref:plot-3d-test-IO-English-difference)", fig.height=4.5, out.width="75%", cache=FALSE}
#plot_IO_test(d.IO, d.test.IO, language = c("English"), cues = selected_cues, show.difference = T)

# plot separately because previous function call failed to generate specified fill
d.temp.diff <- d.test.IO %>%
  filter(Model.Group.matchesParticipant.Group == "yes") %>%
     distinct(NativeLanguage,Item.Word,Item.WordID, Item.MinimalPair, Model.Group,Model.Posterior.t, Model.Posterior.Intended, Model.Group.matchesParticipant.Group, rvowel, rclosure, rburst, Sound) %>%
  select(-Model.Posterior.t) %>%
  pivot_wider(
        names_from = Model.Group,
        values_from = Model.Posterior.Intended
      ) %>%
...
# fix colorscale to red, white, grey
colorscale.IO.difference = list(c(0,'rgb(153,153,153)'), 
                                list(0.5,'rgb(255, 255, 255)'), 
                                list(1,'rgb(255,10,10)'))
colorbar.difference = list(len = 0.5, title = 'Advantage of\n/d/-exposure over\ncontrol model')
text.posterior.difference = ~paste("Advantage:", signif(Model.Posterior.Intended.Difference, 4),
                     "<br>vowel:", signif(rvowel, 4),
                     "<br>closure:", signif(rclosure, 4),
                     "<br>burst:", signif(rburst, 4))
...
p = plot_ly(
        ...
        showlegend = T,
        type="scatter3d",
        text= text.posterior.difference,
        hoverinfo= "text",
        mode="markers") %>%
        add_markers(x= ~vowel,
                  y= ~closure,
                  ...
                  marker = list(color = ~Model.Posterior.Intended.Difference,
                                size = I(250),
                                 colorscale = colorscale.IO.difference, # list three colors: gray, white and red
                                cmin = -1,
                                cmid = 0,
                                cmax = 1,
                                colorbar = colorbar.difference,
                                showscale = T)) %>%
 layout(legend = list(itemsizing = "constant"),
        scene = list(xaxis = axes[[1]], 
                     yaxis = axes[[2]], 
...
```{r plot-3d-test-IO-Swedish-d-exposure, fig.cap="(ref:plot-3d-test-IO-Swedish)", fig.height=4.5, out.width="75%"}
#plot_IO_test(d.IO, d.test.IO, language = "Swedish", cues = selected_cues)

p3 = plot_ly(
        data = d.temp %>%
          filter(NativeLanguage == "Swedish" & Model.Group == "/d/-exposure") %>% 
          mutate(
            vowel = rvowel,
            closure = rclosure,
            burst = rburst),
...
plot_IO(d.IO, language = "Swedish", .plot = p3, cues = cues, alpha = 0.2, addSlider = T)
```


```{r plot-3d-test-IO-Swedish-control, fig.cap="(ref:plot-3d-test-IO-Swedish)", fig.height=4.5, out.width="75%"}
# plot Control Exposure Swedish IO separately because plotly confuses the point fills if plotted together

p4 = plot_ly(
        data = d.temp %>%
          filter(NativeLanguage == "Swedish" & Model.Group == "control") %>% 
          mutate(
            vowel = rvowel,
            closure = rclosure,
            burst = rburst),
...
plot_IO(d.IO, language = "Swedish", .plot = p4, cues = cues, alpha = 0.2, addSlider = T)
...
p = plot_ly(
        data = d.temp.diff %>%
          ...
          mutate(
            ...
            closure = rclosure,
            burst = rburst,
            Model.Posterior.Intended.Difference.Adjusted = Model.Posterior.Intended.Difference + 0.5),
        symbol= ~Sound,
        symbols = shapes.Category,
        color = ~Sound,
        colors = colors.Category,
        ...
        add_markers(x= ~vowel,
                  ...
                  marker = list(color = ~Model.Posterior.Intended.Difference,
                                size = I(200),
                                 colorscale = colorscale.IO.difference,
                                cmin = -1,
                                cmid = 0,
                                cmax = 1,
                                colorbar = colorbar.difference,

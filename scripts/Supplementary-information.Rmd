...
p.loglik.by.fold <- d.test.IO.wTraining_folds.wTest_folds.byItem %>% 
   crossing(
    bias.d = .5,
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>%
...
p.loglik.by.fold %>% mutate(Model.Group.matchesParticipant.Group = ifelse(Model.Group.matchesParticipant.Group == "yes", "Matching", "Mismatching")) %>% 
  ...
  scale_color_manual("Exposure group",
    ...
    labels = c("/d/-exposure", "control"),
    values = colors.Accent) +
  scale_fill_manual("Exposure group",
    breaks = c("/d/-exposure", "control"),
    labels = c("/d/-exposure", "control"),
    values = colors.Accent) +
  # scale_linetype_manual("Ideal observer",
  ...
  #   values = 1:2) +
  #coord_trans(x = "log10") +
  facet_grid(Model.Group.matchesParticipant.Group ~ Language) +
  theme(panel.grid.minor = element_blank())

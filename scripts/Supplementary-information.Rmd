...
get_tvalue <- function(.data, include.sound) {
  my.model <- lmer_IO(.data, include.sound) %>% tidy()
  if (include.sound) {
   return(my.model[[3, 6]])
  } else { 
   return(my.model[[2, 6]])
...
```{r IO-t-values-posterior-d, fig.cap="(ref:IO-t-values-posterior-d)", fig.height=6.5, fig.width=6.5, out.width='85%'}
my.models <- full_join(models.sound.likelihood, models.likelihood)

my.models %>% mutate(model.label = ifelse(include.sound == FALSE, "model without sound", "model with sound")) %>% 
  ggplot(aes(x = t.value.posterior, y = log.likelihood.per.token, colour = Group, shape = Model.Group.matchesParticipant.Group)) +
  geom_point(size = 2.5, alpha = 0.7, position = position_dodge(width = 0.95)) +

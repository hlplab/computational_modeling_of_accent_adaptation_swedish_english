...
d.test.nested <- d.test.IO.n %>% 
  crossing(
    ...
    # lapse_rate = 10^c(seq(-10, 0, length.out = 51))
    bias.d = .5, 
    lapse_rate = 10^c(seq(-10, 0.1, length.out = 51))) %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>% 
...
### Obtaining model likelihoods 

We use a linear mixed-effects regression to predict human /d/-goodness ratings from the posterior probability of /d/ predicted by the ideal observers, while also including the maximal converging random effect structure (random intercepts by participant and minimal pair). For each experimental grouping within each language, we regressed the human /d/-ratings on predicted posterior probabilities of /d/ by the matching ideal observer model as well as the non-matching ideal observer model. Then we obtain the log likelihood of each linear model. 

```{r compare-log-likelihood, echo = FALSE}
lmer_model <- function(.data) {
  loglik <- lmer(Rating.Response.d.zscored ~ 1 + Model.Posterior.d + 
         (1 | Item.MinimalPair) +  (1 + Model.Posterior.d | Participant), 
                     data = .data) %>% logLik()
  return(loglik[[1]])
}

model.likelihood <- d.test.IO %>% 
  group_by(NativeLanguage, Group, Model.Group.matchesParticipant.Group) %>% 
  nest() %>% 
  mutate(
    log.likelihood = map_dbl(data, ~ lmer_model(.x))
  ) 

# get the log likelihood for all training-test fold models
my.models <- d.test.IO.wTraining_folds.wTest_folds %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Training_fold, Test_fold) %>% 
  nest()%>% 
  mutate(
    log.likelihood = map_dbl(data, ~ lmer_model(.x))
...
# plot_grid(my.models %>% filter(Language == "Mandarin-accented English") %>% 
#   ggplot(aes(x = Group , y = log.likelihood, colour = Group, shape = Model.Group.matchesParticipant.Group)) +
#   geom_point() +
#      scale_color_manual(values = colors.Accent) +
#   scale_fill_manual(values = colors.Accent) +
#     scale_shape_manual("Ideal Observer", breaks = c("Matching", "Mismatching"), values = c(1, 2)) +
#   facet_grid(Training_fold ~ Test_fold, scales="free_y") +
#     ggtitle("Mandarin-accented English"),
#   my.models %>%
...
```{r IO-log-likelihood-training-test-folds}
my.models %>% mutate(Model.Group.matchesParticipant.Group = ifelse(Model.Group.matchesParticipant.Group == "yes", "Matching", "Mismatching")) %>% 
  ggplot(aes(x = Group , y = log.likelihood, colour = Group, shape = Model.Group.matchesParticipant.Group)) +
  stat_summary(fun.data = "mean_cl_boot", size = 0.6, position = position_dodge(width = 0.3)) +
     scale_color_manual(values = colors.Accent) +
  scale_fill_manual(values = colors.Accent) +
    scale_shape_manual("Ideal Observer", breaks = c("Matching", "Mismatching"), values = c(1, 2)) +
  facet_wrap( ~ Language)
...
d.IBBU.test <-
  ...
  group_by(NativeLanguage, prior_kappa, prior_nu) %>%
  dplyr::select(Item.MinimalPair, Sound, x, category, m, S, kappa, nu, lapse_rate) %>%
...
d.test.IBBU <-
  ...
  arrange(NativeLanguage, prior_kappa, prior_nu, Item.MinimalPair, Sound, category) %>%
  group_by(NativeLanguage, prior_kappa, prior_nu) %>%
  dplyr::select(Item.MinimalPair, Sound, x, Rating.Response.d.zscored,

...
get_tvalue <- function(.data, include.sound) {
  my.model <- lmer_IO(.data, include.sound) %>%
    tidy()
  post.d <- 2
  t.value <- 6
  return(my.model[[post.d, t.value]])
}
...
p.likelihood <- models.likelihood %>% 
  ...
  geom_point(aes(alpha = 0.04,size = 0.1),
               colour = "lightgrey", 
               position = position_dodge(width = 0.3), 
               show.legend = FALSE)
  
  p.likelihood + 
    stat_summary(geom = "pointrange", fun.data = mean_cl_boot, size = 0.7, position = position_dodge(width = 0.3)) +
...
p.sound.likelihood <- models.sound.likelihood %>% 
  ggplot(aes(x = Group , y = log.likelihood.per.token, colour = Group, shape = Model.Group.matchesParticipant.Group)) +
  geom_point(aes(alpha = 0.04, size = 0.4, shape = Model.Group.matchesParticipant.Group), 
               colour = "lightgrey", 

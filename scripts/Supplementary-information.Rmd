...
Comparing the models against
The following functions are written based on a broader definition for the binomial coefficient as detailed in https://www.johndcook.com/blog/binomial_coefficients/ . I used DEFINITION 2 to create ```binom_coeff()```. This definition is supposed to be implemented in ```choose(r, k)``` function but when I used ```choose()``` it warns of non integer values and rounds up. We need an extended version of the binomial pmf that can take non-integer values.

Our aim is to check the likelihood of the human ratings under the parameters of the various ideal observer models (matching vs. non-matching human groups), lapse rates and bias rates. The per-item human ratings are all expressed as d-goodness ratings, z-scored within participants, and normalised to between 0 and 1. We also note the number of ratings for each item (n.ratings) which we take to be the number of tries, n. We multiply n.ratings by the average normalised ratings per item (n.ratings*average normalised.ratings) to get the number of successes, k. For each item we can get the likelihood under the respective model's parameters, which is it's posterior for d after factoring in the lapse and bias rates. $$\binom{n}{k}p^k(1-p)^{n-k}$$ The log of which gives: $$log\binom{n}{k}+ k\ log(p) + (n-k)log(1-p)$$ After the per token log likelihood is obtained, the data is grouped by Language, Model, Model.Group.matchesParticipant.Group and the average per token log likelihood is computed, summarising the result to 8 log likelihood values.
...
# below I removed pull(per_token_likelihood) because I figured we want a column of average per token log likelihood for each model. pull() seems to cause problems
get_average_log_likelihood <- function(data, lapse_rate = .1, bias.d = .5) {
  data %>% 
    mutate(
      prediction.d = (1 - lapse_rate) * Model.Posterior.d + lapse_rate * bias.d,
...
# this gives 8 log likelihood values (the sum of log likelihood calculations are divided by the number of log likelihoods -- the number equals the number of tokens, within each language, model group and match-- for English this is 120 and Swedish this is 60)
temp <- get_average_log_likelihood(d.test.IO.n) 

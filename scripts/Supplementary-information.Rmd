...
library(assertthat)
if (has_internet()) devtools::install_github('kleinschmidt/daver')
library(daver)
if (has_internet()) devtools::install_github("kleinschmidt/phondisttools")
...
## Lexical decision accuracy during exposure {#sec:lexical-decision-accuracy}
...
## Assumptions of the ideal observers 
Table \@ref(tab:IO-assumptions-table) summarizes the assumptions made by the computational approach used to understand the difference between the English and Swedish data. The final column summarizes whether an assumption is likely to cause a problem for our argument—i.e., does the assumption trivially explain the model’s predictions for English and Swedish?

```{r IO-assumptions-table, out.width="100%"}
correction_table <- as_tibble(read_csv2("../data/IO_assumptions_table.csv")) 
correction_table %>% 
  kbl(caption = "Summary of IO assumptions") %>% 
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = F) %>% 
  column_spec(1, bold = T, border_right = T) %>% 
  column_spec(2, bold = F, border_right = T) %>% 
  column_spec(3, bold = F, border_right = F)  
  # column_spec(4, bold = F, border_right = T, width = "9em") %>% 
  # column_spec(5, bold = F, border_right = T, width = "6em") 
```
...
```{r ideal-observers-posterior, fig.cap="(ref:ideal-observers-posterior)", fig.height=4.5, out.width="95%"}
p.post = d.IO.test %>%
  ggplot(aes(x= Sound, 
...
```{r ideal-observers-accuracy, fig.cap="(ref:ideal-observers-accuracy)", fig.height=4.5, out.width="95%"}
p <- p.post %+% 
  (d.IO.test) +
...
```{r ID-to-ratings, fig.cap="(ref:ID-to-ratings)", fig.height = 4.5, out.width="95%"}
...
```{r IO-ratings-results-byItem, fig.cap="(ref:IO-ratings-results-byItem)", fig.height=4.5, out.width='95%'}
...
```{r ratings-results-byItem, fig.cap="(ref:ratings-results-byItem)", fig.height=4.5, out.width='95%'}
...
```{r IO-ratings-results-byItem-wTraining-folds, fig.cap="(ref:IO-ratings-results-byItem-wTraining-folds)", fig.height=8.5, out.width='95%'}
...
This analysis is summarized in Table \@ref(tab:ratings-IO-analysis) and found a highly significant effect of IO-predicted posterior probabilities on the z-scored human goodness ratings ($\hat{\beta} = 1.22, t = 25.09, p < .001$). 

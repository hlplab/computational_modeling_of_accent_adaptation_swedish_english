...
# Each training iteration uses 4/5 of the training data; data from the non-native talkers are never down-sampled. exclude() drops 1/5 of the training data but keeps 5/5 of non-native talkers' data.
...
d.nonnative.test.bins %>% filter(NativeLanguage == "Swedish") %>% 
group_by(Test.Bin) %>% summarise(n())
...
# test each of 5 models on each test bin ### THIS RUNS INTO PROBLEMS AT TEST.BIN == 3 BECAUSE THERE ISN"T BINS 3 & 4 for SWEDISH
...
join_to_human_ratings <- function(d.test.humans, model.result) {
   d.test.humans %>% droplevels() %>% 
    left_join(model.result) %>% 
    mutate(
    Model.Group.matchesParticipant.Group = factor(case_when(
      NativeLanguage != Model.NativeLanguage ~ NA_character_,
      Group == Model.Group ~ "yes",

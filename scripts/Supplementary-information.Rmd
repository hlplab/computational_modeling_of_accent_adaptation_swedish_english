...
d.stims = 
  ...
  select(-mimic, -phon_ctxt_canonical_vl, -MinimalPairID, -Item.Filename, -Word) %>%
  rename(
  ...
  select(Language, Talker, Block.Type, Item.MinimalPair, Item.WordID, Item.Word, Sound,
         vowel, closure, burst, 
...
```{r functions to correct for phonological context}
...
# /d/ and /t/ test words (data.fit) for the specified experiment (language). The predictions of 
# this linear regression are then substracted from *all* data (data.predict: exposure /d/ + test 
# /d/ + test /t/). The function can correct for segmental (what_for = "segmental") or word length 
# (what_for = "stress") context.

# We do not include lexical stress, coda fricative, rhotic, or lateral since these variables
# did not vary within test within the test stimuli.
correct_for_phonctxt = function(cue.name, 
                          data.fit, 
                          data.predict, 
                          what_for = c("segmental", "length"), 
                          language = c("English", "Swedish")) {
  if (is.null(data.predict)) 
  ...
  f = formula(
    case_when(
      what_for == "length" ~ "y ~ 1 + number_syllables",
      language == "English"~ "y ~ 1 + (onset_has_nasal + coda_has_nasal)",
      language == "Swedish" ~ "y ~ 1 + (has_long_vowel + coda_has_nasal)",
      T ~ NA_character_))
  
  l = lm(f, data = data.fit)
  cat(paste0("Effects of segmental context on ", cue.name, "duration for the ", language, "test data.\n"))
  print(summary(l))
  # tab_model(l, 
  #           title = paste0("Effects of segmental context on ", cue.name, "duration for the ", language, "test data")) %>%
  #   return() %T>%
  #   knitr() %>%
  #   as_is_output()
...
We estimated the effects of segmental context from the foreign-accented /d/- and /t/-final words from the test set. We did not include words used during the training of the ideal observers in the fitting of the regression, because training words

The phonological contexts considered differ somewhat for the Swedish and English datasets, based on previous findings on context effects in the two languages as well as the type of contexts present in each dataset. For the English data, we considered the effect of preceding and following nasals. This was achieved by fitting a separate linear regression to each of the three cues `lm(cue ~ 1 + preceded_by_nasal + followed_by_nasal)`. For the Swedish data, we considered two phonological contexts (phonologically long vs. short vowel; following nasal, e.g., *XXX*---'XXX' vs. *XXX*---'XXX'). This was achieved by fitting a separate linear regression to each of the three cues: `lm(cue ~ 1 + has_long_vowel + preceded_by_nasal)`. Across the two languages, there were a total of six linear regressions. 

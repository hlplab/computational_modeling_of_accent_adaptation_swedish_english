...
Regardless of the specific source of the difference between the Flemish- and Mandarin-accented recordings, the question arises how to *predict* the effect of exposure on native listeners' perception given a specific set of non-native-accented speech stimuli. This motivates the present study: we sought to present a simple but principled approach to quantify the predicted effect of exposure on subsequent perception (here /d/- and /t/-goodness ratings, though the approach we present generalizes to categorization tasks). Ideal observers provide a tool for this purpose, making it possible to predict the effect of exposure on ratings with zero computational degrees of freedom based on only 1) the phonetic properties of the non-native-accented exposure stimuli, 2) the phonetic properties of the  non-native-accented test stimuli, and 3) the phonetic properties of native-accented speech. The third and final component requires some estimates of the phonetic distributions of `typical' native-accented speech. We thus introduce and describe additional data sets containing information about native pronunciations of the same materials used in the perception experiments on non-native-accented speech. 
...
This analysis is summarized in Table \@ref(tab:ratings-IO-analysis) and found a highly significant effect of ideal observer predicted posterior probabilities on the z-scored human goodness ratings ($\hat{\beta} = 1.27, t = 26.6, p < .001$). 
...
plot_IO_test = function(
  ...
  if (!is.null(.data.test)) {
    ...
    if (show.difference) {
      p = plot_ly(
       ...
       add_markers(x= ~vowel,
                  y= ~closure,
                  z= ~burst,
                  marker = list(color = ~Model.Posterior.Intended.Difference.Adjusted,
                                size = I(10),
                                 colorscale = list(c(0,'rgb(153,153,153)'), list(0.5,'rgb(255, 255, 255)'), list(1,'rgb(255,10,10)')), # list three colors: gray, white and red
                                cmin = 0,
                                cmid = 0.5,
                                cmax = 1,
    ...
    # ... or posterior by accent
      ...
      type="scatter3d", 
      mode="markers") %>%
        add_markers(x= ~vowel, # plot the individual tokens with a large size (show up as the outline color)
                  y= ~closure,
                  z= ~burst,
          color = ~Sound,
          colors = colors.Category,
        ...
        add_markers(x= ~vowel, # plot the individual tokens
                  ...
                  z= ~burst,
                  marker = list(
                      color= ~Model.Posterior.t, # the bottom of the scale is 0 (1d and 0t, the middle is 0.5d and 0.5t, the top is most 0d and 1t)
                      size = I(10),
                     # cauto = T,
                      autocolorscale = F,
                     colorscale = list(c(0,'rgb(143, 206, 255)'), list(0.5,'rgb(255, 255, 255)'), list(1,'rgb(255, 165, 0)')), # list three colors: lightblue, white and orange
                     cmin = 0,
                     cmid = 0.5,
                     cmax = 1,
                     colorbar = list(len = 0.7, title = 'Model-predicted posterior\nof intended category')
        )
        )
  } else p = plot_ly()
...
(ref:plot-3d-test-IO-English-difference) Difference between non-native and control ideal observer predicted posterior probabilities of the *intended* category for all test tokens of the experiment on English. The posterior probability predicted by the control ideal observer is subtracted from the posterior probability predicted by the /d/-exposure ideal observer. Positive value thus indicate test tokens for which the /d/-exposure ideal observer makes better predictions.
...
(ref:plot-3d-test-IO-Swedish-difference) Difference between non-native and control ideal observer predicted posterior probabilities for all test tokens of the experiment on Swedish. 

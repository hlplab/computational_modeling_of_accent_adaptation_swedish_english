...
```{r train-IO-for-cross-validation, include = FALSE}
...
models <- map(seq(1,5), ~ make_IO(exclude(d.IO.training.CV,.x), selected_cues) %>%
                mutate(Training.Iteration = paste0("iteration_", .x)))

#m1.IO <- train_IO(exclude(d.IO.training.CV,1), selected_cues, 1)

```

```{r test-IO-for cross-validation, include= FALSE}
...
runs_full_set <- map(seq(1, length(models)), ~ IO_categorization(select(d.nonnative.test,-c(Test.Bin)), models[[.x]])) %>% bind_rows()
...
# make posterior plots with human ratings 
 d.test.IO.CV %>% ggplot(aes(x = Sound, y = Model.Posterior.Intended))
     filter(NativeLanguage == "English" & Model.Group.matchesParticipant.Group == "yes") %>%
     group_by(Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1))) +
  ylab("Ideal observer predicted\n/d/-goodness rating (z-score aligned)") +
  scale_fill_manual("Ideal observer",

  ...
  .data %>%
    ...
    { if (nlevels(.data$Talker) > 1) {
      group_by(., !!! rlang::syms(setdiff(groups, "Talker"))) %>%
        summarise(
          mu = list(Reduce("+", mu) / length(mu)),
          Sigma = list(Reduce("+", Sigma) / length(Sigma))
        ) } else . } %>%
    select(!!! rlang::syms(setdiff(groups, "Talker")), mu, Sigma)
}
```
...
make_ellipse = function(mu, Sigma, cues = c("vowel", "closure", "burst")) {
...
plot_IO = function(.data.IO, NativeLanguage, .data.training = NULL) {
  # axis setting 
  axx <- list(
    nticks = 4,
    range = range(.data.IO$vowel),
    gridwidth = 2
  )
  
  axy <- list(
    nticks = 4,
    range = range(.data.IO$closure),
    gridwidth = 2
  )
  
  axz <- list(
    nticks = 4,
    range =  range(.data.IO$burst),
    gridwidth = 2
  )
  ...
  if (!is.null(.data.training)) {
      p = plot_ly(
        data = .data.training %>%
          mutate(vowel = rvowel, closure = rclosure, burst = rburst) %>%
...
Figures \@ref(ref:plot-IO-3D-English) and \@ref(ref:plot-IO-3D-Swedish) visualize the native and non-native ideal observers for both English and Swedish that result from this training procedure.

(ref:plot-IO-3D-English) English native and non-native ideal observer visualized in 3D cue space.

(ref:plot-IO-3D-Swedish) Swedish native and non-native ideal observer visualized in 3D cue space.
...
d.IO = d.training %>%
  make_IO(., cues = c("rvowel", "rclosure", "rburst")) %>%
  # Create ellipses of the training data
  mutate(ellipse = map2(mu, Sigma, make_ellipse)) %>% 
  unnest(ellipse)

d.IO %>% plot_IO("English", d.training)
```

```{r plot-IO-3D-Swedish, fig.cap="(ref:plot-IO-3D-Swedish)", out.width="75%", fig.show='hold'}
d.IO %>% plot_IO("Swedish", d.training)
``` 

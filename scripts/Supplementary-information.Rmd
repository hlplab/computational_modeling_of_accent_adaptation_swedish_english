...
get_segmental_model = function(
  ...
  # Fit separate effects for each language & cue based on best model fit
 f = formula(
    case_when(
       ...
       language == "English" & cue.name == "closure" ~
         "y ~ 1 + number_segments + Talker + coda_has_lateral + onset_has_rhotic + onset_has_lateral",
...
# doing conditional filtering below because making dataframe long multiplies the cue names rows by 3. Don't want to fit the regression model with 3 times the cue values. 
  # if(cue.name == "vowel") {
...
# I compared the correct_for_suprasegmental_context() function with and without an offset. The number of terms extracted are the same but term values are different when the model does not include the offset. Presumably the offset is added to the terms 
...
# remove the offset columns from the dataframe. 
# d.stims.pivoted %<>% select(c(-offset_segmental, -offset_suprasegmental_context_vowel, 
                             # -offset_suprasegmental_context_closure, -offset_suprasegmental_context_burst, -segmental_context))
# d.stims.step2.no.offset <- d.stims.step2 %>% 
#   select(c(-offset_segmental, 
#            -offset_suprasegmental_context_vowel,
#            -offset_suprasegmental_context_closure, 
#            -offset_suprasegmental_context_burst, 
#            -segmental_context))

# remove the duplicate rows. There should be 1036 rows but R retained 7 duplicates (1043 rows). Can't figure out the problem.
# d.stims.step3 <- d.stims.step2.no.offset %>% distinct()
```
...
# storing the final residualised values from step 2 as r<cue>

...
### Databases
For the native English IO, the /d/ and /t/ distributions are estimated from a database of 10 male native speakers of American English (Li, Jaeger, & Xie, 2020). Native-accented talkers had the same sex and spanned a similar age range (18-35) as the Mandarin-accented talker in Xie et al. (2017). The recording procedure was the same. 

For the non-native English IO, the /d/ distribution is estimated from the 30 critical words with syllable-final /d/ used during the foreign-accented exposure in Xie et al. (2017). The foreign-accented speech used in Xie et al. (2017) is part of the non-native portion of the Li et al. (2020) database (hence the dotted line between the native English IO's /d/ category and the Li et al., 2020 database in Figure \@ref(fig:ideal-observer-and-data)).

Of note, the words that were used to estimate native-English /d/ and /t/ distributions were not the same as those used for estimating non-native /d/; however, they were all monosyllabic words with word-final /d/s or /t/s and therefore shared the phonological properties of the minimal pairs /d/-/t/ test words used in Xie et al. (2017). We used this dataset instead of tokens from a particular talker for the following two reasons: 1) recordings from native-English speakers producing the same stimuli as used in Xie et al. (2017) were not available; 2) for the purpose of estimating `typical' native-English productions of /d/ and /t/, this dataset was adequate, and perhaps better than data from just a single speaker. Given the sample size of this database of native English, we were able to construct ideal observers that simulate a *prototypical* native-English speaker. We use this *prototypical* speaker as the native IO, instead of any single native-English speaker, as a comparison to our non-native speaker.
...
The data for the non-native /d/ comes from the 30 critical words with syllable-final /d/ used during the foreign-accented exposure in the experiments. 
...
<!-- Xin and Maryann had a talk. Maryann mentioned that for Swedish data, we actually only considered /r/ and nasals before the vowels, i.e., we considered the effect of following /r/ and following nasals only. I've edited the text above-->

...
# fold training: train models, excluding 1/5 each iteration.
d.IO.5folds <- map(
  seq(1,5), 
  ~ make_IO(
...
# Maryann: Please check whether the above is folded the way you intended, and then delete this comment?
# Maryann: The next command should be inheriting the test folds from above (just like it did inherit the absence of them when we used the function earlier)
#          Can you check and then delete this comment?
d.test.IO.5folds.wTest_folds <- join_IO_categorizations_with_human_test(d.IO.test.5folds.wTest_folds, d.test)

# Maryann: when you're done checking, replace all of the above code with this (corrected for whatever you change)
# d.test.IO.5folds.wTest_folds <- 
#   left_join(
#     d.IO.test.5folds, 
#     # Getting the random minimal pair IDs for each test fold
#     d.IO.test.5folds %>%
...
```{r cross-validation-ratings-results-byItem-test-folded-test, fig.cap="(ref:ratings-results-byItem-5folds-folded-test)", fig.height=22.5, out.width='75%'}
...
ggsave(filename = "../figures/Result-ratings-byItem-5folds-folded-test.png",
       width = 21, height = 20)
```

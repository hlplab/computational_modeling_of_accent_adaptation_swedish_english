...
# # train_IO is a minor modification of make_IO() 
# train_IO = function(
#   .data, 
#   cues,
#   excluded_set,
...
#m2.IO <- train_IO(exclude(d.IO.training.CV,2), selected_cues, 2)
#m3.IO <- train_IO(exclude(d.IO.training.CV,3), selected_cues, 3)
#m4.IO <- train_IO(exclude(d.IO.training.CV,4), selected_cues, 4)
#m5.IO <- train_IO(exclude(d.IO.training.CV,5), selected_cues, 5)
...
# join the assigned min.pairs with the main test dataframe
d.nonnative.test <- left_join(d.nonnative.test, d.nonnative.test.bins)
d.nonnative.test %>% group_by(Item.MinimalPair, Test.Bin) %>% summarise(n())


# test models on each test bin
run_tests <- function(model, test_data, test_bin_count) {
  runs = map(seq(1, test_bin_count), ~ IO_categorization(filter(test_data, Test.Bin == .x), model) %>% droplevels() %>% mutate(RunCount = paste0("test_", .x))) 
bind_rows(runs)
}

test_bin_count = 4
results = map(seq(1,length(models)), ~ run_tests(models[[.x]], d.nonnative.test, test_bin_count) %>% mutate(Training.Iteration = paste0("iteration_", .x))) 


results[[1]]

result = bind_rows(results) %>% group_by(Model, Test.Bin)

plot = result %>% filter(Model.NativeLanguage == "English") %>% 
  ...
  scale_color_manual(values = colors.Group) +
  scale_fill_manual(values = colors.Group) +
  facet_grid(RunCount ~ Trainint.Iteration)
plot

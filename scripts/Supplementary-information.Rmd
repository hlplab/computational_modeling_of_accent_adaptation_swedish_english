...
p.loglik.byItem.fold <- d.test.IO.wTraining_folds.wTest_folds.byItem %>% 
   crossing(
    bias.d = .5,
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>%
...
(ref:IO-log-likelihood-wTraining-folds-folded-test) **Per item log likelihood across training-by-test folds**. Lines show per item log likelihoods smoothed all training and test folds, and at each lapse rate for the /d/-exposure and control models that either match or do not match the human exposure groups Shaded areas show 95\% confidence intervals over average per item log likelihoods across all training and test folds, and at each lapse rate for the /d/-exposure and control models that either match or do not match the human exposure groups. Each row shows one training fold excluding a different half of the native training data. Each column shows a separate test bin with 15 randomly chosen minimal pairs (resulting in 4 test folds for the English experiment and 2 test folds for the Swedish experiment). For additional details, see Figure \@ref(fig:IO-ratings-results-byItem).
...
p.loglik.byItem.fold %>% mutate(Model.Group.matchesParticipant.Group = ifelse(Model.Group.matchesParticipant.Group == "yes", "Matching", "Mismatching")) %>% 
  ...
  geom_rug(mapping = aes(x = NULL, y = per_token_loglik)) +
    scale_x_continuous(
    "lapse rate",
    breaks = 10^c(seq(-10, 0, 2)),
    trans = "log10") +

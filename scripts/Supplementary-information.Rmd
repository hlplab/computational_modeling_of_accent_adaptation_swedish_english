...
```{r fig-ratings-IO-item-level, fig.height = 9, out.width="100%"}
ggscatter(d.test.IO %>%
  ...
  facet_grid(Model.Accent.matchesGroup ~ NativeLanguage + Group, 
             # margins = c("NativeLanguage"),
             labeller = "label_both") 

# # Only IOs matching exposure conditions:
# ggscatter(d.test.IO %>%
#             filter(Model.Accent.matchesGroup == "yes") %>%
#             group_by(Language, NativeLanguage, Group, Model.Accent, Model.Accent.matchesGroup, Item.Word, Sound) %>%
#             summarise(Model.Rating.d.zscored = mean(Model.Rating.d.zscored),
#                       Rating.Response.d.zscored = mean(Rating.Response.d.zscored)), 
#           x = "Model.Rating.d.zscored", y = "Rating.Response.d.zscored", color = "Group",
#           cor.method = "pearson",
#           xlab = "Ideal observer predicted rating of /d/", ylab = "human mean /d/-goodness rating") + 
#   stat_cor(label.x.npc = "middle", label.y.npc = "bottom") + 
#   geom_smooth(method = "lm", formula = y ~ x, color = "black", fill = "black") +
#   scale_color_manual(
#     breaks = c("/d/-exposure", "control"),
#     values = colors.Group
#   ) +
#   facet_grid(Group ~ NativeLanguage, labeller = "label_both") 
```
...
d.test.IO <- within(d.test.IO, {
  contrasts(NativeLanguage) = cbind("Swedish vs English" = c(-1,1))
  contrasts(Group) = cbind("/d/-exposure vs control" = c(1, -1))
  contrasts(Sound) = cbind("/d/ vs /t/" = c(1, -1))
  contrasts(Model.Accent.matchesGroup)
})
...
### Do ideal observers that are intended to capture the expousre condition predict human /d/-goodness rating responses *better* than ideal observers intended for another exposure condition?
...
plot_IO_test = function(
  ...
  if (!is.null(.data.test)) {
    p = plot_ly(
      data = .data.test %>%
        filter(NativeLanguage == language) %>%
        mutate(
          vowel = !! rlang::sym(cues[1]),
          closure = !! rlang::sym(cues[2]),
          burst = !! rlang::sym(cues[3])),
  ...
  p = plot_IO(.data.IO, language = language, .plot = p, cues = cues, alpha = .1, addSlider = T)

...
# to predict its values from the number of syllables. The linear regression is fit against the /d/-final words (data.fit) for the specified experiment (language). 
# The predictions of this linear regression are then substracted from *all* data (data.predict: 
# exposure /d/ + test /d/ + test /t/). 
correct_for_word_length = function(cue.name, 
  ...
  f = formula(
    case_when(
      language == "English" ~ "y ~ 1 + Accent / (number_syllables + has_stress + syllable_duration)",
      language == "Swedish" ~ "y ~ 1 + Accent / (number_syllables + has_stress + syllable_duration)",
      T ~ NA_character_))
  
  l = lm(f, data = data.fit)
  print(plot_model(l, type = "est",
                  show.p = T, show.values = T, show.data = T,
            title = paste("Effects of supra-segmental context on", gsub("^r", "", cue.name), "duration for the", language, "test data"),
            axis.title = c(paste("Change in", cue.name, "duration (in msec)"), "")))
...
### Step 3: Correcting for speaking rate

```{r step 3 functions to correct for speaking rate}
# Correct for speaking rate. For the specified cue (cue.name), we use linear regression 
# to predict its values from the syllable duration. The linear regression is fit against *all* words (data.fit: exposure /d/ + test /d/ + test /t/) for the specified experiment (language). 
# The predictions of this linear regression are then substracted from *all* data (data.predict: 
# exposure /d/ + test /d/ + test /t/). 
correct_for_speaking_rate = function(cue.name, 
                          data.fit, 
                          language = c("English", "Swedish")
) {
  data.predict = data.fit
  ...
  f = formula(
    case_when(
      language == "English" ~ "y ~ 1 + syllable_duration",
      language == "Swedish" ~ "y ~ 1 + syllable_duration",
      T ~ NA_character_))
  
  l = lm(f, data = data.fit)
  print(plot_model(l, type = "est",
                  show.p = T, show.values = T, show.data = T,
            title = paste("Effects of speaking rate on", gsub("^r", "", cue.name), "duration for the", language, "test data"),
            axis.title = c(paste("Change in", cue.name, "duration (in msec)"), "")))
...
```{r step 3 correct for speaking rate, eval = FALSE, include = FALSE}
d.stims %<>%
  group_by(NativeLanguage) %>%
  nest() %>%
  mutate(
    rvowel = map2(
      data, NativeLanguage,
      ~ correct_for_speaking_rate("rvowel", 
                             data.fit = .x, 
                             language = .y)),
    rclosure = map2(
      data, NativeLanguage, 
      ~ correct_for_speaking_rate("rclosure", 
                             data.fit = .x,
                             language = .y)),
    rburst = map2(
      data, NativeLanguage, 
      ~ correct_for_speaking_rate("rburst", 
                             data.fit = .x, 
                             language = .y))
...
d.stims %<>%
  unnest(data, rvowel, rclosure, rburst) %>%
  rename(
    rvowel.step3 = rvowel1,
    rclosure.step3 = rclosure1,
    rburst.step3 = rburst1) %>%
  # Override rvowel, rclosure, rburst with second step
  mutate(
    rvowel = rvowel.step3,
...
```{r prepare for cue plot after step 2 and 3, include=FALSE}
d.stims.long = 
  d.stims %>%
  make_long()
```
...
### Step 4: Align all data sets with the non-native test /t/ 
```{r step 4 align to /t/ category from non-native test words}
...
d.stims %<>%
  ...
  mutate(
    rvowel.step4 = rvowel - diff.vowel,
    rclosure.step4 = rclosure - diff.closure,
    rburst.step4 = rburst - diff.burst,
    # Always make sure that the most recent step is *also* stored as rvowel, rclosure, rburst
    rvowel = rvowel.step4,
    rclosure = rclosure.step4,
    rburst = rburst.step4
  ) %>%

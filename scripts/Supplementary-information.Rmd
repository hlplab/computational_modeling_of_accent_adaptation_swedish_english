...
# doing conditional filtering below because making dataframe long multiplies the cue names columns by 3. Don't want to fit the regression model with 3 times the cue values. 
  if(cue.name == "vowel") {
...
# I compared the predict function with and without an offset. The number of terms are the same but term values are different when the model does not include the offset. Presumably this means that the offset is added to the terms 
...
d.stims <- d.stims.pivoted %>% distinct()

 ...
 f = formula(
    case_when(
       language == "English" & cue.name == "vowel" ~ 
         "y ~ 1 + number_segments + Talker + coda_has_lateral + coda_has_nasal + onset_has_rhotic",
       language == "English" & cue.name == "closure" ~
         "y ~ 1 + number_segments + Talker + coda_has_lateral + onset_has_lateral + onset_has_rhotic ",
       language == "English" & cue.name == "burst" ~
         "y ~ 1 + number_segments + Talker + coda_has_lateral",
       ...
       language == "Swedish" & cue.name == "burst" ~
         "y ~ 1 + number_segments + has_long_vowel", #according to validate none of the predictors affect burst. To determine whether we should leave burst values uncorrected.
       T ~ NA_character_))
...
get_predictions_for_segmental_context = function(
  ...
  data.fit %<>%
    ...
    filter(
      Accent == "native" & Block.Type != "exposure"
      )
...
# make data long to facilitate easier step 2 process. keep the long form different from d.stims.long which was used for plotting. Results in 3108 rows and duplicated 
# d.stims.pivoted<- d.stims.step1 %>% pivot_longer(cols = c(offset_segmental_context_vowel,
...
# I compared the get_predictions() function with and without an offset. The number of terms extracted are the same but term values are different when the model does not include the offset. Presumably the offset is added to the terms 
correct_for_suprasegmental_context = function(data.fit,
  ...
  return(
    residuals(
      lm(
        y ~ 1 + Talker,
        offset = get_predictions_for_segmental_context(data.fit, 
                                                       cue.name, 
                                                       language, 
                                                       plot = F) + 
          get_predictions_for_suprasegmental_context(data.fit, 
                                                     cue.name, 
                                                     language, 
                                                     plot = plot),
...
d.stims.step2 <- d.stims.step1 %>% group_by(NativeLanguage) %>% nest() 
...
d.stims.long = 
  d.stims.step2 %>% select(c(-contains("offset"))) %>% 
  make_long()
```
...
```{r native-english-cue-before-and-after-suprasegmental-correction, fig.cap="(ref:native-english-cue-before-and-after-suprasegmental-correction)", out.width="75%", fig.show='hold', eval = FALSE}
plot_corrected_cues(d.stims.long %>% filter(NativeLanguage == "English", Accent == "native"))
```
...
```{r non-native-english-cue-before-and-after-suprasegmental-correction, fig.cap="(ref:non-native-english-cue-before-and-after-suprasegmental-correction)", out.width="75%", fig.show='hold', eval = FALSE}
plot_corrected_cues(
   d.stims.long %>% 
          filter(NativeLanguage == "English", Accent == "non-native"))
```
...
```{r native-swedish-cue-before-and-after-suprasegmental-correction, fig.cap="(ref:native-swedish-cue-before-and-after-suprasegmental-correction)", out.width="75%", fig.show='hold', eval = FALSE}
plot_corrected_cues(
   d.stims.long %>% 
          filter(NativeLanguage == "Swedish", Accent == "native"))
```
...
```{r non-native-swedish-cue-before-and-after-suprasegmental-correction, fig.cap="(ref:non-native-swedish-cue-before-and-after-suprasegmental-correction)", out.width="75%", fig.show='hold', eval = FALSE}
plot_corrected_cues(
   d.stims.long %>% 
          filter(NativeLanguage == "Swedish", Accent == "non-native"))
```

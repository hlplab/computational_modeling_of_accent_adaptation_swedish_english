...
p.likelihood <- models.likelihood %>% 
  ...
  geom_point(aes(alpha = 0.04, size = 1, shape = Model.Group.matchesParticipant.Group), 
               colour = "lightgrey", 
               stroke = 1, position = position_dodge(width = 0.3), 
               show.legend = FALSE)
  
  p.likelihood + 
    stat_summary(geom = "pointrange", fun.data = mean_cl_boot, size = 1, position = position_dodge(width = 0.3)) +

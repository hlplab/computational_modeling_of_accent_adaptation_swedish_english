...
# plot model posteriors by language
p.english <-  d.test.data.CV %>% filter(Model.NativeLanguage == "English") %>% 
...
p.swedish <- d.test.data.CV %>% filter(Model.NativeLanguage == "Swedish") %>% 
  ggplot( aes(x=Sound, y=Model.Posterior.Intended)) + 
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
               position = position_dodge(.93)) +
...
# facet plots of each traning fold by test bin 
 d.test.IO.CV %>% 
   group_by(Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
   #summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)) 
    filter(Model.Group.matchesParticipant.Group == "yes") %>%
...
# facet plot of the training runs tested on all test tokens
 p.results.perception.bySubject %+%

...
Figures \@ref(fig:plot-IO-3D-English) and \@ref(fig:plot-IO-3D-Swedish) visualize the native and non-native ideal observers for both English and Swedish that result from this training procedure, along with the training data that was used to train these four models.
...
d.IO %>% plot_IO("English", d.IO.training, cues = selected_cues)
```

```{r plot-IO-3D-Swedish, fig.cap="(ref:plot-IO-3D-Swedish)", out.width="75%", fig.show='hold'}
d.IO %>% plot_IO("Swedish", d.IO.training, cues = selected_cues)
``` 
...
IO_categorization = function(
  ...
  .data.test %>%
    ...
    mutate(
      ...
      `model_posterior_/t/` = `model_likelihood_/t/` / (`model_likelihood_/d/` + `model_likelihood_/t/`),
      model_posterior = ifelse(Sound == "/d/", `model_posterior_/d/`, `model_posterior_/t/`),
      model_posterior_categorization = ifelse(`model_posterior_/d/` >= .5, "/d/", "/t/")
    ) %>%
    mutate(
      x = NULL
    )
...
IO_ratings = function(.data) {
  ...
  l = lm(
    Rating.Response.d.zscored ~ `model_posterior_/d/` * NativeLanguage,
    data = .data %>%
      # Only use posterior from models that match the exposure group
      filter(model_matches_group == "yes") %>%
      ...
      summarise(
        ...
        `model_posterior_/d/` = mean(`model_posterior_/d/`))
  )
  
  .data %>%
    mutate(
      `model_rating_/d/` = predict(l, newdata = .)
    )
}
```
...
d.test.IO = 
  ...
  mutate(
    model_matches_group = case_when(
      NativeLanguage != Used_for.NativeLanguage ~ NA_character_,
      Group == "control" & Used_for.Accent == "native" ~"yes",
      Group == "control" & Used_for.Accent == "non-native" ~"no",
      Group == "/d/-exposure" & Used_for.Accent == "non-native" ~"yes",
      Group == "/d/-exposure" & Used_for.Accent == "native" ~"no",
      T ~ NA_character_
    )) %>%
  IO_ratings()
...
p.post = d.IO.test %>%
  ggplot(aes(x= Sound, 
           y = `model_posterior`, 
           color = Used_for.Accent, 
           fill = Used_for.Accent)) +
  scale_x_discrete("Intended sound category") +
...
p.post + 
    stat_summary(aes(group = Used_for.Accent), fun.data = mean_cl_boot, geom = "pointrange", 
               alpha = .8, size = .8, 
...
p.post %+% 
  ...
  stat_summary(aes(color = Used_for.Accent), fun.y = mean, geom = "bar",
               alpha = .6, fill = NA,
               position = position_dodge(.93)) +
  stat_summary(aes(group = Used_for.Accent), fun.data = mean_cl_boot, geom = "linerange", 
...
(ref:ratings-results-repeat) Human /d/-goodness ratings during test (z-scored) by native language and exposure group. Replotted from Figure \@ref(fig:ratings-results). Errorbars show 95\% bootstrapped confidence intervals over by-subject means.

(ref:IO-ratings-results) /d/-goodness ratings during test (z-scored) predicted by the native and non-native ideal observers. Shown for both the English and Swedish data. Errorbars show 95\% bootstrapped confidence intervals over by-item predictions (since there aren't multiple subjects).

```{r ratings-results-repeat, fig.cap="(ref:ratings-results-repeat)", fig.height=4.5, out.width='75%'}
p.results.perception.bySubject
```


```{r get-predicted-ratings-from-IO, fig.cap="(ref:IO-ratings-results)", fig.height=4.5, out.width='75%'}
p.results.perception %+%
  (d.test.IO %>%
     filter(model_matches_group == "yes") %>%
     group_by(Language, NativeLanguage, Group, Sound, Item.WordID) %>%
     summarise(Rating.mean = mean(`model_rating_/d/`))) +

...
exclude <- function(.data, excluded_group) {
  .data %>% mutate(Used_for = case_when(Talker == "M5" ~ "training",
                                        Talker == "F1" ~ "training",
                                        Training.Set == excluded_group ~ "exclude",
                                        TRUE ~ "training"))
}
...
train_IO = function(
  ...
  .data %>%
    ...
    select(!!! rlang::syms(setdiff(groups, "Talker")), Model) %>%
    mutate(Training.Iteration = paste0("iter_", excluded_group)) %>%
    applyFactorLevels()
}
...
# After assigning English min.pairs into 4 bins and Swedish min.pairs into 2 bins join the assigned min.pairs with the main test dataframe

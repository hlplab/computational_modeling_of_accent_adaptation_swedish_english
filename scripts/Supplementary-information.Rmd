...
```{r bootstrap-downsampling, warning=F, fig.cap = "(ref:bootstrap-downsampling)", fig.height=4.5, out.width='75%'}
...
lmer_IO <- function(.data, model.sound = "no") {
  if (model.sound == "no") { 
    my.model <- lmer(Rating.Response.d.zscored ~ 1 + scale(Model.Posterior.d) + 
         (1 + scale(Model.Posterior.d) | Item.MinimalPair) +  (1 + scale(Model.Posterior.d) | Participant), 
                     data = .data, 
         control = lmerControl(optCtrl = list(maxfun=20000)), REML = F)
  } else 
    my.model <- lmer(Rating.Response.d.zscored ~ 1 + Sound + scale(Model.Posterior.d) + 
         (1 + Sound + scale(Model.Posterior.d) | Item.MinimalPair) +  (1 + scale(Model.Posterior.d) | Participant), 
                     data = .data, 
         control = lmerControl(optCtrl = list(maxfun=20000)), REML = F) 
}

get_loglik <- function(.data, model.sound) {
  my.model <- lmer_IO(.data, model.sound) %>% 
    logLik()
  return(my.model[[1]])
}

get_tvalue <- function(.data, model.sound) {
  my.model <- lmer_IO(.data, model.sound) %>% tidy()
  if (model.sound == "no") {
    return(my.model[[2, 6]])
  } else
  return(my.model[[3, 6]])
...
models.likelihood <- d.test.IO.wTraining_folds.wTest_folds %>% 
  ...
  mutate(
    ...
    log.likelihood = map2_dbl(.x = data, .y = model.sound, ~ get_loglik(.x, .y)), 
    log.likelihood.per.token = log.likelihood/n.token,
    t.value.posterior = map2_dbl(.x = data, .y = model.sound, ~ get_tvalue(.x, .y))
  ) 
...
models.likelihood %>% 
  ...
  stat_summary(geom = "pointrange", fun.data = mean_cl_boot, size = 0.8, position = position_dodge(width = 0.3)) +
  layer(geom = "point", stat = "identity", position = position_dodge(width = 0.3), params = list(size = 1)) +
...
```{r IO-t-values-posterior-d, fig.cap="(ref:IO-t-values-posterior-d)", fig.height=8.5, fig.width=6.5, out.width='75%'}
my.models <- full_join(models.sound.likelihood, models.likelihood)

my.models %>% mutate(model.label = ifelse(model.sound == "no", "model without sound", "model with sound")) %>% 
  ggplot(aes(x = t.value.posterior, y = log.likelihood.per.token, colour = Group, shape = Model.Group.matchesParticipant.Group)) +
  geom_point(size = 2.5, alpha = 0.7, position = position_jitterdodge(jitter.width = 0.95)) +
  geom_vline( xintercept = 2, linetype = 3) +
  scale_color_manual(values = colors.Accent) +
  scale_shape_manual("IO matches exposure", breaks = c("yes", "no"), values = c(16, 17)) +
  scale_y_continuous(name = "Fit against perceptual responses\n(per token log likelihood") +
  scale_x_continuous(name = "t-value: posterior prediction effect on human ratings") +
  facet_grid(model.label ~ Language)

```
(ref: IO-t-values-posterior-d) **Comparing t-values of model posterior /d/-ratings** when the linear mixed effects regression models include or exclude sound as a predictor. 

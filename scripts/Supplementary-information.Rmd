...
get_average_log_likelihood <- function(data, lapse_rate = .1, bias.d = .5) {
  data %>% 
    mutate(
      prediction.d = (1 - lapse_rate) * Model.Posterior.d + lapse_rate * bias.d,
      log.likelihood = pmap_dbl(.l = list(n.ratings, Rating.Response.d.normalized, prediction.d), .f = function(x, y, z) log_likelihood(x, y, z))) %>% 
    #group_by(Language, Group, Model.Group.matchesParticipant.Group) %>%
...
d.test.IO.n <-  left_join(d.test.IO, 
                          d.test.IO %>% 
                            group_by(Language, Group, Model.Group.matchesParticipant.Group, Item.WordID) %>% summarise(n.ratings = n())) 

d.test.IO.n <- d.test.IO.n %>% group_by(Language, Group, Model.Group.matchesParticipant.Group, Sound, Item.WordID, n.ratings) %>%
  summarise(
    Model.Posterior.d = mean(Model.Posterior.d),
    Rating.Response.d.zscored = mean(Rating.Response.d.zscored)) %>%
  # Normalize human ratings'
...
d.test.nested <- d.test.IO.n %>% crossing(
    ...
    # lapse_rate = 10^c(seq(-10, 0, length.out = 51))
    bias.d = seq(0, 1, length.out = 51),
    lapse_rate = seq(0, 1, length.out = 51)) %>% group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>% 
...
d.test.nested %>% ggplot(aes(x = lapse_rate, y = per_token_loglik, color = Group, linetype = Model.Group.matchesParticipant.Group)) +
  geom_line(alpha = .9) +
  scale_x_continuous(
    "lapse rate",
    breaks = 10^c(seq(-10, 0, 2))) +
...
#  mutate(per_token_loglik = ifelse(per_token_loglik == "-Inf", -280, per_token_loglik))  %>%
  ungroup() %>%
  group_by(lapse_rate, bias.d, Group, Model.Group.matchesParticipant.Group) %>%
  ...
  facet_grid(Group~ Model.Group.matchesParticipant.Group) +
  coord_cartesian(expand = c(0)) +
  theme(legend.position = "top")+ 
 # scale_x_continuous(labels = c("0","0.05","0.1","0.15","0.2")) +
...
d.test.IO.wTraining_folds.wTest_folds.byItem <- 
  d.test.IO.wTraining_folds.wTest_folds %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Training_fold, Test_fold, Sound, Item.WordID) %>%
  summarise(
    Model.Posterior.d = mean(Model.Posterior.d),
    Rating.Response.d.zscored = mean(Rating.Response.d.zscored)) %>%
  # Normalize human ratings'

...
rvowel <- as.numeric(d.IO$rvowel)
rclosure <- as.numeric(d.IO$rclosure)
rburst <- as.numeric(c(min(d.IO$rburst), range(d.temp$rburst))) #include the minimum of the ellipse to fully capture it; use range from test data because its maximum value exceeds ellipse's (without, it misses points)
...
p
# plot Control Exposure English IO separately because plotly confuses the point fills if plotted together

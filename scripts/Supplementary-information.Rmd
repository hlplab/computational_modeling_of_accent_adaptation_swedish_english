...
knitr::opts_chunk$set(echo = FALSE, 
                      message=FALSE, warning=FALSE, error=TRUE, cache = FALSE,
                      fig.width = 4.5,
                      fig.height = 4)
```
...
d.IO.wTraining_folds <- map(
  ...
  reduce(rbind) %>% # why is this form preferred over bind_rows()? they seem to do the same
  group_by(Training_fold, .add = T)
```

We first evaluated the two folded models on the entire test set, following the same procedure outlined in Section \@ref(sec:IO-test). Figure \@ref(fig:IO-ratings-results-byItem-wTraining-folds) plots the predictions of the ideal observers. 
...
With only 50% of the training data in each fold for /d/ and /t/ in the control model and /t/ in the /d/-exposure model, the ideal observer model predictions change substantially for Swedish but maintain the same trend for English. For English, we find that the /d/-exposure model always outperforms the control model regardless of the training fold. This pattern reflects that of the model trained on 100% of the data \@(fig:IO-ratings-results-byItem) and qualitatively, the human ratings \@(fig:ratings-results-byItem). The predictions of the models vary greater between the folds for Swedish especially with respect to the average predictions of /t/-goodness. The relative results of the /d/-exposure and control models trained with the second-fold matched the models trained with 100% of the data since the /d/-exposure model's predictions for /d/-goodness ratings are higher than the control model. This relationship is reversed under the first fold -- the control-model gives marginally higher ratings than the /d/-exposure model. In both folds however, the differences appear small and can be said to qualitatively match human ratings for /d/-goodness. Comparing the results of the first fold for /t/-goodness predictions against the unfolded results, we find that average /t/-goodness ratings by the /d/-exposure and control models falls considerably and that there is almost no difference between the two models. This relative performance between the two models however does match the relative performance of the two human exposure groups. Finally, the second fold seems to have biased the /t/-goodness ratings predictions of the control model as its predictions for /t/ was maximised. Although the human data showed that the control exposure group gave numerically higher average ratings for intended /t/s, it appears that training data from the second fold greatly exaggerates that difference.
...
In an additional analysis, we randomly split the test data into folds of 30 tokens each, resulting in 2 folds for the Swedish test data (which had 60 test tokens) and 4 folds for the English test data (which had 120 test tokens). By comparing across test bins within each training fold, we can estimate how robust our results depend on the specific test bins. Each fold was created by sampling minimal pairs---rather than individual words---to hold the phonological context constant between the /d/ and /t/ test items within each fold. To compare like with like, we then compared the model predictions for each test fold against the human ratings on the same test items. Figure \@ref(fig:IO-ratings-results-byItem-wTraining_folds-folded-test) plots the predictions of the ideal observers. 

(ref:ratings-results-byItem-wTraining-folds-folded-test) **2-fold cross-validated ideal observer /d/-goodness ratings evaluated on separate test folds**. Error bars show 95\% bootstrapped confidence intervals over by-item predictions (since there aren't multiple subjects). Each row shows one training fold excluding a different half of the native training data. Each column shows a separate test bin with 15 randomly chosen minimal pairs (resulting in 4 test folds for the English experiment and 2 test folds for the Swedish experiment). For additional details, see Figure \@ref(fig:IO-ratings-results-byItem).

Overall with test items either quartered for English and halved for Swedish, the confidence interval of the mean predicted goodness ratings of the ideal observers noticeably widened as indicated by the error bars. While there are considerable differences between the predictions of each training-test combination, there is a consistent pattern of the /d/-exposure model surpassing the control model in average predicted ratings for every test fold in the English cross-validation. This result is encouraging and suggests that there was sufficient data to produce a robust model that qualitatively matches the English behavioural results. The Swedish models were expected to display more variable results between tests and this was indeed apparent from the four training-test runs. In some training-test combinations the prediction patterns that followed human performance before were no longer held and instead showed either the /d/-exposure model performing better than control or vice-versa. <!-- is there something that can be deduced about training fold 2 that makes the /t/ model predict /t/ at 100% in all test runs?-->
...
```{r ratings-results-byItem-wTraining-folds-folded-test, fig.cap="(ref:ratings-results-byItem-wTraining-folds-folded-test)", fig.height=8.5, out.width='75%'}

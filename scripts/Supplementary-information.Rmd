...
For English, we followed the same procedure with two exceptions. First, recordings of the /d/- and /t/-final words from the experimental test block were available only from the non-native talker. We instead fitted the effects of phonological context with words from @Li2020database. As mentioned above, the words in @Li2020database. were slightly different from those in @xie2017more's test block. However, both sets of words were mono-syllabic sets of minimal pairs and included the same type of segmental contexts.

All effects were treatment coded and estimated with linear regression models that were the best fit for each cue and language. To achieve this we used the ```validation()``` function available in the rms package for R [@Harrell2020rms]. Each cue was regressed onto the full combination of predictors that could possibly affect its realisation. The predictors considered for English were:
...
For Swedish, phonological vowel length (phonologically long vs. short vowel, e.g., *sladd*---'cord' vs. *ved*---'wood) and a presence of a following nasal were expected to have significant effects on vowel and closure durations. The best-fitting model for the vowel and closure cues was ```cue ~ 1 + number_segments + has_long_vowel + coda_has_nasal```. While those predictors did not affect burst duration we chose to keep the model consistent for burst as well since the estimated effects were insignificant, practically and statistically.
  
In both languages, the correction procedures involved first estimating the phonological context effects for each cue and subsequently obtaining the sum of estimated linear predictor for all terms (estimated coefficient*predictor) except for the intercept and Talker. This sum of predicted terms was then added as an offset term to a separate linear regression of each cue by Talker: ```cue ~ 1 + Talker + offset_segmental_context``` from which the residual cue values are obtained. Effectively, the predicted phonological context effects were taken as constant values to be subtracted from all data.

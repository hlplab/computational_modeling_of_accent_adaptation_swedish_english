...
run_tests <- function(model, test_data, test_bin_count) {
  runs = map(seq(1, test_bin_count), ~ IO_categorization(filter(test_data, Test.Bin == .x), model) %>% droplevels() %>% mutate(RunCount = paste0(Training.Iteration, "_bin", .x))) 
bind_rows(runs)
}
...
join_to_human_ratings <- function(model.result, d.test.humans) {
  joins = d.test.humans %>% droplevels() %>% 
...
test_bin_count = 4
results.CV = map(seq(1,length(models)), ~ run_and_join(models[[.x]], d.nonnative.test, test_bin_count)) 

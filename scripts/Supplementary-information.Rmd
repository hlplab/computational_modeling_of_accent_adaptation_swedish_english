...
plot_cue_matrix <- function(data) {
  data %>% mutate("Vowel" = rvowel.step2,
         "Closure" = rclosure.step2,
         "Burst" = rburst.step2) %>% 
...
```{r native-english-corrected-cues-2D, fig.cap="Native English corrected cues in 1D density plots and 2D pair-wise correlation plots", out.width="75%", fig.show='hold'}
plot_cue_matrix(d.stims.step2 %>% filter(NativeLanguage == "English" & Accent == "native"))
```
...
```{r non-native-english-corrected-cues-2D, fig.cap="Non-native English corrected cues in 1D density plots and 2D pair-wise correlation plots", out.width="75%", fig.show='hold'}
plot_cue_matrix(d.stims.step2 %>% filter(NativeLanguage == "English" & Accent == "non-native"))
```
#### Swedish (native)
...
```{r native-swedish-corrected-cues-2D, fig.cap="Native Swedish corrected cues in 1D density plots and 2D pair-wise correlation plots", out.width="75%", fig.show='hold'}
plot_cue_matrix(d.stims.step2 %>% filter(NativeLanguage == "Swedish" & Accent == "native") )
```
...
```{r non-native-swedish-corrected-cues-2D, fig.cap="Non-native Swedish corrected cues in 1D density plots and 2D pair-wise correlation plots", out.width="75%", fig.show='hold'}
plot_cue_matrix(d.stims.step2 %>% filter(NativeLanguage == "Swedish" & Accent == "non-native"))
```
...
(ref:non-native-swedish-corrected-cues-2D) 1D density plots and 2D pair-wise correlation plots of non-native Swedish cues corrected for both segmental and supra-segmental context.
...
In addition to testing the ideal observer models on the respective non-native talker test data we ran 5-fold cross-validation on each of the models to estimate their test error rate. Each model's training data was apportioned into 5 test sets. For each iteration of ideal observer training, all but one of the test sets were used; the left-out test set was reserved for validation. This process is reiterated until all five test sets are exhausted.
```{r k-fold-CV-of-IO, include = FALSE}
k = 5

# group the training data by talker for each model and divide into 5 parts
d.IO.CV <- d.IO.training %>% group_by(Used_for.Model.NativeLanguage,
                           Used_for.Model.Group,
                           Talker, Used_for.Model.Sound) %>%
  add_count() %>% 
  rename(Length.Data = n) %>%  
  mutate(Segment.Size = Length.Data/k, 
         Count = row_number(), 
         Test.Set = paste0(rep("test_",length.out = 30), 
                           rep(c(1:5), 1, each = 6))) 
...
train_IO = function(
  ...
  .data %>%
    ...
    select(!!! rlang::syms(setdiff(groups, "Talker")), Model) %>%
    mutate(Training.Set = paste0("training_", Training.Set )) %>%
    applyFactorLevels()
}

#Filter d.IO.CV into 5 training sets and pipe into train_IO

training.1 <- d.IO.CV %>% filter(Test.Set != "test_1") %>% 
  train_IO(., cues = selected_cues, Training.Set = 1)

training.2 <- d.IO.CV %>% filter(Test.Set != "test_2") %>% 
  train_IO(., cues = selected_cues, Training.Set = 2)

training.3 <- d.IO.CV %>% filter(Test.Set != "test_3") %>% 
  train_IO(., cues = selected_cues, Training.Set = 3)

training.4 <- d.IO.CV %>% filter(Test.Set != "test_4") %>% 
  train_IO(., cues = selected_cues, Training.Set = 4)

training.5 <- d.IO.CV %>% filter(Test.Set != "test_5") %>% 
  train_IO(., cues = selected_cues, Training.Set = 5)

d.IO.training.models <- rbind(training.1, training.2, training.3, training.4, training.5)
# join with d.IO.CV to have the test sets in the same data frame

# loop for the above?
# for (i in 1:2){
#  d.IO.CV %>% filter(Test.Set != paste("test_",i)) %>%
#   train_IO(., cues = selected_cues, Test.Set = i)
#   }
```
...
# plot_IO_test(d.IO %>% filter(Model.Group == "control"), d.test.IO %>% filter(Model.Group == "control"), language = "English", cues = selected_cues)
``` 
...
Maryann: let's add camera shots

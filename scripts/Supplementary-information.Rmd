...
rburst <- as.numeric(d.IO$rburst)
d.IO.cues <- tibble(rvowel, rclosure, rburst)
cues <- names(d.IO.cues) 
axes <- get_axes(d.IO.cues, cues)
...
#Xin's code:
p =  plot_ly(
          data = d.temp %>% filter(NativeLanguage == "English"),
          x= ~rvowel, # plot the individual tokens
                    y= ~rclosure,
                    z= ~rburst,
          frame= ~Model.Group,
          text = ~paste("Posterior predicted /t/:", signif(Model.Posterior.t, 4),
  ...
  # add_markers(x= ~rvowel, # plot the individual tokens with a large size (show up as the outline color)
  #                   y= ~rclosure,
  #                   z= ~rburst,
  #                   # color = ~Sound,
  #                   # colors = colors.Category,
...
# use get_axes() to set axes
rvowel <- as.numeric(d.temp.diff$rvowel)
rclosure <- as.numeric(d.temp.diff$rclosure)
rburst <- as.numeric(d.temp.diff$rburst)
d.temp.diff.cues <- tibble(rvowel, rclosure, rburst)
cues <- names(d.temp.diff.cues) 
axes <- get_axes(d.temp.diff.cues, cues)
...
p = plot_ly(
        ...
        showlegend = T,
        type="scatter3d",
        text= ~paste("Advantage:", signif(Model.Posterior.Intended.Difference, 4),
                     "<br>vowel:", signif(vowel, 4),
                     "<br>closure:", signif(closure, 4),
                     "<br>burst:", signif(burst, 4)),
        hoverinfo= "text",
        ...
        add_markers(x= ~vowel,
                  ...
                  marker = list(color = ~Model.Posterior.Intended.Difference,
                                ...
                                cmid = 0,
                                cmax = 1,
                                colorbar = list(len = 0.5, title = 'Advantage of\n/d/-exposure over\ncontrol model'),
                                showscale = T)) %>%
 layout(legend = list(itemsizing = "constant"),
        scene = list(xaxis = axes[[1]], 
                     yaxis = axes[[2]], 

...
We apply this model to the present data while considering talker, segmental and supra-segmental phonotactic contexts. A number of properties of the data that were available to us complicate this context-correcting process, requiring us to estimate context effects sequentially on subsets of the data (see \@ref(tab:summary-cue-correction)). This is not an ideal situation but one that many researchers aiming to predictively model speech perception across L1 and L2 accents will have to face. In the following sections, we motivate and describe the two steps in \@ref(tab:summary-cue-correction), laying out for each step the assumptions we make and what consequences they might have for the training and testing of the ideal observers, as well as the comparison of interest between the native and non-native ideal observers across the experiment on English and Swedish.
...
This raises questions as to how we should estimate effects of phonological context. One could estimate the effects of phonological context from native data alone or separately for native and non-native speech. The first models the assumption that non-native speakers do not deviate from native speakers in how they are affected by phonological context, or the assumption that native listeners do *not* learn that non-native speakers deviate from native speakers in how phonological context affects their speech. The second models the assumption that native listeners learn, not only the general cue distribution of non-native speakers, but also how non-native speakers differ from native speakers in how they are affected by phonological context.

As we opted to follow the established procedure outlined in C-CuRE, we computed estimates with native speaker data only as this follows from its principle that native speaker expectations of phonological context effects are derived from native (L1) input and thus aim to estimate those expectations from native data whenever possible. This contrasts with expectations about the talker, which by their very nature involve adaptation to the specific talker.

For Swedish, we *estimated* (fit) the effects of segmental context from the native recordings of the /d/- and /t/-final words from the test block of the behavioural experiment. We then *corrected* (residualized) all data used to test and train the ideal observer, for the predicted effects. We did not include words from the training data in the fitting of the regression, because test and training words differed in terms of word length (corrected for in Step 2 below). Our procedure thus assumes that the effect of segmental context is identical regardless of word length. However, even if this assumption turned out to be wrong, it does not introduce any obvious bias with regard to the ideal observer comparisons. 

For English, recordings of the /d/- and /t/-final words from the experimental test block were available only from the non-native talker. We instead fitted the effects of phonological context with words from @Li2020database. As mentioned above, the words in @Li2020database were slightly different from those in @xie2017more's test block. However, both sets of words were mono-syllabic sets of minimal pairs and included the same type of segmental contexts. For the present analyses we have also set aside all words with nasal codas (e.g. *fond*, *found*) from the @Li2020database. This is because we discovered wider than expected disparities in the annotation of those words compared to that of @xie2017more. The annotations of both databases were done by different individuals but we intend to include them once we ensure that both sets of nasal codas were annotated using consistent criteria.

Given the high number of possible predictors we evaluated the linear regression models that were the best fit for each cue and language with cross-validation methods. We used the ```validation()``` function available in the rms package for R [@Harrell2020rms]. Each cue was regressed onto the full combination of predictors that were expected to affect its realisation. The predictors considered for English were:
...
- onset_has_nasal
- coda_has_nasal
- onset_has_rhotic, and
- Talker (the database of @Li2020database comprised multiple talkers)

The model comparisons employed a group cross-validation method with the backward step-down approach for each training sample. The final best-fitting models for the respective cues for English were as follows:

- ```vowel ~ 1 + number_segments +  coda_has_lateral + onset_has_rhotic + Talker```
- ```closure ~ 1 + number_segments + coda_has_lateral + onset_has_lateral + onset_has_rhotic + Talker ``` 
- ```burst ~ 1 + number_segments + coda_has_lateral + Talker```

For Swedish, phonological vowel length (phonologically long vs. short vowel, e.g., *sladd*---'cord' vs. *ved*---'wood) and the presence of a following nasal were expected to have significant effects on vowel and closure durations. The best-fitting model for the vowel and closure cues was ```cue ~ 1 + number_segments + has_long_vowel + coda_has_nasal```. While those predictors did not affect burst duration we chose to keep the model consistent for burst as well since the estimated effects were insignificant, practically and statistically.
  
In both languages, the correction procedures involved first estimating the phonological context effects for each cue and subsequently obtaining the sum of estimated linear predictors for all terms (estimated coefficient*predictor) except for the intercept and Talker. This sum of predicted terms was then added as an offset term to a separate linear regression of each cue by Talker: ```cue ~ 1 + Talker + offset_segmental_context``` from which the residual cue values were obtained. Effectively, the predicted phonological context effects were taken as constant values to be subtracted from all data.

Across the two languages, there were a total of six linear regressions, summarized in the next six figures. All predictors were treatment coded. Each figure shows the predicted effect (in msecs) and confidence interval for all predictors. Significance is indicated through stars.
...
Supra-segmental factors like---lexical stress and speech rate---are also known to affect the phonetic realization of sounds. For example, segments tend to have longer durations in stressed, compared to unstressed, syllables [@lindblom1963spectrographic; @van1987effects]. Processes like compensatory lengthening/shortening [@peterson1960duration; @lindblom1973some; @fowler1981relationship] can also cause segments to be longer in syllables with fewer segments @klatt1976linguistic, and words with fewer syllables [@rietveld1987effect]. We thus removed effects of lexical stress, and syllable/word complexity on vowel, closure, and burst duration.

The models were estimated using only the /d/-final words, since the syllable-final /t/ (by design) only occurred as mono-syllabic test words. Furthermore, recordings of multi-syllabic /d/-words in our English database were only available for the non-native talker, forcing us to fit the models with non-native data. While the Swedish database had native recordings of multi-syllabic /d/-words we also used the non-native data for fitting to keep the modeling consistent and comparable. Using non-native data for this purpose is not ideal given that C-CuRE is fundamentally meant to estimate the expected cue values based on native realisations.<!-- additional comment to this point--> Our procedure thus assumes that supra-segemental effects are identical for both syllable-final /d/ and /t/. However, even if this assumption turned out to be wrong, it does not introduce any obvious bias with regard to the ideal observer comparisons.

We estimated the effects of the number of syllables in the word and the presence or absence of stress of the final syllable; the model for both languages was```cue ~ 1 + number_syllables + has_stress```. Similar to step 1, we took the sum of all linear predictor terms from this model as a constant (offset_suprasegmental) which was added to a linear regression model from which we obtained the residual cue values. This correction model included Talker as a predictor and the terms from Step 1 as an offset as well: ```cue ~ 1 + Talker + offset_segmental + offset_suprasegmental``` All the data for both English and Swedish were residualised in this final step.

The following plots summarize the segmental and supra-segmental effects on vowel, closure, and burst duration for both languages.
...
Figures \@ref(fig:native-english-cue-before-and-after-suprasegmental-correction) to \@ref(fig:non-native-swedish-cue-before-and-after-suprasegmental-correction) show the distribution of cues to the /t/ and /d/ categories before and after correction for supra-segmental context for the English and Swedish experiment, respectively.
...
There is a clear difference between the English and Swedish data, but the predictions of the non-native vs. native ideal observers qualitatively match the patterns of human ratings in the /d/-exposure vs. control conditions:
  ...
  + For the English data, the Ideal Observers predict significantly better performance from the non-native model over the native model for both /d/ and /t/; this pattern was also reflected in the higher ratings for the intended category (for both /d/ and /t/) in the human ratings.

(ref:IO-ratings-results-byItem) **Ideal observer /d/-goodness ratings** during test (z-scored) predicted by the native and non-native ideal observers. Shown for both the English and Swedish data. Error bars show 95\% bootstrapped confidence intervals over by-item predictions (since there aren't multiple subjects). To facilitate comparison to the human data, predicted ratings were derived from each ideal observer by projecting its predicted posterior probability of /d/ linearly onto the range of the human z-scored /d/-goodness ratings (see Section \@ref(sec:IO-methods-link)). This was done only for the purpose of visualization---all analyses presented below directly predict human ratings from the ideal observers' posterior probabilities.
...
For this first analysis, we predict human responses across all native languages and exposure conditions from the ideal observers that are meant for those languages and exposure conditions. That is, we predict human ratings in the /d/-exposure group from the non-native ideal observer, and human ratings in the control group from the native ideal observer. Specifically, we use a linear mixed-effects regression, predicting human /d/-goodness ratings from the posterior probability of /d/ predicted by the ideal observers, while also including random intercepts by minimal pair. SAY STH ABOUT BY PARTICIAPNT (DIDN'T CONVERGE B/C OF Z-SCORING AND THERE NOT BEING MUCH VARIANCE LEFT). NOW with Participant intercepts.
...
pred.labels = c("Intercept", "Model.Posterior.d")
...
pred.labels = c("Intercept",
                ...
                "English:Model.Posterior.d",
                "Swedish:Model.Posterior.d",
                "English:Model.Accent.match (Yes vs No)",
                "Swedish:Model.Accent.match (Yes vs No)",
                "English:Model.Posterior.d:Model.Accent.match (Yes vs No)",
                "Swedish:Model.Posterior.d:Model.Accent.match (Yes vs No)")
...
Figure \@ref(fig:plot-3d-test-IO-English) and \@ref(fig:plot-3d-test-IO-Swedish) visualises the posterior predictions of the native and non-native Ideal Observers for English and Swedish for each token in the 3D cue space.

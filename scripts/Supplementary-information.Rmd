...
(ref:IO-log-likelihood-wTraining-folds-folded-test) **Per item log likelihood across training-by-test folds**. Shaded areas show 95\% confidence intervals over by-item predictions. Each row shows one training fold excluding a different half of the native training data. Each column shows a separate test bin with 15 randomly chosen minimal pairs (resulting in 4 test folds for the English experiment and 2 test folds for the Swedish experiment). For additional details, see Figure \@ref(fig:IO-ratings-results-byItem).
...
p.loglik.byItem.fold %>% mutate(Model.Group.matchesParticipant.Group = ifelse(Model.Group.matchesParticipant.Group == "yes", "Matching", "Mismatching")) %>% 
  ggplot(mapping = aes(x = lapse_rate, y = per_token_loglik, color = Group, fill = Group)) +
  geom_ribbon(stat = "smooth", alpha = 0.2, aes(colour = NULL, group = Group)) +
  geom_line(stat = "smooth", mapping = aes(line = Group)) +
    scale_x_continuous(
    "lapse rate",
    breaks = 10^c(seq(-10, 0, 2)),
    trans = "log10") +
  ...
  #   labels = c("matching", "mismatching"),
  #   values = 1:2) +
  # scale_shape_manual("Ideal observer",
  #   breaks = c("yes", "no"),
  #   labels = c("matching", "mismatching"),
  #   values = 1:2) +
  #coord_trans(x = "log10") +

...
The validation employed a group cross-validation method with the backward step-down approach for each training sample. The final models for the respective cues were as follows:
```vowel ~ 1 +
```closure ~ 1 +
```burst ~ 1 + 
//-->
...
# d.stims.step1.english <- d.stims.step1 %>% filter(NativeLanguage == "English")
# d.stims.step1.english.s2 <- as_tibble(get_predictions_for_suprasegmental_context("vowel", 
#                              data.fit = d.stims.step1.english,
#                              language = "English"))

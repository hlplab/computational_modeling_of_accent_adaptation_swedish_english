...
train_IO = function(
  ...
  .data %>%
    ...
    select(!!! rlang::syms(setdiff(groups, "Talker")), Model) %>%
    mutate(Training.Set = paste0("training_", Test.Set )) %>%
    applyFactorLevels()
}
...
training.1 <- d.IO.CV %>% filter(Test.Set != "test_1") %>% 
  train_IO(., cues = selected_cues, Test.Set = 1)

training.2 <- d.IO.CV %>% filter(Test.Set != "test_2") %>% 
  train_IO(., cues = selected_cues, Test.Set = 2)

training.3 <- d.IO.CV %>% filter(Test.Set != "test_3") %>% 
  train_IO(., cues = selected_cues, Test.Set = 3)

training.4 <- d.IO.CV %>% filter(Test.Set != "test_4") %>% 
  train_IO(., cues = selected_cues, Test.Set = 4)

training.5 <- d.IO.CV %>% filter(Test.Set != "test_5") %>% 
  train_IO(., cues = selected_cues, Test.Set = 5)

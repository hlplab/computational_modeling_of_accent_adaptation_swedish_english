...
my.models %>% mutate(model.label = ifelse(include.sound == FALSE, "LMER without sound", "LMER with sound")) %>% 
  ...
  scale_shape_manual("IO matches exposure", breaks = c("yes", "no"), values = c(16, 17)) +
  scale_y_continuous(name = "Fit against perceptual responses\n(per token log likelihood") +
  scale_x_continuous(name = "t-value: IO-predicted posterior of /d/") +
  facet_grid(model.label ~ Language)
```
(ref: IO-t-values-posterior-d) **Comparing t-values of ideal observer predicted posterior /d/-ratings** when the linear mixed effects regression models include or exclude sound as a predictor. 

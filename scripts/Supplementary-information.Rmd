...
knitr::opts_chunk$set(echo = FALSE, 
                      message=FALSE, warning=FALSE, error=TRUE, fig.align = 'center')
```
...
p.results.perception.bySubject + scale_y_continuous("Mean /d/-goodness ratings\n(z-scored) by subject")
```
...
```{r ID-to-ratings, fig.cap="(ref:ID-to-ratings)", fig.height = 4.5, out.width="75%", fig.align='center'}
...
## Group-level comparison between human and model-predicted ratings {#sec:results-main}
...
```{r IO-ratings-results-byItem, fig.cap="(ref:IO-ratings-results-byItem)", fig.height=4.5, out.width='75%', fig.align='center'}
...
```{r ratings-results-byItem, fig.cap="(ref:ratings-results-byItem)", fig.height=4.5, out.width='75%', fig.align='center'}
...
## How robust are these results? How much data does this type of modeling require?
Our initial analysis did not employ cross-validation or similar methods. This decision was made since the data the ideal observers are tested on do not overlap with the data the ideal observers are tested on. During the review process, reviewers raised questions about the amount of data that would be required for the type of computational analysis conducted here. Here we address this questions by subsetting the training and test data to assess the variability of the results across different folds of the training and test data. This also allows us to assess the extent to which the results in the previous section are due to overfitting to the training data. 

Our ideal observers are trained on two types of data: data that estimate native listeners' expectations about /d/ and /t/ based on previous lifelong experienced with their native language, and data that estimate the effects of the exposure phase (for the /d/ category in the /d/-exposure condition). For the former data, we randomly created two non-overlapping folds of 50% of the data. The folds were created by sampling (without replacement) the native data described in Section \@ref(sec:IO-training) *within* each unique combination of talker and category. Each model was then trained on one of the two folds (50% of the data, i.e., 15 tokens per category and talker), resulting in two models, each omitting another half of the native training data. Since the exposure data---by hypothesis---form the basis of adaptation, we do *not* resample or subset these data but rather hold them constant across all folds of the training data. By comparing across these two models, we can thus assess the extent to which the specific native data used to train the ideal observers affects our results (recall that these data determine the /t/ category in both the control and the /d/-exposure model, as well as the /d/ category in the control model). At the same time, this analysis sheds light on whether 15 tokens per talker and category are even remotely sufficiently to achieve robust estimates of category means and covariances. This is a question, in particular, for the Swedish models, for which we had phonetically annotated data for only *one* native talker (as opposed to 6 talkers for the English data).  

```{r cross-validation-train-IO, include = FALSE}
...
We first evaluated the two folded models on the entire test set, following the same procedure outlined in Section \@ref(sec:IO-test). Figure \@ref(fig:IO-ratings-results-byItem-wTraining-folds) plots the predictions of the ideal observers. For English, we find that the /d/-exposure model continues to outperform the control model in both training folds. For both training folds, the predictions closely reflect those of the model trained on 100% of the data \@ref(fig:IO-ratings-results-byItem) and thus continue to provide a good qualitative fit for the human goodness ratings in the English experiment (shown in \@ref(fig:ratings-results-byItem)). Specifically, we continue to see that the predicted effects of /d/-exposure, compared to control exposure, are larger for the /t/ category than for the /d/ category, mirroring the human ratings.

For Swedish, the predictions of two models are consistent for the /d/ category: as was the case for the models fit to the complete training data, and mirroring human ratings, the models for both folds find that /d/-exposure should not affect ratings. For the /t/ category, however, the predictions are less consistent across the two folds. Whereas one fold predicts no difference between /d/-exposure and control, the other fold predicts an advantage of control exposure. This latter prediction is driven by the fact that /t/ test items are recognized with 100% accuracy by the model trained on the second training fold (see gray points the bottom-right bar of bottom-right panel of Figure \@ref(fig:IO-ratings-results-byItem-wTraining-folds)). This suggests that the prediction of a small numerical advantage of control exposure for /t/-ratings might reflect over-fitting to the training data, and that the fact that this prediction matched the numerical advantage of control exposure for /t/ observed in the human ratings should be interpreted with caution. What is, however, stable across training folds even for the Swedish /t/ category is that no model predicts and advantage of /d/-exposure over control. Taken together, we find the consistency across training folds surprising, though encouraging, given the relatively small amount of data that the ideal observers were trained on---in particular for Swedish, for which we only had access to recordings from a single native talker (age and gender-matched to the non-native talker).
...
Next, we evaluated the two training folds on separately folded test folds. By comparing across test folds within each training fold, we can estimate how robust our results are as the test items change. Specifically, we randomly split the test data into folds of 30 tokens each (15 minimal pairs), resulting in 2 folds for the Swedish test data (which had 60 test tokens) and 4 folds for the English test data (which had 120 test tokens). Each fold was created by sampling minimal pairs---rather than individual words---to hold the phonological context constant between the /d/ and /t/ test items within each fold. To compare like with like, we then compared the model predictions for each test fold against the human ratings on the same test items. 
Figure \@ref(fig:IO-ratings-results-byItem-wTraining-folds-folded-test) plots the predictions for each combination of training and test fold. As expected given that the number of test items had quartered for English and halved for Swedish, the confidence interval of the mean predicted goodness ratings by the ideal observers noticeably widened. For the English data, the qualitative fit to the human ratings remained across all test folds except for the combination of training-fold 1 and test-fold 1. Here the /d/ ratings prediction by the /d/-exposure model no longer had an advantage over the control model instead they were both matched in average performance. Within each training-fold, the predictions of the control models varied more from test-fold to test-fold but the average predictions of the control models never performed better than the /d/-exposure model predictions. This result is encouraging and suggests that there was sufficient data to produce robust predictions that qualitatively match the human ratings for the English experiment.

The Swedish models were expected to display more variable results between test folds and this was indeed apparent from the four training-test runs. Within the first test fold the /d/-exposure models of both training folds perform better than the control models for /d/ predictions. This does not parallel human ratings. Within the second test fold, both training folds do qualitatively match human ratings. Although the models do not accurately predict the human ratings across both sets of test folds, there is some measure of consistency across training folds. 
...
```{r cross-validation-fold-test}
# Fold test: 
#   1) Take the k-fold IOs from above 
#   2) left_join in test fold IDs 
#      (divide English test data into 4 test folds and Swedish test data into 2 folds, each with 15 minimal pairs per fold)
...
# Maryann: This would also work if one used Training_fold ~ Language + Test_fold in the grid, but I wanted the plots a bit more clearly separate by language,
# and so I made it a plot_grid
plot_grid(
  p.IO_prediction %+% 
    (d.test.IO.wTraining_folds.wTest_folds %>%
...
# TO DO: THIS NEEDS WORK. IS THE LIKELIHOOD FUNCTION DOING THE RIGHT THING?
<!-- # Could add lapse-rate-based fit eval here? -->
```{r}
get_per_token_log_likelihood <- function(data, lapse_rate = .1, bias.d = .5) {
  data %>%
    mutate(
      prediction.d = (1 - lapse_rate) * Model.Posterior.d + lapse_rate * bias.d,
      log_likelihood = log(
        Rating.Response.d.normalized * prediction.d + 
        (1 - Rating.Response.d.normalized) * (1 - prediction.d))) %>%
    summarise(per_token_loglik = sum(log_likelihood) / length(log_likelihood)) %>%
    pull(per_token_loglik)
}

d.test.IO.byItem <- 
  d.test.IO %>%
  # There seem to be points for which no model predictions exist. 
  # Removing them for now (It's two English data points: Item.WordID E104, E105)
  #filter(!is.na(Model.Group.matchesParticipant.Group)) %>%
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Sound, Item.WordID) %>%
  summarise(
    Model.Posterior.d = mean(Model.Posterior.d),
    Rating.Response.d.zscored = mean(Rating.Response.d.zscored)) %>%
  # Normalize human ratings'
  group_by(Language, Group, Model.Group.matchesParticipant.Group) %>%
  mutate(
    Model.Posterior.d = Model.Posterior.d,
    Rating.Response.d.normalized = (Rating.Response.d.zscored - min(Rating.Response.d.zscored)) / (max(Rating.Response.d.zscored) - min(Rating.Response.d.zscored))) 

d.test.IO.byItem %>%
  # Crossing in lapse rate and apply likelihood
  crossing(
    bias.d = .5,
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>%
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>%
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), .f = function(x, y, z) get_per_token_log_likelihood(x, y, z)))) %>%
  ggplot(aes(x = lapse_rate, y = per_token_loglik, color = Group, linetype = Model.Group.matchesParticipant.Group)) +
  geom_line(alpha = .9) +
  scale_x_continuous(
    "lapse rate",
    breaks = 10^c(seq(-10, 0, 2))) +
  scale_y_continuous("Fit against perceptual responses\n(per token log likelihood)") +
  scale_color_manual("Exposure group",
    breaks = c("/d/-exposure", "control"),
    labels = c("/d/-exposure", "control"),
    values = colors.Accent) +
  scale_linetype_manual("Ideal observer",
    breaks = c("yes", "no"),
...
d.test.IO.byItem %>%
  # Crossing in lapse rate and bias and then apply likelihood
  crossing(
    lapse_rate = seq(0, 0.2, length.out = 51),
    bias.d = seq(0, 1, length.out = 51)) %>%
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>%
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), .f = function(x, y, z) get_per_token_log_likelihood(x, y, z)))) %>%  
  drop_na() %>%
  ggplot(aes(x = lapse_rate, y = bias.d, fill = per_token_loglik)) +
  ...
  scale_y_continuous(
    "bias towards /d/",
    breaks = seq(0, 1, .1)) +
  scale_fill_viridis_c("Fit against perceptual responses\n(per token log likelihood)") +
  coord_cartesian(expand = 0) +
  facet_grid(Group + Model.Group.matchesParticipant.Group ~ Language) +
  theme(panel.grid.minor = element_blank())

# Make difference plots (AFTER FIGURING OUT WHETHER THESE FUNCTIONS DO THE RIGHT THING)
```

### Comparing model likelihoods
In this section we walk-through how we assess the ideal observer models' goodness-of-fit to the respective human experimental groups. One way to do this is to compare the average likelihood of test token ratings given the predictions of its representative ideal observer (e.g. human ratings by the /d/-exposure group under the /d/-exposure ideal observer; "matching" ) against its average likelihood under an ideal observer that does not represent that group (e.g. human ratings by the /d/-exposure group under the control exposure ideal observer; "non-matching).

Given our assumption of a linear link between categorisation and ratings response, the rating of each token was treated as a Bernoulli process. The rating responses for an item can thus be considered a binomial distribution of the number of /d/-responses given out of the total number of responses, given the model's posterior prediction of /d/ for that item, $$p$$. 

To obtain the likelihood of response for each test token we first averaged participant ratings and normalised each token's average rating response to be between 0 and 1. We took the number of participant ratings for each token as the number of trials, $$n$$ and the number of trials multiplied by the per-token normalised ratings as thenumber of success, $$k$$. As this would result in non discrete values for $$k$$ we used an extended definition for the binomial distribution for non-integer values.

<!-- as this we  each test token within an exposure group there is a given number of participant ratings expressed as /d/-ratings and z-scored. The number of ratings, n We multiply n.ratings by the average normalised ratings per item (n.ratings*average normalised.ratings) to get the number of successes, k. For each item we can get the likelihood under the respective model's parameters, which is it's posterior for d after factoring in the lapse and bias rates. After the per token log likelihood is obtained, the data is grouped by Language, Model, Model.Group.matchesParticipant.Group and the average per token log likelihood is computed, summarising the result to 8 log likelihood values. -->

The likelihood of each token is obtained through the binomial distribution: $$L(k) = \binom{n}{k}p^k(1-p)^{n-k}$$.The log of which gives: $$LL(k) = log\binom{n}{k}+ k\ log(p) + (n-k)log(1-p)$$. Averaging the per-token log likelihoods gives the average per-token log likehood under each model. This was obtained for the /d/-exposure matching and non-matching, and control exposure matching and non-matching models respectively. 

For Mandarin-accented English the matching /d/-exposure ideal observer model was XXX [NEED HELP DETERMINING THE RATIO -- LRT?]  times more likely to correctly predict the human ratings than the non-matching model. This pattern was reversed for the control models where the mismatching models gave a better fit. Matching and non-matching models for Swedish showed little difference in goodness-of-fit. 

### Model fit under lapse rates and categorisation biases
To check model fit under different scenarios of (non-neural) noise we modified the model posterior predictions for /d/ to include different rates of attentional lapses in decisions and biases.

```{r log-likelihood-function}

# The kth falling power of r starts with r and multiplies together k numbers, each being one less than its predecessor. Given this notation, define the binomial coefficients as
k_fall <- function(n, k){
  sequence <- seq(n, n-k+1)
  as.numeric(sequence) %>% reduce(`*`) #as.numeric is required because seq() makes integers.
}

binom_coeff <- function(n, k) {
  #message("binom_coeff: n=",n," k=",k,"\n")
  if(k >= 0) {
    #message("k_fall:",k_fall(n, k))
    k_fall(n, k)/factorial(k)
  } 
  else (k = 0) 
}

log_likelihood <- function(n, rating, p) {
  #message("vars: ",n," ",rating," ",p,"\n")
  #message("bin:",binom_coeff(n, n*rating))
  #message("mid:",(n*rating)*log(p))
  #message("last:",(n-n*rating)*(log(1-p)))
  log(binom_coeff(n, n*rating)) + (n*rating)*log(p) + (n-n*rating)*log(1-p)
}

# nbelow I removed pull(per_token_likelihood) because I figured we want a column of average per token log likelihood for each model. pull() seems to cause problems.
# at the baseline assumption the lapse rate should be zero?
get_average_log_likelihood <- function(data, lapse_rate = 0, bias.d = .5) {
  data %>% 
    mutate(
      prediction.d = (1 - lapse_rate) * Model.Posterior.d + lapse_rate * bias.d,
      log.likelihood = pmap_dbl(.l = list(n.ratings, Rating.Response.d.normalized, prediction.d), 
                                .f = function(x, y, z) log_likelihood(x, y, z)))  
    # %>% summarise(per_token_loglik = mean(log.likelihood)) 
 # %>% pull(per_token_loglik)
  }

# mutate column with number of ratings for each item
d.test.IO.n <- left_join(d.test.IO, d.test.IO %>% 
                           group_by(Language, 
                                    Group, 
                                    Model.Group.matchesParticipant.Group, 
                                    Item.WordID) %>% 
                           summarise(n.ratings = n())) 

d.test.IO.n <- d.test.IO.n %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Sound, Item.WordID, n.ratings) %>%
  summarise(
    Model.Posterior.d = mean(Model.Posterior.d),
    Rating.Response.d.zscored = mean(Rating.Response.d.zscored)) %>%
  # Normalize human ratings'
...
# no log likelihoods for Swedish data if the lapse rate is 0
Model.likelihood <- get_average_log_likelihood(d.test.IO.n) 

Model.likelihood %>% filter(Language == "Flemish-accented Swedish")

# now cross-in a sequence of different lapse rates 
d.test.nested <- d.test.IO.n %>% 
  crossing(
    # bias.d = 10^c(seq(-10, 0, length.out = 51)),
    # lapse_rate = 10^c(seq(-10, 0, length.out = 51))
    bias.d = .5, 
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>% 
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), .f = function(x, y, z) get_average_log_likelihood(x, y, z))))

# try plotting. The figures are moving in the opposite direction to the previous fitting method. Don't know if this makes sense --it's saying that the higher the lapse rate, the better the fit? For English, the general pattern of d-exposure-matching performing better than control-matching is there, and d-exposure-nonmatching worse than control-non matching. This is stable all the way. The Swedish data looks a bit crazy.
...
# see how the log likelihood changes as a function of lapse rate and lapse bias

d.test.nested <- d.test.IO.n %>% crossing(
  # lapse_rate = seq(0, 1, length.out = 51))
    bias.d = seq(0, 1, length.out = 51),
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
  nest() %>% 
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), 
                                        .f = function(x, y, z) get_average_log_likelihood(x, y, z))))

d.test.nested %>%
  #filter(per_token_loglik != "-Inf") %>%
  ungroup() %>%
  group_by(lapse_rate, bias.d, Group, Model.Group.matchesParticipant.Group, Language) %>%
  summarise(per_token_loglik = as.integer(mean(per_token_loglik))) %>%
  ggplot(aes(x = round(lapse_rate), y = bias.d, fill = per_token_loglik)) +
  geom_raster(interpolate = T) +
  scale_fill_viridis_c("Average log likelihood by token") + 
  facet_grid(Language + Group~ Model.Group.matchesParticipant.Group) +
  theme(legend.position = "top")+ 
  coord_cartesian(xlim = c(0,1), ylim = c(0,1)) +
  scale_x_continuous(labels = c("0","0.25","0.5","0.75","1")) +
  xlab("Lapse rate") + ylab("Lapse bias")

```


```{r IO-wTraining-folds-folded-test}
d.test.IO.wTraining_folds.wTest_folds <- left_join(d.test.IO.wTraining_folds.wTest_folds, 
                                                          d.test.IO.wTraining_folds.wTest_folds %>% 
                                                            group_by(Language, Group,
                                                                     Model.Group.matchesParticipant.Group,
                                                                     Training_fold, Test_fold, Item.WordID) %>% 
                                                            summarise(n.ratings = n()))


d.test.IO.wTraining_folds.wTest_folds.byItem <- d.test.IO.wTraining_folds.wTest_folds %>% 
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Training_fold, Test_fold, Sound, Item.WordID, n.ratings) %>%
  summarise(
    Model.Posterior.d = mean(Model.Posterior.d),
    Rating.Response.d.zscored = mean(Rating.Response.d.zscored)) %>%
  # Normalize human ratings'
...
# cross in variable lapse rate
loglik.byItem.fold.lapse <- d.test.IO.wTraining_folds.wTest_folds.byItem %>% 
   crossing(
    bias.d = .5,
    lapse_rate = 10^c(seq(-10, 0, length.out = 51))) %>%
  group_by(Language, Group, Model.Group.matchesParticipant.Group, Training_fold, Test_fold, lapse_rate, bias.d) %>%
  nest() %>%
  mutate(per_token_loglik = unlist(pmap(.l = list(data, lapse_rate, bias.d), 
                                        .f = function(x, y, z) get_average_log_likelihood(x, y, z))))


#cross in variable d-bias and variable lapse rate
loglik.byItem.fold.lapse.bias <-  d.test.IO.wTraining_folds.wTest_folds.byItem %>% 
  crossing(
    lapse_rate = seq(0, 0.2, length.out = 51),
    bias.d = seq(0, 1, length.out = 51)) %>%
  group_by(Language, Group, Model.Group.matchesParticipant.Group, lapse_rate, bias.d) %>%
...
```{r IO-log-likelihood-wTraining-folds-folded-test, fig.cap="(ref:IO-log-likelihood-wTraining-folds-folded-test)", fig.height=8.5, fig.width=10.5, out.width='100%'}


loglik.byItem.fold.lapse  %>% mutate(Model.Group.matchesParticipant.Group = ifelse(Model.Group.matchesParticipant.Group == "yes", "Matching", "Mismatching")) %>% 
  ggplot(mapping = aes(x = lapse_rate, y = per_token_loglik, linetype = Model.Group.matchesParticipant.Group, color = Group)) +
  geom_ribbon(stat = "smooth", alpha = 0.2, aes(colour = NULL, fill = Group)) +
  geom_line(stat = "smooth", mapping = aes(line = Model.Group.matchesParticipant.Group, color = Group)) +
    scale_x_continuous(
    "lapse rate",
    breaks = 10^c(seq(-10, 0, 2)),
    trans = "log10") +
  ...
  scale_linetype_manual("Ideal Observer", breaks = c("Matching", "Mismatching"), values = c(1, 2)) +
  facet_grid(Group ~ Language) +
  theme(panel.grid.minor = element_blank())

ggsave(filename = "../figures/IO-log-likelihood-wTraining-folds-folded-test.png",
       width = 10.5, height = 8.5)
  
```

(ref:IO-log-likelihood-wTraining-folds-folded-test) **Per item log likelihood across training-by-test folds** Lines show per item log likelihoods at each lapse rate smoothed over all training and test folds, for the /d/-exposure and control models. Solid lines represent ideal observers that match human exposure groups while broken lines are models that do not match human exposure groups. Shaded areas show 95\% confidence intervals over the fitted lines. 


```{r IO-log-likelihood-wTraining-folds-folded-test-bias-lapse, fig.cap="(ref:IO-log-likelihood-wTraining-folds-folded-test)", fig.height=8.5, fig.width=10.5, out.width='100%'}


loglik.byItem.fold.lapse.bias %>% ggplot(aes(x = lapse_rate, y = bias.d, fill = per_token_loglik)) +
  geom_raster(interpolate = T) +
  scale_x_continuous(
    "lapse rate",
    breaks = seq(0, .2, .05)) +
...
This analysis is summarized in Table \@ref(tab:ratings-IO-analysis) and found a highly significant effect of ideal observer predicted posterior probabilities on the z-scored human goodness ratings ($\hat{\beta} = 1.25, t = 25.09, p < .001$). 
...
Next, we asked whether the effect of the ideal observer predicted posterior probabilities differed depending on the native language, exposure group, and/or the test token's intended category label. To this end, we repeated the above analysis while incrementally including additional control predictors in the model. We started by adding only native language, then the full factorial of native language and exposure condition, and finally the full factorial of native language, exposure condition, and sound category. At each step, we asked whether the effect of the ideal observer predicted posterior probability of /d/ was still significant. All control predictors were sum-coded (native language: English = -1 vs. Swedish = 1; exposure group: control = -1 vs. /d/-exposure = 1; sound category: /t/ = -1 vs. /d/ = 1), and all analyses contained the the maximal by-item random effect structure (random intercept and slopes for exposure group, sound category, and their interaction) and by-participant random intercepts (more complex random effect structures by participant did not converge).

The effect of the ideal observer predicted posterior probability of /d/ remained significant when native language or its full factorial with exposure group were added to the model ($p$s $< .02$), but stopped being a significant predictor once the tokens' correct (intended) category label was included in the model. This final analysis is summarized in Table \@ref(tab:ratings-IO-analysis-full). Together with the first analysis, this suggests that the effect of the ideal observer predicted posterior probability is largely driven by the fact that the ideal observer correctly predicts whether a sound should be categorized as /d/ or /t/. The fact that the ideal observer predicted posterior probability of /d/ is no longer a significant predictor of /d/-goodness ratings when tokens' intended category label is included as a predictor is not surprising: the test data for both experiments comes from natural productions (rather than experimenter-controlled resynthesized speech), so that comparatively few of our test tokens fell into the phonetic space exactly between the /d/ and /t/ categories. This means that comparatively few of our test tokens fell into the space for which the gradient posterior probability of /d/ or /t/ provides information that goes beyond what is already contained in the category label. In short, there is exceedingly little power to detect effects of the ideal observer predicted posterior probabilities *within* each category.

```{r}
# summary(lmer(Rating.Response.d.zscored ~ 1 + NativeLanguage * Group + Model.Posterior.d  + (1 + Group * Sound | Item.MinimalPair) + (1 + Model.Posterior.d | Participant),
#                       data = d.test.IO %>%
#                         filter(Model.Group.matchesParticipant.Group == "yes")))

lmer.IOtoRating.full <- lmer(Rating.Response.d.zscored ~ 
                               1 + NativeLanguage * Group * Sound + Model.Posterior.d + 
                               (1 + Group * Sound | Item.MinimalPair) + (1 + Model.Posterior.d | Participant),
                     data = d.test.IO %>%
                       filter(Model.Group.matchesParticipant.Group == "yes"))
...
Next, we asked whether ideal observers that match the exposure conditions (i.e., the control ideal observer for the control condition and the /d/-exposure ideal observer for the /d/-exposure condition) provide a better fit against human ratings than ideal observers that do not. This would provide the strongest item-level evidence that the ideal observers we constructed capture the effects of exposure in the English and Swedish experiments. To this end, we repeated the linear mixed-effects analysis while including the posterior predictions for both matching and non-matching ideal observers in the analysis and letting them interact with an indicator variable (sum-coded: non-matching = -1 vs. matching = 1) and the native language (sum-coded: English = -1 vs. Swedish = 1). The analysis again contained random intercepts by participant and minimal pair.

This analysis replicates the effect of ideal observer predicted posterior probabilities on the z-scored human goodness ratings ($\hat{\beta} = 1.2, t = 35.4, p < .0001$). A simple effect analysis reported in Table \@ref(tab:ratings-IO-model-comparison) confirmed that the effect of ideal observer predicted posterior probabilities was highly significant for both languages. No other effect, including any of the interactions with the indicator variable, reached significance.   
...
# summary(lmer(Rating.Response.d.zscored ~ 1 + NativeLanguage * cModel.Posterior.d * Model.Group.matchesParticipant.Group + 
#                (1 | Item.MinimalPair) + 
#                (1 | Participant), data = d.test.IO))

lmer.IOtoRating.ModelComparison =
  lmer(
    Rating.Response.d.zscored ~ 1 + NativeLanguage / (cModel.Posterior.d * Model.Group.matchesParticipant.Group) + 
...
In short, we did *not* find evidence that ideal observers that match the exposure condition of human participants did significantly better in predicting human goodness ratings. One possible reason for this becomes apparent when we visualize the predictions of the ideal observers. Figure \@ref(fig:plot-3d-test-IO-English) and \@ref(fig:plot-3d-test-IO-Swedish) visualises the posterior predictions of the control and /d/-exposure ideal observers for all test token. Figure \@ref(fig:plot-3d-test-IO-English-difference) and Figure \@ref(fig:plot-3d-test-IO-Swedish-difference) show the *difference* between the predictions of the control and /d/-exposure ideal observers. These latter figures thus highlight the test tokens that can distinguish between the control and /d/-exposure models. 

On the one hand, the results are encouraging: the /d/-exposure model predicts the intended (non-native) category better for English but not for Swedish, paralleling the results for human ratings. On the other hand, these plots also show clear limitations of the present approach: for many test tokens, the control and /d/-exposure ideal observers make basically identical predictions, limiting our ability to distinguish between the two models. This provides a straightforward explanation as to why ideal observers that match the exposure condition do not provide a better fit against human ratings: the control and /d/-exposure models do not differ sufficiently enough in their predictions. We note that this is not *necessarily* a limitation of the ideal observers but likely at least in part is a limitation of experiments that use natural speech stimuli as test tokens. These natural speech stimuli were not chosen so as to maximize the statistical power to detect differences between the exposure groups (or ideal observers) but rather reflect the natural distribution of the relevant non-native accent.
...
(ref:plot-3d-test-IO-English-difference) Difference between /d/-exposure and control ideal observer predicted posterior probabilities of the *intended* category for all test tokens of the experiment in English. The posterior probability predicted by the control ideal observer is subtracted from the posterior probability predicted by the /d/-exposure ideal observer. Positive values thus indicate test tokens for which the /d/-exposure ideal observer makes better predictions.

(ref:plot-3d-test-IO-Swedish) Ideal observer predicted posterior probabilities for all test tokens of the experiment in Swedish. Shapes indicate the intended sound category and color indicates whether the posterior probability of that *intended* category was low or high. 

(ref:plot-3d-test-IO-Swedish-difference) Difference between /d/-exposure and control ideal observer predicted posterior probabilities for all test tokens of the experiment on Swedish. 
...
For the English data, the /d/-exposure ideal observer correlates with human ratings in *both* exposure conditions, whereas the control ideal observer did not correlate with human ratings in either exposure condition. That is, the /d/-exposure ideal observer performed better in predicting human ratings regardless of whether it matched the exposure condition. This is unexpected. While the correlation with human ratings was numerically highest when the ideal observer's assumptions matched the exposure condition, follow-up analyses reported below found that this difference was not significant. For the Swedish data, both the native and the /d/-exposure ideal observers exhibited significant correlations of with human ratings. The magnitude of these correlations did not differ depending on whether the ideal observer matched the exposure condition. In short, while the ideal observers correctly predict the qualitative pattern found in the human results, the item-level analyses suggest that the ideal observers either predict the correct qualitative patterns for the wrong reasons, or that the ideal observers ....XXXX
...
2. Are the correlations stronger for the consistent pairing (consistence coded as "yes": /d/-exposure and /d/-exposure model; control and control model)?
...
ggscatter(data = d.test.IO %>%
            group_by(Language, NativeLanguage, Group, Model.Group, Model.Group.matchesParticipant.Group, Item.Word, Sound) %>%
            summarise(
              Model.Posterior.d = mean(Model.Posterior.d),
  ...
  facet_grid(NativeLanguage + Group ~ Model.Group.matchesParticipant.Group,
...
# Belief updating (DRAFT)

We use incremental Bayesan belief updating to .... MORE HERE.

We set the prior $m_0$ and $S_0$ for the /t/ and /d/ categories based on the category mean $\mu$ and covariance $\Sigma$ observed for the native data (i.e., the control model) and then consider a range of values for the strengths of the prior beliefs about the mean ($\kappa$) and covariance ($\nu$). Specifically, for each $\nu$, $S_0$ is set so that the *expected value* of the covariance matrix $\Sigma$ is identical to the observed category covariance in the native data. We consider $\kappa$ = 1, 2, 4, ..., 512 and $\nu$ = 8, 16, ..., 512 (the model is only well defined for $\nu > D + 1$, where $D$ is the number of cues).

```{r belief-updating-setup}
if (!file.exists("../models/IBBU_updates.RData")) {
  d.IBBU <-
    d.IO.training %>%
    # Make priors from control model's training data
    filter(Used_for.Model.Group == "control") %>%
    rename(
      NativeLanguage = Used_for.Model.NativeLanguage,
      category = Used_for.Model.Sound) %>%
    # Assume kappa = nu = 8 in generating the prior
    # (this choice is arbitrary but needs to be taken into account below when different nus are
    # considered since the scatter matrix S_0 is set to yield the sample Sigma given the assumed
    # nu),
    make_NIW_prior_from_data(
      groups = "NativeLanguage",
      cues = c("rvowel", "rclosure", "rburst"),
      kappa = 8,
      nu = 8) %>%
    select(-c(kappa, nu)) %>%
    # Consider a range of prior strengths
    crossing(
      kappa = 2^(0:9),
      nu = 2^(3:9)) %>%
    # Correct the S_0 for each nu so that it's still the case that the expected covariance
    # matrix Sigma matches the sample covariance matrix.
    mutate(
      S = map2(S, nu, ~ get_Sigma_from_S(.x, 8) %>% get_S_from_Sigma(nu = .y))) %>%
    # Make a copy of kappa and nu and keep it around as a grouping variable (factor)
    mutate(
      prior_kappa = factor(kappa),
      prior_nu = factor(nu)) %>%
    # Update each prior with exposure data from /d/-exposure group
    group_by(NativeLanguage, prior_kappa, prior_nu) %>%
    group_map(
      ~ update_NIW_beliefs_incrementally(
        .x,
        # Select exposure data to be only /d/-exposure data for /d/ category
        # for the matching NativeLanguage
        exposure = d.IO.training %>%
          filter(
            Used_for.Model.NativeLanguage == .y$NativeLanguage,
            Used_for.Model.Sound == "/d/",
            Used_for.Model.Group == "/d/-exposure") %>%
          rename(
            NativeLanguage = Used_for.Model.NativeLanguage,
            category = Used_for.Model.Sound),
        add_noise = NULL),
      .keep = T) %>%
    reduce(rbind)
  save(d.IBBU, file = "../models/IBBU_updates.RData", compress = T)
} else {
  message("Loading previously calculated incremental Bayesian belief updates. If you have made changes to the training data of the ideal observers, delete the file IBBU_updates.RData under models/ and then rerun this code.")
  load("../models/IBBU_updates.RData")
}
```

Does super quick updating indeed provide the best fit against the *ground truth* (not yet the human responses)? This is not guaranteed because the training and test data (for /d/) is not the same. Plotting the posterior probability of the correct category under the IBBU model for English (top) and Swedish (bottom).

```{r IBBU-sanity-check, fig.width=10}
d.IBBU.test <-
  d.IBBU %>%
  # select final (updated) IOs
  filter(observation.n == 30) %>%
  full_join(
    # Get test data (after removing IO information and removing duplicate rows)
    d.test.IO %>%
      select(
        NativeLanguage, Item.MinimalPair, Sound, rvowel, rclosure, rburst) %>%
      distinct()) %>%
  make_vector_column(
    cols = c("rvowel", "rclosure", "rburst"),
    vector_col = "x") %>%
  arrange(NativeLanguage, prior_kappa, prior_nu, Item.MinimalPair, Sound, category) %>%
  group_by(NativeLanguage, prior_kappa, prior_nu) %>%
  select(Item.MinimalPair, Sound, x, category, m, S, kappa, nu, lapse_rate) %>%
  nest(belief = c(category, m, S, kappa, nu, lapse_rate)) %>%
  nest(data = c(Item.MinimalPair, Sound, x)) %>%
  # Get posterior probability of /d/ for each data point
  mutate(IBBU.Posterior.d = map2(
    data,
    belief,
    ~ get_posterior_predictive_from_NIW_belief(.x$x, .y, log = F) %>%
      mutate(ID = rep(1:(nrow(.)/2), 2)) %>%
      group_by(ID) %>%
      pivot_wider(
        values_from = pp,
        names_from = category) %>%
      transmute(posterior = `/d/` / (`/d/` + `/t/`)) %>%
      pull(posterior))) %>%
  # Get posterior probability of correct category
  # (and then collapse to overall probability of data)
  mutate(
    IBBU.Posterior.Intended = map2(
      data,
      IBBU.Posterior.d,
      ~ ifelse(.x$Sound == "/d/", .y, 1 - .y)),
    IBBU.Posterior.Intended.AllData = unlist(map(
      IBBU.Posterior.Intended,
      ~ prod(.x)^(1/length(.x)) %>% unlist())))
    #~ get_categorization_from_NIW_belief(.x$x, .y, decision_rule = "proportional")))

d.IBBU.test %>%
  ggplot(aes(x = prior_kappa, y = prior_nu, fill = IBBU.Posterior.Intended.AllData)) +
  geom_raster(interpolate = T) +
  scale_fill_viridis_b("Avg. posterior probability\nof correct category\nunder IBBU") +
  facet_wrap(~ NativeLanguage)
```

Predicting participants' responses for English (top) and Swedish (bottom).
...
d.test.IBBU <-
  ...
  full_join(
    ...
    d.test.IO %>%
      select(
        NativeLanguage,
        Item.MinimalPair, Sound, Rating.Response.d.zscored,
        rvowel, rclosure, rburst) %>%
      distinct()) %>%
  make_vector_column(
    cols = c("rvowel", "rclosure", "rburst"),
  ...
  arrange(NativeLanguage, prior_kappa, prior_nu, Item.MinimalPair, Sound, category) %>%
  group_by(NativeLanguage, prior_kappa, prior_nu) %>%
  select(Item.MinimalPair, Sound, x, Rating.Response.d.zscored,
         category, m, S, kappa, nu, lapse_rate) %>%
  nest(belief = c(category, m, S, kappa, nu, lapse_rate)) %>%
  nest(data = c(Item.MinimalPair, Sound, x, Rating.Response.d.zscored)) %>%
  # Get posterior probability of /d/ for each data point
  mutate(IBBU.Posterior.d = map2(
    data,
    belief,
    ~ get_posterior_predictive_from_NIW_belief(.x$x, .y, log = F) %>%
      ...
      transmute(posterior = `/d/` / (`/d/` + `/t/`)) %>%
      pull(posterior))) %>%
  # Get posterior probabiltiy of correct category
  # (and then collapse to overall probability of data)
  mutate(
    IBBU.Posterior.Correlation = unlist(map2(
      data,
      IBBU.Posterior.d,
      ~ cor(.x$Rating.Response.d.zscored, .y))))

d.test.IBBU %>%
  filter(NativeLanguage == "English") %>%
  ggplot(aes(x = prior_kappa, y = prior_nu, fill = IBBU.Posterior.Correlation)) +
  geom_raster(interpolate = T) +
  scale_fill_viridis_b("Correlation between\nIBBU posterior\nand participant ratings")

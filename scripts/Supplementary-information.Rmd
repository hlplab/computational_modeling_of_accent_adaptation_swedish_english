...
### THIS RUNS INTO PROBLEMS AT TEST.BIN == 3 BECAUSE THERE ISN"T BINS 3 & 4 for SWEDISH
### MODEL1:TEST.BIN1 RETURNS 120 ROWS
### MODEL1:TEST.BIN3 RETURNS 60 ROWS WITH MODEL.NATIVELANGUAGE valuing to "English" only. Trips up the mutate() condition in join_to_human_ratings
run_tests <- function(model, test_data, test_bin_count) { 
  runs = map(seq(1, test_bin_count), ~ IO_categorization(filter(test_data, Test.Bin == .x), model) %>%  mutate(RunCount = paste0(Training.Iteration, "_bin", .x)) %>% droplevels() %>%  join_to_human_ratings(d.test)) 
bind_rows(runs)
}
...
CV.data <- results.CV %>% bind_rows() %>% group_by(Training.Iteration, Test.Bin)


p.english <-  CV.data %>% filter(Model.NativeLanguage == "English") %>% 
...
p.swedish <- CV.data %>% filter(Model.NativeLanguage == "Swedish") %>% 
  ggplot( aes(x=Sound, y=Model.Posterior.Intended)) + 
  stat_summary(aes(fill = Model.Group), fun = mean, geom = "bar", 
               alpha = .6,
               position = position_dodge(.93)) +
...
p.results.perception.bySubject %+%
  CV.data %>%
     filter(Model.NativeLanguage == "English", Model.Group.matchesParticipant.Group == "yes") %>%
     group_by(Language, NativeLanguage, Group, Sound, Item.WordID, Model.Group) %>%
     summarise(Rating.mean = mean(Model.Posterior.d * 2 - 1)) +
  ylab("Ideal observer predicted\n/d/-goodness rating (z-score aligned)") +
  scale_fill_manual("Ideal observer",

...
IO_categorization = function(
  ...
  .data.test %>%
    ...
    mutate(
      ...
      model_posterior = ifelse(Sound == "/d/", `model_posterior_/d/`, 1 - `model_posterior_/d/`),
...
d.test.IO = 
  ...
  mutate(
    ModelGroupConsistency = case_when(
...
d.test.IO.rating <- d.test.IO %>%
  ...
  select(NativeLanguage, Dataset, Group, Participant, Used_for.Accent, Consistence, Sound, Item.MinimalPair, Item.Word,  rvowel, rclosure, rburst, Posterior, Rating.Response.d.zscored) %>%
...
```{r IO-ratings-results, fig.cap="(ref:IO-ratings-results)", fig.height=4.5, out.width='75%'}
p.model <- ggplot(data = d.test.IO.rating %>%
                    mutate(Used_for.Accent = factor(Used_for.Accent, levels = c("Non-native", "Native"))),
            aes(x=Sound, y = PredictedRating.d, color = Used_for.Accent, fill = Used_for.Accent)) + 

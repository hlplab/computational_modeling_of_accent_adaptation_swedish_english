...
d.swedish %<>%
  ...
  mutate(
    ...
    Item.Word = str_replace(stri_trans_general(
      ...
      "Latin-ASCII"),
      "^[A-Z]+_[0-9]+_([a-z]+)_.*$", "\\1"),
   Item.Word = ifelse(Item.WordID == "S15" & Item.Word == "nod", "nod15", as.character(Item.Word))
  ) %>%
...
d.test = d.swedish %>%
  ...
  mutate_at(
    c("Group", "Participant", 
      "Block.Order", "Item.MinimalPair", "Item.WordID", "Item.Word",
      "Rating.for"),
    .funs = factor
  )
...
```{r bootstrap down-sampling of analysis, warning=F}
set.seed(333)
...
This result does not, however, entail that power explains the null results we observed for Swedish. Indeed, other aspects of the bootstrap results argue *against* this conclusion: even though the downsampled data did not provide high power, the bootstrapped samples reliably showed a clear difference between the English and Swedish data. For instance, even in the downsampled English data, the simple effect went in the predicted direction (regardless of significance) in `r d.bootstrap$p.simple.English.direction.expected`\% of the samples. Significant two-way interactions in the *opposite* of the originally observed direction occurred in only `r d.bootstrap$p.simple.English.significant - d.bootstrap$p.simple.English.significant.direction.expected`\% of all samples (`r 1-(d.bootstrap$p.simple.English.significant.direction.expected/d.bootstrap$p.simple.English.significant)`\% of the samples with significant effects). For the downsampled Swedish data, the simple effect of the two-way interaction between exposure group and sound was significant in the predicted direction in `r d.bootstrap$p.simple.Swedish.significant.direction.expected`\% of all bootstrap samples. Tellingly, the effect went in the predicted direction in only `r d.bootstrap$p.simple.Swedish.direction.expected`\% of the samples. In line with our main analysis, this bootstrap result suggests that the effect of exposure on goodness ratings---i.e., the two-way interaction between exposure group and sound---differs between the Swedish and English data. Indeed,  for `r d.bootstrap$p.interaction.significance`\% of the bootstrap samples, the three-way interaction between native language, exposure group, and sound went in the same direction as in the original data. This interaction was significant in `r d.bootstrap$p.interaction.significant.direction.expected`\% of the bootstrap samples. Significant interactions in the opposite direction occurred in `r d.bootstrap$p.interaction.significant-d.bootstrap$p.interaction.significant.direction.expected`\% of all samples. 
...
# select the appropriate subset (matching what are available for Swedish data) from English modeling results
...
d.IO = results.swedish %>%
  ...
  mutate_if(is.character, as.factor) # there are a total of 180 words across the two datasets; each word has 2 ModelType (TS or native) and 2 category_model (d or t)
   
d.IO <- left_join(d.IO, d.test %>%
...
ggplot(data = d.IO %>%
         ...
         mutate(Accuracy = Posterior),
            aes(x= Sound, y = Accuracy, color = ModelType, fill = ModelType)) + 
...
#### Linking functions: ID task, rating task and Ideal Observer predictions
...
d.english.ID <- read.csv("../data/English/d.identification.xin.csv")
...
d.english.ID <- d.english.ID %>%
 ...
 mutate(
   ID.Response.d = case_when(
    (Sound == "d" & ID.Response == 1) | (Sound == "t" & ID.Response == 0) ~ 1,
    (Sound == "t" & ID.Response == 1) | (Sound == "d" & ID.Response == 0) ~ 0),
   NativeLanguage = "English",
   Participant = paste0("E", Participant),
   Group = ifelse(Group == "experimental", "/d/-exposure", "control"),
   Block.Type = "test",
...
d.test.IO.english.ID <- left_join(d.english.ID, d.IO %>%
                            filter(NativeLanguage == "English") %>%
                              ungroup(), by = c("NativeLanguage", "Item.Word", "Sound"))


```


```{r item-level-consistence-between-ID-rating-tasks, fig.height = 6, fig.width = 12}
##-------------------------------------------------------------------------------------
...
ggscatter(d.english.test.byItem, x = "Proportion.d", y = "zRating.d", 
         # color = "Sound", 
          ...
          xlab = "Proportion of /d/ from ID task", ylab = "/d/ goodness from rating task") + 
...
We considered a linear (smoother = "lm") and a non-linear (smoother = "gam") relationship between human responses from the two tasks (Proportiona of /d/ in ID task and zscored-Goodness-as-/d/ in rating task). It appears that there was a perfectly linear relationship between the two tasks at the item level. We thus assume that we can use the same regression model to obtain PredictedAccuracy from the posterior of /d/ in Ideal Observers.
...
```{r visualize-cue-before-and-after-residualization}
...
d.resid <- bind_rows(
  ...
  select(-CategoryModel, -Lhood, -Posterior, -Posterior.Choice) %>%
  mutate(ResidualizationType = factor(ResidualizationType, levels = c("Prior", "Post"))) %>%
  distinct() %>%
  mutate(Metric = factor(paste0(NativeLanguage, ResidualizationType), 
                         levels = c("EnglishPrior", "EnglishPost", "SwedishPrior", "SwedishPost"))
        ) 


plot_ly(data = d.resid,
               x= ~vowel,
               y= ~closure,
               z= ~burst,
               color= ~Sound,
...
d.model.parameter.ellipse = d.model.parameter %>%
  ...
  ungroup() %>%
  mutate(Sound = factor(Sound, levels = c("d", "t")))

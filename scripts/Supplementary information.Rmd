...
### English
```{r wordlist-English}
wordlist.english.exposure <- read_csv("../data/English/English_exposure_wordlist.csv")
wordlist.english.exposure %>% replace_na(list(`Critical (/d/-Exposure)` = "-", `Critical (Control)` = "-" , `Filler` = "-" )) %>%
...
```{r test-wordlist-English}
  wordlist.english.test <- read_csv("../data/English/d.test.csv")
wordlist.english.test %<>% distinct(Word, MinimalPairID, Sound) %>% 
  arrange(MinimalPairID, Sound) %>% 
  transmute(`/d/-final` = ifelse(Sound == "d", Word, NA),
         `/t/-final` = ifelse(Sound == "t", Word, NA)) 

#wordlist.english.test %<>% filter(!is.na(`/d/-final`) & !is.na(`/t/-final`))

knitr::kable(wordlist.english.test, booktabs = TRUE, caption = 'English - Test')

...
d.exposure = 
  d.swedish %>%
  filter(Block.Type == "exposure") %>%
  select(NativeLanguage, Group, Participant, Block.Type, Item.Type, LD.Correct, LD.RT) %>%
  ...
  full_join(d.english.exposure %>%
              ...
              mutate(
                NativeLanguage = "English",
                Participant = paste0("E", Participant + 100),
                Item.Type = ifelse(Item.Type == "non.word", "non-word", Item.Type),
                Block.Type = "exposure",
                LD.RT = NA,
                Count = case_when(
                  Item.Type == "non-word" ~ 90,
                  Item.Type == "filler" ~ 60,
                ...
                accuracy = NULL) %>%
              group_by(NativeLanguage, Group, Participant, Block.Type, Item.Type, LD.Correct) %>%
              tidyr::expand(Count = 1:Count) %>%
              mutate(Count = NULL),
            by = c("NativeLanguage", "Group", "Participant", "Block.Type", "Item.Type", "LD.Correct")) %>%
    mutate_at(
    c("NativeLanguage", "Group", "Participant", "Block.Type", "Item.Type"),
    .funs = factor
  )
```
...
### Swedish
```{r wordlist-Swedish}
wordlist.exposure <- read_csv("../data/Swedish/Swedish_Exposure_wordlist.csv")
wordlist.exposure %>% replace_na(list(`Critical (/d/-Exposure)` = "-", `Critical (Control)` = "-" , `Filler` = "-" )) %>%
...
wordlist.test <- read_csv("../data/Swedish/Swedish_test_wordlist.csv")
knitr::kable(wordlist.test, booktabs = TRUE, caption = 'Swedish - Test')
```
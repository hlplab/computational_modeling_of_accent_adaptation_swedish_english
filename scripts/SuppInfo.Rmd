...
\parindent=5pt\paragraph{Goodness ratings}
Participant ratings were modelled using linear mixed-effects regression. Word-final sound type (d:1; t:-1) was contrast coded, as was the sound rated for (d:1; t:-1), and exposure group (control:-1; \/d\/-exposure: 1)
...
=======
>>>>>>> cab6ef28fa35dda46ba263e714c0ec3257d22f31
...
d.swedish.expoosure.BySubject = d.swedish %>%
...
d.swedish.exposure.ByGroup <- d.swedish.expoosure.BySubject %>% 
  group_by(Group) %>% 
...
levels(d.swedish.expoosure.BySubject$Group) <- c("Control", "/d/-Exposure")
d.swedish.expoosure.BySubject %>%
  ggplot(
...
The full list of stimuli is available in section XXX for both Swedish and English. In both experiments participants in both groups heard a total of 180 words, including the same 60 filler words in the respective languages and 90 non-words that obeyed English or Swedish phonotactical rules where applicable. The remaining 30 words were the critical words, manipulated between exposure groups.

### English ###
The /d/-exposure group heard 30 critical words with between three and four syllables, ending with \/d\/, and without \/t\/-final minimal pair neighbors (e.g. \textit{overload}). The replacement words (e.g., animal) were matched to the critical -/d/ words in syllabic length and mean lemma frequency in CELEX (Baayen, Piepenbrock, & Gulikers, 1995). All words or nonwords were multisyllabic and contained three to four syllables. Other than the critical /d/-final words, no other alveolar stops, voiced stops, dental fricatives, and postalveolar affricates occured. The voiceless stops (/k/ and /p/) did not appear in word-final position. 
...
In place of the critical words, the Control group heard 30 replacements with no occurences of \/d, t, b, g\/. These words were matched in syllable length and average base form frequency according to selected balanced corpora of 25 million words available on the Korp database of Språkbanken (https://spraakbanken.gu.se/korp, accessed January 2019). The words were presented in random order.

Unintended by the design, one critical word in the /d/-exposure group contained a medial /d/ (*medellivslangd*--- average life span), one pseudoword contained medial /d/ (*mörvinder*), and one pseudoword contained final /t/ (*spållrivet*).

### Why the difference?
Other than the unintended occurences of stops, differences in sound exlusion for the Swedish stimuli were dictated by the distribution of these sounds in the language. Excluding them completely would result in fewer exposure words.

### Is this difference likely to explain the results?
The medial /d/ in *medellivslängd* occurs as the onset of the second syllable (*me-del*) and thus is unlikely to have provided evidence of a voiced-final /d/. Crucially, the control group did not hear this word. The  occurence of medial /d/ and final /t/ in the pseudoword list are also unlikely to have provided sufficient evidence for perceptual learning as has been shown in prior experiments using this paradigm (cf. Norris, McQueen \& Cutler, 2003). In addition, the fact that the control group had little difficulty judging /d/ from /t/ words even without prior exposure suggests it would not have granted benefit to the /d/-exposure group.
...
Test stimuli consisted of 32 \/d-\/t\/-final minimal pair words (e.g., *röd* and *röt*) as well as 32 \/d-\/t\/-initial minimal pair words (e.g., *dom* and *tom*), matched in phonological vowel length. The pairs were spread across four lists (two for word-final pairs, two for word-initial pairs) such that the two words of a pair did not appear on the same list. The order in each list was randomised for each participant. 

### Why the difference?
We included the set of \/d-\/t\/-initial minimal pair words to investigate the question of whether exposure to devoiced /d/-final words might result in higher goodness ratings for \/d-\/t\/-initial words. The results were not included in the current analysis.  While there are many /d/-/t/-final minimal pairs in Swedish a large number of them are different forms of the same word and hence semantically identical which we chose to exclude. Additionally, we relaxed the sound restrictions when selecting the minimal pairs, allowing voiced stops to occur word initially (e.g. *bädd-bett*) in order to have a sufficient number of pairs. 

### Is this difference likely to explain the results?
Compared to the English experiment, the Swedish experiment had 50\% fewer items. 
## Procedure ##
...
The test phase followed immediately after the exposure phase. Participants were given written instructions on the screen which told them to rate the final sound of the words they heard for how good they represented a named category (either /d/ or /t/) on a 1-7 scale. This was performed in  two blocks which was counter-balanced across participants. The experimenter checked that the participant understood the task. In addition to the two final-sound ratings blocks they were asked to rate the \/d\/-\/t\/-*initial* words for how good the first sound represented a named category. The order of these latter two blocks was counter-balanced but participants always completed the word-final ratings *before* performing the word-initial ratings. The data from word-initial ratings tasks are not part of the current analysis.

### Why the difference?


### Is this difference likely to explain the results?
Because the word initial-ratings were always either the third or fourth block any additional random effects resulting from prolonged testing or changing of task would not have affected the word-final judgments.

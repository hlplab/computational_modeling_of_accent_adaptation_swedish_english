...
# Pipes
get_accuracy = . %>% 
  group_by(Language, Group, Participant.ID) %>% 
  summarise(Accuracy = mean(LD.Correct)) %>% 
...
d.swedish %<>%
  ...
  mutate(
    Group = if(sum(ifelse(Item.Type == "ECW", 1, 0)) > 0) 
      "control" else "training",
    Trial = row_number()
  ) %>%
  ungroup() %>%
  mutate_at(
    c("Item.MinimalPair", "Item.WordID", "Participant", "Group", "Rating.for"),
    .funs = factor
  ) %>%
  ...
  select(Language, List, 
         ...
         Block, Block.Type, 
         Trial, 
         Item.MinimalPair, Item.WordID, Item.Type, Item.Word, Item.WordStatus,
         Sound.Type, Sound.Position,
         LD.Expected, LD.Answer, LD.Correct,

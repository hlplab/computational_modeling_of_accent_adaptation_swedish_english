...
get_accuracy = . %>% 
  ...
  summarise(Accuracy = mean(LD.Correct, na.rm = T)) %>% 
  group_by(Language, Group) %>% 
  summarise(Accuracy.mean = mean(Accuracy, na.rm = T), 
            Accuracy.sd = sd(Accuracy, na.rm = T)) %>%
  ungroup() %>%
  mutate_at(vars(starts_with("Accuracy")),
...
get_accuracy = function(
  ...
  d %<>% 
    filter(Language %in% language, Group %in% group, Block.Type == "exposure") %>% 
    droplevels() %>% 
    get_accuracy() 
  ...
  for (l in language) {
    ...
    for (g in group) {
      text = paste0(
        ...
        "M = ", 
        d[d$Language == l & d$Group == g, "Accuracy.mean"], 
        ", SD = ",
        d[d$Language == l & d$Group == g, "Accuracy.sd"])
      if (
        !and(
...
d.swedish.exposure.BySubject = d.swedish %>%
  ...
  summarise(
    meanAccuracy = mean(LD.Correct),
    meanAccuracy.critical.training = mean(ifelse(
      Item.Type == "critical" & Group == "/d/-exposure", 
      LD.Correct, 
      NA), na.rm = T),
...
d_exposure.accuracy <- d.swedish %>%
  filter(Block.Type == "exposure" & Group == "/d/-exposure") %>%
  group_by(Group) %>% 
  summarise(meanAccuracy = mean(LD.Correct))

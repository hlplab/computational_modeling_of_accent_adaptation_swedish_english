...
d.swedish.exposure.BySubject = d.swedish %>%
  ...
  summarise(
    ...
    meanAccuracy.critical.training = mean(ifelse(Item.Type == "critical" & Group == "training", key_resp_2.corr, NA), na.rm = T),
    meanAccuracy.critical.control = mean(ifelse(Item.Type == "critical" & Group == "control", key_resp_2.corr, NA), na.rm = T),
      meanAccuracy.critical = mean(ifelse(Item.Type == "critical", key_resp_2.corr, NA), na.rm = T),
    meanRT = mean(key_resp_2.rt),
          meanAccuracy.nonword = mean(ifelse(Item.WordStatus == "non-word", key_resp_2.corr, NA), na.rm = T),
    meanRT = mean(key_resp_2.rt)
  ) 

d.swedish.exposure.ByGroup <- d.swedish.exposure.BySubject %>% 
  group_by(Group) %>% 
  summarise( mean_group = mean(meanAccuracy), mean_critical.training = mean(meanAccuracy.critical.training, na.rm=T), mean_critical.control = mean(meanAccuracy.critical.control, na.rm=T), SD = sd(meanAccuracy), SD_critical.training = sd(meanAccuracy.critical.training, na.rm=T), SD_critical.control = sd(meanAccuracy.critical.control, na.rm=T))
...
d.swedish.exposure.BySubject %>%
  ...
  ggplot(aes(x = meanRT, y = meanAccuracy.critical.training, label = participant, fill = Group) ) + 
  geom_label() + scale_fill_manual( values = c( "#80d0ff")) + scale_y_continuous(name = "Mean Accuracy - Critical Words", breaks = c(0, 0.25, 0.5, 0.75, 1.0), limits = c(0, 1.0)) + xlab("Mean Reaction Time") + theme_cowplot()

d.swedish.exposure.BySubject %>%
  ggplot(aes(x = meanRT, y = meanAccuracy.critical, label = participant, fill = Group) ) + 
  geom_label() + scale_fill_manual( values = c( "#a6a6a6","#80d0ff")) + scale_y_continuous(name = "Mean Accuracy - Critical Words", breaks = c(0, 0.25, 0.5, 0.75, 1.0), limits = c(0, 1.0)) + xlab("Mean Reaction Time") + theme_cowplot()

d.swedish.exposure.BySubject %>%
  ggplot(aes(x = meanRT, y = meanAccuracy.nonword, label = participant, fill = Group) ) + 
  geom_label() + scale_fill_manual( values = c( "#a6a6a6","#80d0ff")) + scale_y_continuous(name = "Mean Accuracy - Pseudowords", breaks = c(0, 0.25, 0.5, 0.75, 1.0), limits = c(0, 1.0)) + xlab("Mean Reaction Time") + theme_cowplot()
```

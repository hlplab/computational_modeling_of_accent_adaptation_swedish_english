...
d.test = d.swedish %>%
  ...
  select(NativeLanguage,
         ...
         Sound, Sound.Position, 
         Rating.for, Rating.Response, Rating.RT) %>%
  # Bind in English test data about here --- NOT YET DONE !!!!!!!!!
...
```{r prepare-test-data}
##--------------------------------
## Test ratings: tranform ratings for comparison
##--------------------------------

# tranformation: change all ratings to rating_for_d; if rating for t is 1, then it equates to rating_for_d = 7

d.swedish <- d.pilot.swedish %>%
  filter(Block.Type == "test") %>%
  ungroup() %>%
  filter(Sound.Position == "final") %>%
  mutate(
    rating.simplified_d = ifelse(Rating.for == "d", Rating.Answer, 8 - Rating.Answer) 
  ) %>%
  group_by(Participant) %>%
  mutate(
    zrating.simplified_d = as.numeric(scale(rating.simplified_d))
  ) %>%
  ungroup()
...
```{r sanity-check-rating}
# examining by-item rating_for_t and rating_for_d (tranformed into /t/ goodness) for tranformed ratings
d.english %>%
  group_by(Group, Rating.for, Sound, Word) %>%
  summarise(meanRating_z = mean(zrating.simplified_d)) %>%
  spread(Rating.for, meanRating_z) %>%
  # mutate(t_trans = 8-d) %>%
  ggplot(
    aes(x = d, y = t, label = Word, fill = Group)
  ) + 
  geom_label() + 
  xlab("Zrating_as_d (from /d/ goodness)") +
  ylab("Zrating_as_d_trans (from /t/ goodness)") +
  ggtitle("Ratings from goodness_as_/d/ and goodness_as_/t/\nXin's data")

# show table of mean ratings by group
t1<- d.english %>%
  group_by(Group,Participant,Sound,Rating.for) %>%
  summarise(meanRating_d = mean(zrating.simplified_d)) %>%
  group_by(Group, Sound, Rating.for) %>%
  summarise(mean_d = mean(meanRating_d))
...
```{r plot-compare-between-experiments}

# bar plots
ggplot(data = d.test.all.rating %>%
         mutate(Group = factor(Group, levels = c("control", "/d/-exposure"))) %>%
         group_by(dataset, native_language, Group, Sound, Participant) %>%
         summarise(meanRating_d = mean(zrating.simplified_d)),
aes(x= Sound, y = meanRating_d, color = Group)) + 
stat_summary(fun.y = "mean", geom = "point", alpha = 1, size= 0.3, stroke=3, position = position_dodge(.9)) +
  facet_grid(.~dataset, scales="free_y") +
  geom_dotplot(binaxis = "y", stackdir = "center", alpha = .3,  dotsize = 0.5, position = position_dodge(.9)) +
  	stat_summary(fun.data = mean_cl_boot, geom = "pointrange", linetype=1, size = 1, position = position_dodge(.9)) +
  scale_color_discrete(breaks = c("control", "/d/-exposure"), labels = c("control", "/d/-exposure")) +
  ggtitle("Mean /d/ goodness from rating task (transformed)")
```
...
```{r lmer-compare-between-experiments}
# variable coding for mixed effects models

d.test.all.rating <-  within(d.test.all.rating, {
...
contrasts(d.test.all.rating$native_language)
contrasts(d.test.all.rating$Group)
contrasts(d.test.all.rating$Sound)


# The regression contained the maximal random effect structure justified by the design (random by-participant intercepts and slope for sound; random by-item intercepts and slopes for sound, exposure group, and their interaction; item IDs did not overlap between experiments)

# using z-scored ratings
rating.lmer <- lmer(zrating.simplified_d ~ native_language*Group*Sound + (1 + Sound*Group|MinimalPairID) + (1 + Sound | Participant), data = d.test.all.rating)
print(summary(rating.lmer))

# using un-z-scored ratings
# rating.lmer <- lmer(rating.simplified_d ~ native_language*Group*Sound + (1 + Sound*Group|MinimalPairID) + (1 + Sound|Participant), data = d.test.all.rating)
# print(summary(rating.lmer))

rating.lmer.simple <- lmer(zrating.simplified_d ~ native_language/Group*Sound + (1 + Sound|MinimalPairID) + (1 + Sound|Participant), data = d.test.all.rating)
# print(summary(rating.lmer.simple))
swedish.table = tidy(rating.lmer.simple, effects = "fixed")
...
```{r lmer-compare-between-experiments-downsampled}

set.seed(101)

# down-sample the number of participants in Xin's experiment to the size of Maryann's experiment and re-examine the 3-way interaction
# after the downsampling, models using maximal random effects structure does not converge. So change the RE structure so that they converge.
...
for(i in 1:n_times){
SS.downsampled = d.test.all.rating %>%
  distinct(native_language, Group, Participant) %>%
  group_by(native_language, Group) %>%
  sample_n(., 11, replace = TRUE) %>%
  droplevels()

Item.downsampled = d.test.all.rating %>%
  distinct(native_language, MinimalPairID) %>%
...
rating.lmer.downsampled <- lmer(zrating.simplified_d ~ native_language*Group*Sound + (1 |MinimalPairID) + (1|Participant),
                    data = d.test.all.rating %>% 
                      filter(MinimalPairID %in% levels(Item.downsampled$MinimalPairID)) %>%
...
rating.lmer.downsampled.simple <- lmer(zrating.simplified_d ~ native_language/Group*Sound + (1 |MinimalPairID) + (1|Participant),
                    data = d.test.all.rating %>% 
                      filter(MinimalPairID %in% levels(Item.downsampled$MinimalPairID)) %>%
                      filter(Participant %in% levels(SS.downsampled$Participant)))
...
tally1 <- rating.lmer.table.downsampled %>% 
  filter(term == "native_language1:Group1:Sound1") %>%
  arrange(term) %>%
...
tally1.interaction = tally1 %>%
  filter(term == "native_language1:Group1:Sound1" & direction == "-") %>%
  select(n) %>%
  ungroup() %>%
  summarise(n = sum (n))
tally1.interaction.sig <- tally1 %>%
  filter(term == "native_language1:Group1:Sound1" & significance == "yes" & direction == "-") %>%
  select(n)
  
tally2 <- rating.lmer.table.downsampled.simple %>%
  filter(term == "native_languageEnglish:Group1:Sound1" | term == "native_languageSwedish:Group1:Sound1") %>%
  arrange(term) %>%
  group_by(term, significance, direction) %>%
  summarise(n = n()) %>%
...
tally2.English <- tally2 %>%
  filter(term == "native_languageEnglish:Group1:Sound1" & significance == "yes" & direction == "+")
tally2.Swedish <- tally2 %>%
  filter(term == "native_languageSwedish:Group1:Sound1" & significance == "yes") %>%
  summarise(n = sum(n))
tally2.Swedish.negative <- tally2 %>%
  filter(term == "native_languageSwedish:Group1:Sound1" & significance == "yes" & direction == "-") %>%
  summarise(n = sum(n))
...
tally3 <- rating.lmer.table.downsampled.simple %>%
  filter(term == "native_languageEnglish:Group1:Sound1") %>%
  arrange(term) %>%
...
<!-- Florian: edit to be clear what we're doing here -->
...
To address this possibility, we randomly bootstrapped (with replacement) both the English and the Swedish data to the same number of participants and test tokens included in the analysis of the Swedish data (11 in the /d/-exposure group and 11 in the control group). We then repeated the analyses reported in the main text, comparing the Swedish and (down-sampled) English test data. This process was repeated `r n_times` times. 

`r tally1.interaction$n` out of `r n_times` times we observed a three-way interaction in the predicted direction and `r tally1.interaction.sig$n` out of `r n_times` times, we replicated the critical significant three-way interaction between Language, Exposure group, and Sound category. The simple effect of the two-way interaction between Exposure group and Sound was significant for the English data in `r tally2.English$n` samples out of `r n_times` times (all in the predicted direction), whereas it was significant for the Swedish data in `r tally2.Swedish$n` samples out of `r n_times` times (`r tally2.Swedish.negative$n` in the *opposite* of the predicted direction). 

<!-- Xin: this needs editing to use the actual t-value -->
In addition, `r tally3_comparison` out of `r n_times` times, $t$-value of the English data's two-way interaction is as high as the actual $t$-value of the Swedish data. In short we reliably replicate the difference between the English and Swedish even when the both data sets only have 22 participants. 

...
d.swedish <- d.pilot.swedish %>%
  ...
  # group_by(participant, rating_for) %>%
  # mutate(
  #   zrating.simplified_d_byTask = as.numeric(scale(rating.simplified_d))) %>%
  ungroup()
...
tally2.Swedish <- tally2 %>%
  filter(term == "native_languageSwedish:Group1:Sound1" & significance == "no")
...
`r tally1$n[2]` out of `r n_times` times, we replicated the critical significant three-way interaction between Language, Exposure group, and Sound category. The simple effect of the two-way interaction between Exposure group and Sound was significant for the English data in `r tally2.English$n` samples out of `r n_times` times (all in the predicted direction), whereas it was significant for the Swedish data in `r tally2.Swedish$n` samples out of `r n_times` times (all in the *opposite* of the predicted direction). In addition, `r tally3_comparison` out of `r n_times` times, t-value of the English data's two-way interaction is as high as the actual t-value of the Swedish data. 

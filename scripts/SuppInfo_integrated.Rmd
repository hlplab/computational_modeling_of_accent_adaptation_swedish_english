...
This supplementary information describes the results an dmethods of the perception experiments on Mandarin-accented English and Flemish-accented Swedish, as well as the development and evaluation of the ideal observer models. This latter part includes the description of additional data sets containing information about native pronunciations of the same materials used in the perception experiments on foreign-accented speech (used as part of the ideal observers models).

The analyses of the perception experiments suggest that the native Swedish listeners had an easier time understanding the Flemish-accented speaker than the American listeners had understanding the Mandarin-accented English speaker. This showed in the lexical decision accuracies during exposure, as well as the goodness ratings during test. 

The ideal observer analyses we present investigate *one* possible explanations for this difference: that the phonetic distribution of syllable-final /d/ in the Flemish-accented recordings deviated less from typical native Swedish than the phonetic distributions of syllable-final /d/ in the Mandarin-accented recordings deviated from typical American English. Such a difference in the `accentedness' between the Flemish- and Mandarin-accented recordings could be particular to the specific recordings used in the experiment, reflect the talker-specific pronunciation of the speaker we recorded (e.g., due differences in L2 proficiency), or reflect more general differences between Flemish-accented Swedish and Mandarin-accented English. Regardless of the specific source for the difference between the Flemish- and Mandarin-accented recordings, the ideal observers provide a way to *quantify* the difficulty native listeners are predicted to experience for a specific set of foreign-accented speech stimuli. Finally, we discuss reasons why the difference is likely at least in part specific to the way the Flemish-accented recordings were elicited (rather than reflecting more general properties of the speaker or accent).
...
### Swedish ###
Figures \ref{fig:LD.overall} and \ref{fig:LD.critical} summarize participants' accuracy and reaction time for during exposure. 
...
```{r combined LD accuracy plot, fig.cap=c("\\label{fig:LD.overall}Lexical decision accuracies and RTs in the Swedish experiment by exposure group", "\\label{fig:LD.critical}Lexical decision accuracies and RTs  in the Swedish experiment for the critical words with syllable-final /d/"), fig.height=3.5, fig.width=3.5}
d.swedish %>%
  get_LD_byParticipant() %>%
  ggplot(
    aes(x = RT, 
...
### Converting /d/- and /t/-goodness ratings for analysis

```{r rating figure of correlations between /d/ and /t/ goodness, , fig.cap=c("\\label{fig:Ratings.Correlation}/d/-goodness ratings during test (z-scored) by native language and exposure group"), fig.height=3.5, fig.width=5}
d.test %>%
  ...
  xlab("/d/-goodness rating (z-scored)") +
  ylab("/t/-goodness rating (z-scored)")
```


The /d/- and /t/-goodness ratings could range from 1 to 7. The two types of ratings were highly inversely correlated (see Figure \ref{fig:Ratings.Correlation}). To simplify data analysis, we thus converted  all goodness ratings into /d/-goodness ratings. For /t/-goodness ratings, this means that we subtracted them from 8 (i.e., if a token was given a /t/-goodness of 3, it is tranformed to have a /d/-goodness of 5). This decision does not affect the results: all results remain the same if the original ratings are analyzed while including `rating for` (/d/- vs. /t/-goodness) and all its interactions as a predictor in the analysis. 
Following Xie et al. (2017), we then z-scored /d/-goodness ratings within participants, by subtracting the participant's mean and dividing by the participant's standard deviation. 

### Results

```{r ratings figure, fig.cap=c("\\label{fig:Ratings}/d/-goodness ratings during test (z-scored) by native language and exposure group"), fig.height=3.5, fig.width=5}
d.test %>%
  ...
  geom_dotplot(aes(fill = Group), 
               binaxis = "y", stackdir = "center", color = NA, alpha = .3,  
               dotsize = 0.5, position = position_dodge(.9)) +
  stat_summary(aes(color = Group), fun.data = mean_cl_boot, geom = "pointrange", 
               linetype=1, size = .8, position = position_dodge(.9)) +
...
lmer.rating = lmer(Rating.Response.d.zscored ~ NativeLanguage * Group * Sound + 
         (1 + Sound * Group | Item.MinimalPair) + (1 | Participant), 
       data = d.test)
summary(lmer.rating)

lmer.rating.simple = 
  lmer(Rating.Response.d.zscored ~ NativeLanguage / (Group * Sound) + 
         (1 + Sound * Group | Item.MinimalPair) + (1 | Participant), 
       data = d.test)
summary(lmer.rating.simple)

swedish.table = tidy(lmer.rating.simple, effects = "fixed")
swedish.interaction = swedish.table %>%
  filter(term == "native_languageSwedish:Group1:Sound1") %>%
  select(estimate)
...
### Are differences in the number of participants and test items likely to explain the results?

```{r lmer-compare-between-experiments-downsampled, warning=F}
set.seed(101)

rating.lmer.table.downsampled = data.frame()
rating.lmer.table.downsampled.simple = data.frame()
n_times = 100


for(i in 1:n_times){
SS.downsampled = d.test %>%
  distinct(NativeLanguage, Group, Participant) %>%
  group_by(NativeLanguage, Group) %>%
  sample_n(., 11, replace = TRUE) %>%
  droplevels()

Item.downsampled = d.test %>%
...
rating.lmer.downsampled <- lmer(Rating.Response.d.zscored ~ NativeLanguage*Group*Sound + (1 |Item.MinimalPair) + (1|Participant),
                    data = d.test %>% 
                      filter(Item.MinimalPair %in% levels(Item.downsampled$Item.MinimalPair)) %>%
                      filter(Participant %in% levels(SS.downsampled$Participant)))
summary(rating.lmer.downsampled)
rating.lmer.table = tidy(rating.lmer.downsampled, effects = "fixed")
rating.lmer.table <- rating.lmer.table %>%
  mutate(downsample_n = i)
rating.lmer.table.downsampled = bind_rows(rating.lmer.table.downsampled, rating.lmer.table)

# check simple effects
rating.lmer.downsampled.simple <- lmer(Rating.Response.d.zscored ~ NativeLanguage/Group*Sound + (1 |Item.MinimalPair) + (1|Participant),
                    data = d.test %>% 
                      filter(Item.MinimalPair %in% levels(Item.downsampled$Item.MinimalPair)) %>%
                      filter(Participant %in% levels(SS.downsampled$Participant)))
summary(rating.lmer.downsampled.simple)
rating.lmer.table.simple = tidy(rating.lmer.downsampled.simple, effects = "fixed")
rating.lmer.table.simple <- rating.lmer.table.simple %>%
  mutate(downsample_n = i)
rating.lmer.table.downsampled.simple = bind_rows(rating.lmer.table.downsampled.simple, rating.lmer.table.simple)

}

# print 3-way interaction results
rating.lmer.table.downsampled <- rating.lmer.table.downsampled %>%
  mutate(significance = ifelse(p.value < 0.05, "yes", "no"),
         direction = ifelse(estimate>=0, "+", "-"))

# print simple effects results
rating.lmer.table.downsampled.simple <- rating.lmer.table.downsampled.simple %>%
  mutate(significance = ifelse(p.value < 0.05, "yes", "no"),
         direction = ifelse(estimate>=0, "+", "-"))


#tally the number of significant effects and their direction
tally1 <- rating.lmer.table.downsampled %>% 
  filter(term == "NativeLanguage1:Group1:Sound1") %>%
  arrange(term) %>%
  group_by(term, significance, direction) %>%
  summarise(n = n()) %>%
  mutate(effects = "3-way interaction")

tally1.interaction = tally1 %>%
  filter(term == "NativeLanguage1:Group1:Sound1" & direction == "-") %>%
  select(n) %>%
  ungroup() %>%
  summarise(n = sum (n))
tally1.interaction.sig <- tally1 %>%
  filter(term == "NativeLanguage1:Group1:Sound1" & significance == "yes" & direction == "-") %>%
  select(n)
  
tally2 <- rating.lmer.table.downsampled.simple %>%
  filter(term == "NativeLanguageEnglish:Group1:Sound1" | term == "NativeLanguageSwedish:Group1:Sound1") %>%
  arrange(term) %>%
  group_by(term, significance, direction) %>%
  summarise(n = n()) %>%
  mutate(effects = "simple effects")

tally.table <- bind_rows(tally1, tally2)
print(tally.table)

tally2.English <- tally2 %>%
  filter(term == "NativeLanguageEnglish:Group1:Sound1" & significance == "yes" & direction == "+")
tally2.Swedish <- tally2 %>%
  filter(term == "NativeLanguageSwedish:Group1:Sound1" & significance == "yes") %>%
  summarise(n = sum(n))
tally2.Swedish.negative <- tally2 %>%
  filter(term == "NativeLanguageSwedish:Group1:Sound1" & significance == "yes" & direction == "-") %>%
  summarise(n = sum(n))

# how often in these simulations is the (signed) t-value of the English data's two-way interaction at least as high as the actual t-value on the non-bootstrapped swedish data?
tally3 <- rating.lmer.table.downsampled.simple %>%
  filter(term == "NativeLanguageEnglish:Group1:Sound1") %>%
  arrange(term) %>%
  mutate(comparison = (estimate > swedish.interaction$estimate)) %>%
  group_by(comparison) %>%
  summarise(n = n()) %>%
  filter(comparison == T)
tally3_comparison <- tally3$n
...
To address this possibility, we repeated the linear mixed-effects regression over the combined data from both experiments (reported in the result section above), while down-sampling with replacement both the English and Swedish data to 11 participants per group and 30 minimal pair items during test. This process was repeated `r n_times` times. 

`r tally1.interaction$n` out of `r n_times` times we observed a three-way interaction in the predicted direction and `r tally1.interaction.sig$n` out of `r n_times` times, we replicated the critical significant three-way interaction between native language, exposure group, and sound category. The simple effect of the two-way interaction between exposure group and sound was significant for the English data in `r tally2.English$n` samples out of `r n_times` times (all in the predicted direction), whereas it was significant for the Swedish data in `r tally2.Swedish$n` samples out of `r n_times` times (`r tally2.Swedish.negative$n` in the *opposite* of the predicted direction). 

These results suggest that the Swedish experiment was indeed underpowered (since the two-way interaction in the English data was significant only in `r round(100 * tally2.English$n/n_times, 1)`\% of the bootstrap samples). The results do, however, also show that the Swedish data would be *very* unlikely if the English data presents the ground truth. Indeed, the $t$ value of the two-way interaction between exposure group and sound in the English data was as low or lower as the $t$-value of that interaction in the Swedish data only on `r tally3_comparison` out of the `r n_times` bootstrap samples.
<!-- Xin: this needs editing to use the actual t-value -->
In short, the results of the Swedish perception experiment are unexpected given the effect size observed in the English perception experiment. 
<!-- something about successful replication of the English effect with same stimuli and about the conceptual replication for dutch by eisner? -->
...
Due to a misunderstanding, the Swedish data did not counter-balance whether a word recording was rated for /d/- or for /t/-goodness (e.g., the word *rid*---'ride' was always rated for /d/-goodness; the word *rit* ---'rite' was always rated for /t/-goodness; across minimal pair words, /d/- and /t/-final words were equally often rated for /d/- and /t/-goodness). This is unlikely to cause any problems: as shown in Figure \ref{fig:Ratings.Correlation} /d/- and /t/-goodness ratings were almost perfectly inversely correlation, so that the gain in information from having each word rated for both /d/- and /t/-goodness (across participants) is minimal.
